...
response.flush
	...
	sSQL = "SELECT r.[OrgID] ,category_id, action_autoid ,[mobileoption_latitude] ,[mobileoption_longitude] ,f.action_form_name " _
  		...
  		& " WHERE r.orgid = " & dbsafe(request("orgid")) & " and r.category_id = '" & dbsafe(request("actionid")) & "' and mobileoption_latitude IS NOT NULL " _
	...
	if not oRs.EOF then
		...
		Do While Not oRs.EOF
       			strJS = strJS & "[""<div class=\""info_content\""><h3>" & oRs("action_form_name") & "<\/h3><p>Click <a href=\""javascript:upvote('" & oRs("action_autoid") & "');\"">HERE<\/a> to also report this issue.<\/p><\/div>""]," 
			oRs.MoveNext
		loop
