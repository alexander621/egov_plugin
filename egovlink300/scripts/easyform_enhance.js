  ...
  for (count=0; count<numElements; count++) {
    if (eval("document." + formName + ".elements[" + count + "].type") == "hidden") {
      ...
      if (lcase(left(arg,3)) == "ef:") {
		...
		badElement = eval("document." + formName + "." + elementName);
		if (document.getElementById("errmsg-" + elementName))
			document.getElementById("errmsg-" + elementName).remove();
         	if (err != "")
		{
            		errMsg = errMsg + "   - " + errName + err + "\n";

			//Add span after element
			var newSpan = document.createElement("span");
    			newSpan.id = "errmsg-" + elementName;
    			badElement.parentElement.insertBefore(newSpan, badElement.nextSibling);
			newSpan.innerHTML = "&nbsp;" + err;
			newSpan.classList.add("required");


      			//var get = eval("document." + formName + "." + elementName + ".classList.add('incomplete');");
      			badElement.classList.add('incomplete');
			if (!hasScroll)
			{
				hasScroll = true;
      				get = badElement.scrollIntoView({behavior: "smooth", block: "center", inline: "nearest"});

			}
		}
		else
		{
      			var get = badElement.classList.remove('incomplete');
		}
        }
        else {
  ...
  switch(arg) {
    ...
    case "userpass":
      if (eval("document." + formName + ".userpass1.value") != eval("document." + formName + ".userpass2.value"))
        return " This must be equal to password";
      break;
    ...
    case "phone":
      if (!isPhone(elementValue))
        return " This must be a valid phone number, including area code";
      break;

    case "cc":
      if (!isCreditCard(elementValue))
        return " This must be a valid Credit Card Number";
      break;

    case "ssn":
      if (!isSSN(elementValue))
        return " This must be a valid Social Security Number";
      break;

    case "zip":
      if (!isZipCode(elementValue))
        return " This must be a valid zip code";
      break;
      
    case "passcheck":
      if (!isPassCheck(elementValue))
        return " This must be 5-10 characters in length";
      break;
   ...
   case "stmtdatechk":
    ...
    case "currency":
      if (!isCurrency(elementValue))
        return " This must be in valid currency format";
      break;
    
    case "customerid":
      if (!isCustomerID(elementValue))
        return " This must be a combination of 10 letters/numbers in length";
      break;
	  
	case "locationid":
      if (!isLocationID(elementValue))
        return " This must be a combination of 9 letters/numbers in length";
      break;  
	
	case "ninedigits":
      if (!ninedigits(elementValue))
        return " This must be a combination of 9 numbers only or less in length";
      break;  
	
	case "twodigits":
      if (!twodigits(elementValue))
        return " This must be two digits";
      break;  
      
  	case "threedigits":
      if (!threedigits(elementValue))
        return " This must be three digits";
      break;  
      
  	case "fourdigits":
      if (!fourdigits(elementValue))
        return " This must be three or four digits";
      break; 
	  
  	case "sevendigits":
      if (!sevendigits(elementValue))
        return " This must be seven digits";
      break;  

  	case "asn":
      if (!asn(elementValue))
        return " This must be SA and two digits. Example SA12";
      break;  
      
  	case "ppn":
      if (!ppn(elementValue))
        return " This must be three digits or three digits with a letter. Example 123 or 123B";
      break;
	  
  	case "thirteenchar":
      if (!thirteenchar(elementValue))
        return " This must be less than 13 characters";
      break;  
