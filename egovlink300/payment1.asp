...
If NOT oPaymentServices.EOF Then
	...
	response.write "<div class=group style=""width=600px"">"
	...
	response.write vbcrlf & vbcrlf & "<table>"
	...
	' SUBMIT BUTTON
	If iID <> 22 Then
		response.write vbcrlf & "<tr><td colspan=2 align=right><input onclick=""if (validateForm('frmpayment')) { document.frmpayment.submit(); }"" type=""button"" class=paymentbtn name=""btnsubmit"" value=""CONTINUE"" alt=""CONTINUE""></td></tr></table>"
	Else
...
If NOT oPayPalOptions.EOF Then
	...
	Do While NOT oPayPalOptions.EOF 
		...
		' DYNAMIC VALUES
		response.write vbcrlf &  "<input type=""hidden"" name=""" & oPayPalOptions("paypaloptionname") & """ value=""" & oPayPalOptions("paypaloptionvalue") & """>" & vbcrlf
...
If NOT oPayPalFields.EOF Then
		...
		If oPayPalFields("amount") <> "" Then
			...
			sDisabled = "DISABLED"
			response.write vbcrlf & "<tr><td><b>Payment Amount: </b></td><td>" & curValue &"<input type=""hidden"" name=""amount"" maxlength=""200"" value=""" & curValue &"""></td></tr>"
		Else
			curValue = ""
			sDisabled = ""
...
If NOT oPaymentFields.EOF Then
	...
	Do While NOT oPaymentFields.EOF 
	   ...
	   response.write vbcrlf & "<input type=hidden name=""ef:custom_" & oPaymentFields("paymentfieldsname") &   sValidation &  """ value=""" &  oPaymentFields("paymentfielddisplayname")  & """>"
	
	   ' WRITE ACTUAL FORM VALUE	
	   Select Case oPaymentFields("paymentfieldtype") 
			Case "radio"
				' Radio Buttons
				response.write vbcrlf & "<tr><td colspan=""3"" align=""left"" nowrap=""nowrap""><strong>" & oPaymentFields("paymentfielddisplayname") & "</strong> &ndash; Select from the following list.</td></tr>"
				...
				For alist = 0 to ubound(arrAnswers)
					...
					response.write vbcrlf & "<tr><td colspan=""3"" align=""left"" nowrap=""nowrap""><input value=""" & arrAnswers(alist) & """ name=""custom_" & oPaymentFields("paymentfieldsname") & """ class=""formradio"" type=""radio"" />" & arrAnswers(alist) & "</td></tr>"
				Next
				response.write vbcrlf & "<tr><td colspan=""3"">&nbsp;</td><tr>"

			Case "textarea"
			' TEXTAREA
			response.write vbcrlf & "<tr><td colspan=""2"" align=""left"" nowrap=""nowrap""><b>" & oPaymentFields("paymentfielddisplayname") & " :</b><br /><textarea " & oPaymentFields("paymentfieldattributes") & "  name=""custom_" & oPaymentFields("paymentfieldsname") & """ style=""" & oPaymentFields("paymentfieldstyle") & """ class=""formtextarea""></textarea></td><td> " & oPaymentFields("paymentdesc") & " &nbsp;</td></tr>" & vbcrlf 

			Case Else
			' DEFAULT IS TEXTBOX
			response.write vbcrlf & "<tr><td align=""left"" nowrap=""nowrap""><b>" & oPaymentFields("paymentfielddisplayname") & " :</b></td><td align=""left""> <input type=""text"" " & oPaymentFields("paymentfieldattributes") & "  name=""custom_" & oPaymentFields("paymentfieldsname") & """ style=""" & oPaymentFields("paymentfieldstyle") & """ /></td><td width=""40%""> " & oPaymentFields("paymentdesc") & " &nbsp;</td></tr>" & vbcrlf 
