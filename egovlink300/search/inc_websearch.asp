  ...
  If NewQuery Then
	...
	CompSearch = "(#filename=""*.html"" OR #filename=""*.htm"" OR #filename=""*.xml"" OR #filename=""*.jpg"" OR #filename=""*.gif"" OR #filename=""*.pdf"" OR #filename=""*.asp"") AND (#filename=" & chr(34) & SearchString & "*"& chr(34) &  " OR $contents " & chr(34) &  SearchString & chr(34)  & ")"
	Q.Query = CompSearch
    Q.SortBy = "rank[d]"
	...
	Q.MaxRecords = 300
		...
		'DEEP WILL GIVE YOU ALL SUB DIRECTORIES
		'MANUALLY ADD SUBDIRECTORIES IF SOME ARE TO BE OMMITTED
		'util.AddScopeToQuery Q, server.mappath("../../../montgomeryohio.org") , "shallow"
		util.AddScopeToQuery Q, server.mappath("../../../montgomeryohio.org") , "deep"
		'response.write vbcrlf & "<!-- Mappath() = " & server.mappath("../../../montgomeryohio.org") & " -->" & vbcrlf
...
'
					''DEBUG CODE: 
					'response.write server.mappath("../../../montgomeryohio.org") & "<br>" & vbcrlf
				'End If
		        'oRst.MoveNext
			'Loop
		 'End If
...
' BUILD DISPLAY TABLE WITH MATCHED DOCUMENT INFORMATION
  ...
  Response.Write "<font class=""label"">&nbsp;&nbsp;&nbsp;&nbsp;Web Pages " & NextRecordNumber & " to " & LastRecordOnPage
  ...
  If  Not RS.EOF and NextRecordNumber <= LastRecordOnPage then
		...
		strDisplayTable = strDisplayTable & vbcrlf & "<tr style=""height:26px;""><th class=subheading width=""1%"">Rank</th><th>Document Information</th><th align=left>Last Modified</th><th align=left >File Size</th></tr>"
 end if

 Do While Not RS.EOF and NextRecordNumber <= LastRecordOnPage 
 'if right(RS("vpath"),3) = "htm" or right(RS("vpath"),4) = "html" or right(RS("vpath"),3) = "jpg" or right(RS("vpath"),3) = "gif" or right(RS("vpath"),3) = "pdf" or right(RS("vpath"),3) = "asp" or right(RS("vpath"),3) = "xml" then
   
	strDisplayTable = strDisplayTable & vbcrlf & "<p>"
 ...
 ' Graphically indicate rank of document with list of stars (*'s).
    ...
    ' BEGIN BUILDING STRING THAT CONTAINS ROW DATA
	  ...
	  strDisplayTable = strDisplayTable & vbcrlf & "<tr><td valign=top align=left><IMG SRC=""" & stars & """>&nbsp;"
	...
	next
		...
		strLinkPath = "http://www.montgomeryohio.org" & RS("vpath")

		'if len(strDisplayPath) > 20 then strDisplayPath = "..." & Right(strDisplayPath,17)

		if VarType(RS("DocTitle")) = 1 or RS("DocTitle") = "" then
			strDisplayTable = strDisplayTable & "<a href=""" & strLinkPath & """ class=""RecordTitle""> "
...
<% elseif NextRecordNumber > 1 then %>
	<a href="search_web.asp?action=Go&searchstring=<%=searchstring%>">VIEW ALL RESULTS</a>
<% end if%>
   </td>
  </tr>
</table>
