...
<html>
<head>
  <title><%=oOrganization.GetOrgName()%> E-Gov Payments</title>
...
<div id="content">
	...
	<h3><%=oOrganization.GetOrgName()%> Online Payment Details</h3><br />
...
Function Update_Action(iID)
	...
	If (request("selStatus") <> request("currentStatus")) Or Trim(request("internal_comment")) <> "" Or Trim(request("external_comment")) <> "" OR newCommentTask = 1 Then
			  ...
			  elseif request("internal_comment") <> "" then
				...
				AddCommentTaskComment intComment,request("external_comment"),request("selStatus"), iID,Session("UserID"),iOrgId
