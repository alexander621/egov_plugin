...
<%
  ...
  set oDataPicker = New classOrganization

  sLocationName = trim(GetVirtualName(iorgid))
  lcl_message   = ""
  lcl_name      = ""

  if not containsApostrophe(request("message")) then
     lcl_message = request("message")
  end if

  if not containsApostrophe(request("name")) then
 ...
 'This will return the link within an anchor tag (HTML).
  ...
  if request("returnAsHTMLLink") <> "" then
     if not containsApostrophe(request("returnAsHTMLLink")) then
 ...
 'Now check to see if "click-counter" is to be included in the HTML anchor tags ONLY.
  if lcl_returnAsHTMLLink = "Y" AND request("includeClickCounter") <> "" then
     if not containsApostrophe(request("includeClickCounter")) then
 ...
 'Setup additional features
  ...
  if request("displayLinkText") <> "" then
     if not containsApostrophe(request("displayLinkText")) then
        lcl_displayLinkText = request("displayLinkText")
     end if
  end if

 'Get the starting folder
  if not containsApostrophe(request("folderStart")) then
...
<%
  response.write "<input type=""hidden"" name=""currentpath"" id=""currentpath"" size=""80"" />" & vbcrlf
  response.write "<input type=""hidden"" name=""original_currentpath"" id=""original_currentpath"" size=""80"" />" & vbcrlf
  response.write "<table border=""0"" cellpadding=""3"" cellspacing=""0"">" & vbcrlf

 'BEGIN: Current Folder (dropdown) --------------------------------------------
  if lcl_displayDocuments  = "Y" _
  or lcl_displayActionLine = "Y" _
  or lcl_displayPayments   = "Y" then
     response.write "  <tr>" & vbcrlf
     response.write "      <td>&nbsp;</td>" & vbcrlf
     response.write "      <td>" & vbcrlf
     response.write "          <input type=""text"" name=""currentfolder"" style=""height:20px; width:250px;"" readonly=""readonly"" />&nbsp;" & vbcrlf
     response.write "          <a href=""#"" style=""color:#0000ff"" onclick=""explorer.window.history.back();"" name=""anchorBack"">" & vbcrlf
     response.write "            <img src=""images/up.gif"" alt=""Back"" border=""0"" align=""absmiddle"" />" & vbcrlf
     response.write "          </a>" & vbcrlf
     response.write "      </td>" & vbcrlf
     response.write "  </tr>" & vbcrlf
  end if
 'END: Current Folder (dropdown) ----------------------------------------------
 ...
 'BEGIN: Menu -----------------------------------------------------------------
  ...
  response.write "  <tr>" & vbcrlf
  response.write "      <td rowspan=""3"" valign=""top"">" & vbcrlf
 ...
 'Only display the iframe from the loadtree if one of the features that use it is enabled.
 'No need to show it if we are only wanting to create a URL.
  if lcl_displayDocuments  = "Y" _
  or lcl_displayActionLine = "Y" _
  or lcl_displayPayments   = "Y" then
