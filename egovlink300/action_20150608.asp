...
' 6.5 05/08/08  David Boyer - Now disable the SEND REQUEST button when clicked to submit request.
...
'Check to see if the feature is offline
 ...
 dim sError, oActionOrg, iSectionID, sDocumentTitle
...
'Daytime Phone ---------------------------------------------------------------
 if isDisplay(sMASK,5) then
   	if isRequired(sMASK,5) <> "" then
     		response.write "<input type=""" & lcl_hidden & """ name=""ef:cot_txtDaytime_Phone-text/req"" value=""Daytime Phone"" />" & vbcrlf
    end if
    ...
    response.write "       		(<input type=""text"" value=""" & left(sHomePhone,3) & """ name=""skip_user_areacode"" onKeyUp=""return autoTab(this, 3, event);"" size=""3"" maxlength=""3"" />)&nbsp;" & vbcrlf
    response.write "        		<input type=""text"" value=""" & mid(sHomePhone,4,3) & """ name=""skip_user_exchange"" onKeyUp=""return autoTab(this, 3, event);"" size=""3"" maxlength=""3"" />&ndash;" & vbcrlf
    response.write "        		<input type=""text"" value=""" & mid(sHomePhone,7,4) & """ name=""skip_user_line"" onKeyUp=""return autoTab(this, 4, event);"" size=""4"" maxlength=""4"" />" & vbcrlf
    response.write "      </td>" & vbcrlf
    response.write "  </tr>" & vbcrlf
 end if
...
'Fax --------------------------------------------------------------------------
 if isDisplay(sMASK,6) then
    ...
    response.write "      </td>" & vbcrlf
    response.write "      <td>" & vbcrlf
    response.write "      			(<input type=""text"" value=""" & left(sFax,3) & """ name=""skip_fax_areacode"" onKeyUp=""return autoTab(this, 3, event);"" size=""3"" maxlength=""3"" />)&nbsp;" & vbcrlf
    response.write "      				<input type=""text"" value=""" & mid(sFax,4,3) & """ name=""skip_fax_exchange"" onKeyUp=""return autoTab(this, 3, event);"" size=""3"" maxlength=""3"" />&ndash;" & vbcrlf
    response.write "      				<input type=""text"" value=""" & right(sFax,4) & """ name=""skip_fax_line"" onKeyUp=""return autoTab(this, 4, event);"" size=""4"" maxlength=""4"" />" & vbcrlf
...
sub subDisplayQuestions( ByVal iFormID, ByVal sMask )
           ...
           lcl_prompt_req_value = replace(lcl_prompt_req_value,"""","&quot;")
       			...
       			case "2"
					...
					if oQuestions("answerlist") <> "" then
             			...
             			for alist = 0 to ubound(arrAnswers)
							...
							response.write "  <tr><td><input type=""radio"" name=""fmquestion" & iQuestionCount & """ value=""" & lcl_answerslist & """ class=""formradio"" />" & arrAnswers(alist) & "</td></tr>" & vbcrlf
             			next
                    end if
                ...
                'Format the value
                 ...
                 if lcl_answerslist <> "" then
                    lcl_answerslist = replace(lcl_answerslist,"""","&quot;")
       					   		...
       					   		response.write "  <tr><td><input type=""checkbox"" name=""fmquestion" & iQuestionCount & """ id=""fmquestion" & iQuestionCount & "_" & i & """  value=""" & lcl_answerslist & """ class=""formcheckbox"" onclick=""validateCheckbox('fmquestion" & iQuestionCount & "')"" />" & arrAnswers(alist) & "</td></tr>" & vbcrlf
       						next
       			   i = i + 1
            ...
            'Build TEXT Question ----------------------------------------------
					...
					if iFormID = "17890" and oQuestions("prompt") = "Start Date" then
						response.write "<tr><td><input name=""fmquestion" & iQuestionCount & """ value=""6/4/2015"" type=""text"" style=""width:300px;"" maxlength=""" & lcl_text_field_length & """ /></td></tr>"& vbcrlf
					elseif iFormID = "17890" and oQuestions("prompt") = "Address" then
						response.write "<tr><td><input name=""fmquestion" & iQuestionCount & """ value=""1234 Main"" type=""text"" style=""width:300px;"" maxlength=""" & lcl_text_field_length & """ /></td></tr>"& vbcrlf
					else
	...
	'SEND EMAIL
	'-------------------------------------------------------------------------
	'- This IF statement sets up the SEND REQUEST button and the validation for the screen.
	...
	if isDisplay(sMASK,4) then
		...
		response.write "Check here to have email confirmation of this request sent." & vbcrlf
		response.write "</div>" & vbcrlf
	end if
	...
	response.write "<div style=""width:450px;text-align:right;"">" & vbcrlf
	'response.write "<input name=""btnSubmit"" id=""btnSubmit"" class=""actionbtn"" type=""button"" onclick=""" & lcl_onclick & """ value=""SEND REQUEST"" />" & vbcrlf
 response.write "<input type=""button"" name=""btnSubmit"" id=""btnSubmit"" onclick=""" & lcl_onclick & """ value=""SEND REQUEST"" />" & vbcrlf
	response.write "</div>" & vbcrlf
 '--------------------------------------------------------------------------
