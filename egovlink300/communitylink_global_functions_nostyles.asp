...
'------------------------------------------------------------------------------
sub displaySideMenubar(p_orgid, iSideMenuOptionBGColor, iSideMenuOptionBGColorHover, iSideMenuOptionAlignment, p_userid, p_isEgovHomePage)
...
sub displaySideMenubarOption(pID, p_alignment, p_url, p_label)
  ...
  response.write "<div id=""sideMenuBar" & pID & """ align=""" & p_alignment & """" & lcl_onmouseover & lcl_onmouseout & lcl_onclick & ">" & vbcrlf
  response.write "<a href=""" & p_URL & """ id=""sideMenuBarOption" & pID & """>" & p_label & "</a>" & vbcrlf
    ...
    '2. the option (featuer) DOES equal "communitylink" AND it has NOT been set to be the "e-gov home" page.
     if UCASE(oFooter("feature")) <> "COMMUNITYLINK" OR (UCASE(oFooter("feature")) = "COMMUNITYLINK" and NOT lcl_isEgovHomePage) then
        ...
        if ucase(left(oFooter("publicURL"),4)) = "HTTP" Then 
     			...
     			response.write "<a href=""" & sNav & """ target=""_top"">" & oFooter("featurename") & "</a>" & vbcrlf
...
function createCommunityLink(p_orgid, p_userid)
  ...
  if iOrgID <> "" AND p_userid <> "" then
    'First check to see if the org has a community link record.
     ...
     sSQL = sSQL & p_orgid & ", "
     sSQL = sSQL & "lastmodifiedbyid = "    & p_userid                            & ", "
...
sub getCommunityLinkInfo(ByVal iCommunityLinkID, ByVal p_orgid, ByRef lcl_isEgovHomePage, ByRef lcl_website_size, _
                         ...
                         ByRef lcl_pageheader_alignment, ByRef lcl_pageheader_fontcolor, ByRef lcl_pageheader_fonttype, _
                         ByRef lcl_pageheader_bgcolor, ByRef lcl_showRSS, ByRef lcl_url_twitter, ByRef lcl_url_facebook, _
...
sub buildTopBarLink(iLabel, iURL)
 	...
 	response.write "<a href=""" & iURL & """>" & iLabel & "</a>" & vbcrlf
...
sub setupColorSelection(p_fieldid, p_value, p_numLines)
  ...
  response.write "<div id=""" & p_fieldid & "_previewcolor"" bgcolor=""" & p_value & """>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div>" & vbcrlf
...
sub getSocialSiteIcons(p_orientation, p_showRSS, p_twitter, p_facebook, p_myspace, p_blogger)
 ...
 'V = vertical
  ...
  response.write "<table border=""0"" cellspacing=""0"" cellpadding=""4"">" & vbcrlf
 ...
 'Twitter
  if p_twitter <> "" then
     ...
     response.write "          <img src=""" & sEgovWebsiteURL & "/images/communitylink/socialsites/icon_twitter.png"" border=""0"" alt=""Follow us on Twitter"" />" & vbcrlf
 ...
 'Facebook
  if p_facebook <> "" then
     ...
     response.write "          <img src=""" & sEgovWebsiteURL & "/images/communitylink/socialsites/icon_facebook.png"" border=""0"" alt=""Follow us on Facebook"" />" & vbcrlf
     response.write "          </a><br />" & vbcrlf
 ...
 'MySpace
  if p_myspace <> "" then
     ...
     response.write "          <img src=""" & sEgovWebsiteURL & "/images/communitylink/socialsites/icon_myspace.png"" border=""0"" alt=""Follow us on MySpace"" />" & vbcrlf
     response.write "          </a><br />" & vbcrlf
 ...
 'Blogger
  if p_blogger <> "" then
     ...
     response.write "          <img src=""" & sEgovWebsiteURL & "/images/communitylink/socialsites/icon_blogger.png"" border=""0"" alt=""Follow us on Blogger"" />" & vbcrlf
     response.write "          </a><br />" & vbcrlf
 ...
 'Show RSS
  if p_showRSS then
     ...
     response.write "          <img src=""" & sEgovWebsiteURL & "/images/communitylink/socialsites/icon_rss.png"" border=""0"" alt=""Subscribe to our RSS Feeds"" />" & vbcrlf
     response.write "          </a><br />" & vbcrlf
...
sub displayViewRowLink(p_label, p_fieldID, p_URL, p_onMouseOver, p_onMouseOut, p_openNewWin)
  ...
  response.write "<span name=""" & p_fieldID & """ id=""" & p_fieldID & """" & lcl_onMouseOver & lcl_onMouseOut & ">" & vbcrlf
...
'------------------------------------------------------------------------------
sub displayPortalSections(p_portalLayoutType, p_column_num, p_orgid, p_orgRegistration, p_userid, p_wrap_td_tags, _
                          p_column_width, p_showRSS)
 ...
 'Retrieve all of the features for the column specified
  ...
  if not oPortalColumns.eof then
     ...
     if lcl_wrapTDTags = "Y" then
        ...
        response.write "<td height=""100%"">" & vbcrlf
     end if
       ...
       'Build the Section Text styles
        ...
        response.write "            <td align=""right"">" & vbcrlf
                                     if lcl_showRSS = "Y" then
                                        checkForRSSFeed p_orgid, oPortalColumns("featureid"),"", sEgovWebsiteURL
                                     end if
        response.write "            </td>" & vbcrlf
        response.write "        </tr>" & vbcrlf
        response.write "      </table>" & vbcrlf
        response.write "      <div align=""left"">" & vbcrlf
                                getPortalInfo lcl_portalLayoutType, p_orgid, trim(oPortalColumns("featureid")), p_orgRegistration, p_userid, _
    ...
    'It is "CL_portaltype" on egov_organization_features.
     if iPortalType <> "" then
        select case UCASE(iPortalType)
          ...
          case "DOCUMENTS"
             displayNewDocuments p_orgid, p_featureid, p_userid, iNumListItemsShown, p_sectionheader_bgcolor, p_sectiontext_bgcolor, _
                                 p_sectiontext_fontcolor, p_sectionlinks_alignment, p_sectionlinks_fonttype, _
                                 p_sectionlinks_fontcolor, p_sectionlinks_fontcolorhover
...
'------------------------------------------------------------------------------
sub displayBlogInfo(p_orgid, p_featureid, p_orgRegistration, p_userid, p_numListItems, p_sectionlinks_alignment, _
                    p_sectionlinks_fonttype, p_sectionlinks_fontcolor, p_sectionlinks_fontcolorhover)
 ...
 'Set up the View All and Post Comments urls
  ...
  sSQL = "SELECT TOP " & iNumListItems & " mb.blogid, mb.userid, mb.title, SUBSTRING(mb.article,1,500) as article, mb.createdbyid, mb.createdbydate, "
  ...
  if not oBlogInfo.eof then
     do while not oBlogInfo.eof
        ...
        if oBlogInfo("imagefilename") <> "" then
           ...
           if session("deviceViewMode") <> "M" then
              lcl_display_img = lcl_display_img & "<table border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf
       ...
       'Format the Article
        ...
        response.write "             <strong>" & oBlogInfo("title") & "</strong><br />" & vbcrlf
        response.write "             <i>by: " & oBlogInfo("createdbyname") & " on " & FormatDateTime(oBlogInfo("createdbydate"),vbshortdate) & "</i>" & vbcrlf
        response.write "          </p>" & vbcrlf
        response.write lcl_article
        response.write "      </td>" & vbcrlf
        response.write "  </tr>" & vbcrlf
        response.write "  <tr>" & vbcrlf
        response.write "      <td align=""" & iSectionLinks_Alignment & """>" & vbcrlf
    ...
    'View All
     displayViewRowLink "View All", "viewAll_blog_0", lcl_viewAll_url, _
...
sub displayUpcomingEvents(p_orgid, p_featureid, p_numListItems, p_sectionheader_bgcolor, p_sectiontext_bgcolor, _
 ...
 'Set up this portal section's specific styles
  lcl_section_spacing         = "padding:5px 5px 5px 10px;"
  lcl_styles_container        = ""
  lcl_styles_container_anchor = ""
  lcl_styles_eventdate        = ""
  lcl_styles_subject          = ""
  lcl_styles_viewlinks        = ""
 ...
 'Determine if the org has a request form associated with the calendar.
  ...
  if lcl_blnCalRequest then
     ...
     displayViewRowLink "Request an Event", "requestEvent", lcl_addEventURL, _
                        "changeElementStyles('requestEvent'," & lcl_onmouseover_viewlinks & ");", _
                        "changeElementStyles('requestEvent'," & lcl_onmouseout_viewlinks  & ");", _
                        "Y"

     lcl_scripts_viewlinks = lcl_scripts_viewlinks & "changeElementStyles('requestEvent'," & lcl_onmouseout_viewlinks & ");" & vbcrlf
  end if
     ...
     do while not oFAQRumorInfo.eof
       ...
       'Set up the View Links onclick
        ...
        'if iShowMouseOver = "Y" then
           lcl_onmouseover_faqRumor = " onmouseover=""changeElementStyles('" & iFAQType & "_" & oFAQRumorInfo("faqid") & "','','','" & p_sectiontext_fontcolor & "','underline','" & p_sectionheader_bgcolor & "');"""
           lcl_onmouseout_faqRumor  = " onmouseout=""changeElementStyles('"  & iFAQType & "_" & oFAQRumorInfo("faqid") & "','','','','none','" & p_sectiontext_bgcolor & "');"""
        'end if
...
sub displayCurrentNews(p_orgid, p_featureid, p_numListItems, p_sectionheader_bgcolor, p_sectiontext_bgcolor, _
 ...
 'Set up this portal section's specific styles
  ...
  lcl_styles_container        = ""
  lcl_styles_container_anchor = ""
  lcl_styles_articledate      = ""
  lcl_styles_itemtitle        = ""
  lcl_styles_viewlinks        = ""

 'Set up the View All and Suggest a News Item urls
 ...
 'Only allow the user to enter a comment if a actionline request form has been associated to the blog feature.
  if lcl_postcomments_formid > 0 then
     ...
     displayViewRowLink "Suggest a News Item", "postComments_news", lcl_postcomments_url, _
                        "changeElementStyles('postComments_news'," & lcl_onmouseover_viewlinks & ");", _
                        "changeElementStyles('postComments_news'," & lcl_onmouseout_viewlinks  & ");", _
                        "N"

     lcl_scripts_viewlinks = lcl_scripts_viewlinks & "changeElementStyles('postComments_news'," & lcl_onmouseout_viewlinks & ");" & vbcrlf
  end if
...
'------------------------------------------------------------------------------
sub displayNewDocuments(p_orgid, p_featureid, p_userid, iNumListItemsShown, p_sectionheader_bgcolor, p_sectiontext_bgcolor, _
                        p_sectiontext_fontcolor, p_sectionlinks_alignment, p_sectionlinks_fonttype, p_sectionlinks_fontcolor, _
                        p_sectionlinks_fontcolorhover)
 ...
 'Set up the View All url
  'lcl_viewAll_url = sEgovWebsiteURL & "/docs/archive.asp?docsMonth=" & month(now) & "&docsYear=" & year(now)
 ...
 'Get the documents
  ...
  sSQL = "SELECT d.documentid, d.documenturl, d.documenttitle, d.dateadded, d.parentfolderid, "
          ...
          'Display the document if there are no restrictions
           if iShowDoc = 1 then
             ...
             'Set up the View Links onclick
              ...
              lcl_onmouseover_newDoc = " onmouseover=""changeElementStyles('newdoc_" & oNewDocs("documentid") & "','','','" & p_sectiontext_fontcolor & "','underline','" & p_sectionheader_bgcolor & "');"""
              lcl_onmouseout_newDoc  = " onmouseout=""changeElementStyles('newdoc_"  & oNewDocs("documentid") & "','','','','none','" & p_sectiontext_bgcolor & "');"""
             ...
             'We have to "flip" the <DIV> and <A> tags depending on how the user is accessing the site so that the link works properly.
              ...
              'response.write "    <a href=""" & sEgovWebsiteURL & "/docs/menu/home.asp?path=" & oNewDocs("documenturl") & """>" & vbcrlf
              'response.write "    <img src=""" & sEgovWebsiteURL & "/images/communitylink/docfolder.png"" width=""16"" height=""13"" border=""0"" align=""top"" alt=""Click to open folder""/>" & vbcrlf
              'response.write "    </a>" & vbcrlf
              response.write "    <input type=""image"" src=""" & sEgovWebsiteURL & "/images/communitylink/docfolder.png"" value=""test"" alt=""Open Folder"" onclick=""window.open('" & sEgovWebsiteURL & "/docs/menu/home.asp?path=" & oNewDocs("documenturl") & "','_top');"" />" & vbcrlf
              'response.write "    <input type=""image"" src=""" & sEgovWebsiteURL & "/images/communitylink/docfolder.png"" value=""test"" alt=""Open Folder"" onclick=""location.href='" & sEgovWebsiteURL & "/docs/menu/home.asp?path=" & oNewDocs("documenturl") & "';"" />" & vbcrlf
              'response.write "  </li>" & vbcrlf
 ...
 'Display "no documents found" message
  if iLineCnt = 0 then
     ...
     response.write "<div" & lcl_styles_container & "><span>" & lcl_nodisplay_message & "</span><br /><br /></div>" & vbcrlf
...
function checkDocumentAccess(p_orgid, p_userid, p_filepath)
...
'------------------------------------------------------------------------------
function checkDocRestrictionExists(p_orgid, p_filepath)
  lcl_return   = False
  lcl_folderid = 0
...
function checkDocExists(sPath)
  ...
  if sPath <> "" then
     ...
     if fs.FileExists(server.mappath(replace(sPath,"/custom/pub",""))) = true then
        lcl_return = True
     end if
