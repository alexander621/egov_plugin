...
'Check to see if this has already been refunded, if so, bounce them back to receipt
'Response.write GetRegistrantStatus(request.querystring("classlistid"))
'response.end

twfStatus = GetRegistrantStatus(dbsafe(request.querystring("classlistid")))

if twfStatus <> "ACTIVE" AND twfStatus <> "DROPIN" then
	response.redirect "class_receipt.aspx?paymentid=" & request.querystring("paymentid")
end if
...
If bHasRefundDebitAccount and iOrgId <> 60 Then 
...
<!--BEGIN: PAGE TITLE-->
<p>
	<font size="+1"><strong>Process Refund</strong></font><br /><br />
	
	<input type="button" class="button" value="<< Back to Receipt" onclick="javascript:history.go(-1);" />
</p>
<!--END: PAGE TITLE-->
...
<form name="frmDrop" action="drop_registrant.asp" method="post">
	...
	<fieldset style="display:none;"><legend><strong> Payment Details </strong></legend>
	...
	<input type="hidden" name="PaymentLocationId" id="PaymentLocationId" value="3" />
	...
	<p><input class="button" type="button" name="complete" value="Process" onclick="validateForm();" /></p>
...
Sub ShowPurchaseLedgerDetails( ByVal iClassListId, ByVal cRefundFee, ByVal bHasRefundDebitAccount, ByVal iRefundDebitId, ByVal sRefundName )
	...
	If Not oRs.EOF Then 
		...
		Do While Not oRs.EOF
			...
			response.write vbcrlf & "<tr style=""display:none;""><td class=""pricetd"" nowrap=""nowrap"" valign=""top"">"
			...
			response.write "<input type=""hidden"" id=""amount" & oRs("pricetypeid") & """ name=""amount" & oRs("pricetypeid") & """ value=""" & Replace(FormatNumber(CDbl(oRs("amount")),2),",","") & """ size=""10"" maxlength=""9"" onchange=""ValidatePrice(this);"" />"
			...
			oRs.MoveNext
		Loop
		response.write vbcrlf & "<tr><td><strong>Total Paid</strong></td><td class=""signcol"">&nbsp;</td><td align=""right""><span id=""displaytotalprice"">" & Replace(FormatNumber(cTotalRefund,2),",","") & "</span></td></tr>"
		If bHasRefundDebitAccount Then
			' Show the refund fee
			
			response.write vbcrlf & "<tr><td class=""pricetd"" nowrap=""nowrap"" valign=""top"">"
			response.write sRefundName & "</td><td class=""signcol""></td>"
			response.write "<!--td class=""priceentrytd"" valign=""top""--><input type=""hidden"" id=""refundfee"" name=""refundfee"" value=""" & Replace(FormatNumber(CDbl(cRefundFee),2),",","") & """ size=""10"" maxlength=""9"" onchange=""ValidatePrice(this);"" />"
			response.write vbcrlf & "<input type=""hidden"" id=""refundfeeid"" name=""refundfeeid"" value=""" & iRefundDebitId & """ />"
			response.write "</td-->"
			response.write "<td>- " & FormatCurrency(cRefundFee) & "</td>"
			response.write "</tr>"
			cTotalRefund = cTotalRefund - CDbl(cRefundFee)
		End If 
		response.write vbcrlf & "<tr><td><strong>Total Refund</strong></td><td class=""signcol"">&nbsp;</td><td align=""right""><span id=""displaytotalprice"">" & Replace(FormatNumber(cTotalRefund,2),",","") & "</span></td></tr>"
...
Sub ShowRefundChoices( ByVal iHeadUserId )
	...
	If OrgHasFeature( iOrgId, "citizen accounts" ) Then 
		sSql = "SELECT userfname, userlname, userid, ISNULL(accountbalance,0.00) AS accountbalance "
		sSql = sSql & " FROM egov_users WHERE headofhousehold = 1 and isdeleted = 0  and familyid = " & GetFamilyId( iHeadUserId )
