...
'			 All Rights Reserved.
'
' Description:  This module allows an DM Owner to maintain their data in a specific section.
'
' MODIFICATION HISTORY
' 1.0 08/15/2011 David Boyer - Initial Version
'
'------------------------------------------------------------------------------
'
'------------------------------------------------------------------------------
...
'Get Section Info
 getSectionInfo lcl_sectionmode, lcl_sectionid, lcl_sectionname, lcl_sectiontype
...
'Also determine how the user is accessing the screen.
 ...
 if lcl_sectionname <> "" then
    lcl_sectionname = replace(lcl_sectionname,"<em>","")
    lcl_sectionname = replace(lcl_sectionname,"</em>","")
 end if
...
<head>
  ...
  <title>E-Gov Services {<%=lcl_display_featurename%>}</title>
...
<%
  response.write "<form class=""datamgr"" name=""maintain_dmt_section"" id=""maintain_dmt_section"" method=""post"" action=""maintain_dmt_section_action.asp"">" & vbcrlf
  ...
  response.write "  <input type=""hidden"" name=""screen_mode"" id=""screen_mode"" size=""4"" maxlength=""4"" value=""EDIT"" />" & vbcrlf
  response.write "  <input type=""hidden"" name=""dmid"" id=""dmid"" size=""5"" maxlength=""5"" value="""           & lcl_dmid      & """ />" & vbcrlf
  response.write "  <input type=""hidden"" name=""dm_typeid"" id=""dm_typeid"" value="""                            & lcl_dm_typeid & """ />" & vbcrlf
  response.write "  <input type=""hidden"" name=""sectionid"" id=""sectionid"" size=""5"" maxlength=""5"" value=""" & lcl_sectionid & """ />" & vbcrlf
  response.write "  <input type=""hidden"" name=""orgid"" id=""orgid"" size=""4"" maxlength=""10"" value="""        & iorgid        & """ />" & vbcrlf
  ...
  response.write "          <p>" & vbcrlf
                           ...
                           'Determine which section to edit
                            buildSection lcl_dmid, lcl_dm_typeid, lcl_sectionid, _
                                         lcl_orghasfeature_issue_location, lcl_orghasfeature_large_address_list, _
                                         lcl_importAddressFields, lcl_importstreet_number, lcl_importstreet_address
...
sub buildSection(iDMID, iDMTypeID, iSectionID, iOrgHasFeature_issueLocation, _
 ...
 'Get all of the section info
  ...
  getSectionInfo lcl_sectionmode, iSectionID, lcl_sectionname, lcl_sectiontype
 ...
 'Determine how to display the section for editing
  if lcl_sectiontype = "HOURS" then
     ...
     editSection sSQL, iOrgHasFeature_issueLocation, iOrgHasFeature_largeAddressList, iImportAddressFields, _
                       iStreetNumber, iStreetAddress, iDMID
  end if
    ...
    'Check to see if the user wants to import the address fields because the street number/name has been changed.
     if iImportAddressFields = "Y" then
        if iOrgHasFeature_largeAddressList then
           GetAddressInfoLarge oEditDMValues("orgid"), p_street_number, p_street_address, sNumber, sPrefix, sAddress, _
                               sSuffix, sDirection, sCity, sState, sZip, sValidStreet, sLatitude, sLongitude
        else
    		    	GetAddressInfo p_street_address, sNumber, sPrefix, sAddress, sSuffix, sDirection, sCity, sState, sZip, _
                          sValidStreet, sLatitude, sLongitude
        end if
       ...
       'Get the address info from the DM Data itself
        getAddressInfo_byDMID iDMID, sNumber, sPrefix, sAddress, sSuffix, sDirection, _
                              sCity, sState, sZip, sValidStreet, sLatitude, sLongitude
     ...
     do while not oEditDMValues.eof
           ...
           if instr(lcl_fieldtype,"WEBSITE") > 0 OR instr(lcl_fieldtype,"EMAIL") > 0 then
             ...
             'Determine if this field is considered a "multi-line"
              if oEditDMValues("isMultiLine") then
                 ...
                 response.write "<input type=""text"" name=""dm_fieldvalue" & iRowCount & """ id=""dm_fieldvalue" & iRowCount & """ value=""" & lcl_fieldvalue & """ size=""50"" maxlength=""" & lcl_field_maxlength & """ onchange=""clearMsg('dm_fieldvalue" & iRowCount & "');""" & lcl_style_div & " />" & vbcrlf
              end if
           end if
        end if
       ...
       '*** NOT available on public-side as we do NOT want citizen to be able to access client's documents.
        if oEditDMValues("hasAddLinkButton") then
           if instr(lcl_fieldtype,"WEBSITE") < 1 AND instr(lcl_fieldtype,"EMAIL") < 1 then
              response.write "          <input type=""button"" value=""Add a Link"" class=""button"" onclick=""doPicker('maintain_dmt_section.dm_fieldvalue" & iRowCount & "','','','','Y');"" />" & vbcrlf
           end if
        end if
...
'  response.write "["& iFieldValue & "]" & vbcrlf
  response.write "<select class=""smallInput"" name=""" & iStartEnd & "_" & iOptionType & iRowCount & """ id=""" & iStartEnd & "_" & iOptionType & iRowCount & """>" & vbcrlf
