  ...
  <tr valign="top">
      <td>
          ...
          <%
            ...
            '	 else
            if lcl_list_type = "JOB" then
               lcl_feature_name = oActionOrg.GetOrgFeatureName("job_postings")
            elseif lcl_list_type = "BID" then
               lcl_feature_name = oActionOrg.GetOrgFeatureName("bid_postings")
            end if
          %>
        		Welcome to the <%=oActionOrg.GetOrgName()%>, <%=oActionOrg.GetState()%>, <%=lcl_feature_name %>
         	</font>
      </td>
      <td align="right">
                          ...
                          if not oStatus.eof then
                             while not oStatus.eof
                                if lcl_sc_status_id <> "" AND lcl_sc_status_id <> "none" then
                                   if clng(lcl_sc_status_id) = clng(oStatus("status_id")) then
   ...
   '---------------------------------------------------------------------------
      ...
      'Evaluate the search criteria, if it exists
       if lcl_sc_category_id <> "" then
          if LEFT(lcl_sc_category_id,2) = "SC" then
             sSQL = sSQL & " AND dl.distributionlistid = " & REPLACE(lcl_sc_category_id,"SC","")
          else
             sSQL = sSQL & " AND dl.distributionlistid = " & REPLACE(lcl_sc_category_id,"C","")
          end if
       end if
...
function jobbid_per_dlist_count(p_dlistid, p_listtype, p_sc_category_id, p_sc_status_id, p_sc_show_expired)
 ...
 'Evaluate the search criteria if any has been entered.
  if p_sc_category_id <> "" then
     if LEFT(p_sc_category_id,2) = "SC" then
        sSQLcnt = sSQLcnt & " AND djb.distributionlistid = " & REPLACE(p_sc_category_id,"SC","")
     else
        sSQLcnt = sSQLcnt & " AND djb.distributionlistid = " & REPLACE(p_sc_category_id,"C","")
     end if
  end if
 ...
 '-----------------------------------------------------------------------------
  if UCASE(p_sc_show_expired) = "Y" then
    ...
    'Evaluate the search criteria if any has been entered.
     if p_sc_category_id <> "" then
        if LEFT(p_sc_category_id,2) = "SC" then
           sSQLcnt2 = sSQLcnt2 & " AND djb.distributionlistid = " & REPLACE(p_sc_category_id,"SC","")
        else
           sSQLcnt2 = sSQLcnt2 & " AND djb.distributionlistid = " & REPLACE(p_sc_category_id,"C","")
        end if
     end if
