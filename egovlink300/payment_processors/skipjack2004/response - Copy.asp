...
iPaymentControlNumber = CreatePaymentsControlRow( "Skipjack Payment Received", "'Public'", "'Payments'" )
...
Sub fn_AddPaymentInformation( ByVal iPaymentId )
	...
	' not sure how a payment record would already exist. I do not think this is possible.
	...
	Else
		...
		AddToPaymentsLog iPaymentControlNumber, "new paymentid: " & iPaymentId, "'Public'", "'Payments'"
	End If 
...
'	oUser.AddNew
'	oUser("userfname") = dbsafe(sForm_customername)
'	oUser.Update
...
'------------------------------------------------------------------------------------------------------------
' void SendEmail( iPaymentId )
'------------------------------------------------------------------------------------------------------------
Sub SendEmail( ByVal iPaymentId )
	...
	' CREATE MAIL OBJECT
	Set objMail2 = Server.CreateObject("ecMail.SMTP")
	objMail2.Host       = Application("SMTP_Server")
	'If iOrgID = 18 Then 
		objMail2.From   = "noreply@eclink.com"
	'Else 
	'	objMail2.From   = adminEmailAddr 'adminEmailAddr
	'End If 
	objMail2.FromName   =  sOrgName & " E-GOV WEBSITE"
	objMail2.sendto     = adminEmailAddr
	objMail2.Subject    = sOrgName & " E-GOV ACTION ITEM SUBMISSION"
	objMail2.BodyFormat = 1
	objMail2.Body       = sMsg2 
	objMail2.Priority   = 1 'PRIORITY_HIGH
    ErrorCode = objMail2.Send
