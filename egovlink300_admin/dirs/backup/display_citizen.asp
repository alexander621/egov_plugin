...
<head>
	...
	<!--
		...
		function reloadpage( )
		{
			...
			//alert(iSelected);
			location.href='display_citizen.asp?v=' + iSelected + '&searchvalue=' + sSearch ;
		}
...
<%
'------------------------------------------------------------------------------------------------------------------------------------------
' FUNCTIONS AND SUBROUTINES - Formerly in display_citizen.html
'------------------------------------------------------------------------------------------------------------------------------------------
...
'--------------------------------------------------------------------------------------------------
' integer entry( groupid )
'--------------------------------------------------------------------------------------------------
'Function entry( ByVal groupid )
'	Dim sSql, oRs, AllUsers
'
'	' -100 means total records
'	' if a groupid>0 it will show individual group
'	If groupid = -100 Then 
'		sSql = "SELECT COUNT(ug.userid) AS totaluser FROM usersgroups ug INNER JOIN groups g ON ug.userid = u.userid AND u.orgid = " & Session("OrgID")
'	Else 
'		sSql = "SELECT COUNT(ug.userid) AS totaluser FROM usersgroups ug INNER JOIN groups g ON ug.userid = u.userid AND u.orgid = " & Session("OrgID") & " WHERE ug.groupid = " & groupid
'	End If 
'
'	Set oRs = Server.CreateObject("ADODB.Recordset")
...
Sub navagatorbar( ByVal record, ByVal bHeader, ByVal iCurrentPage )
	...
	If iCurrentPage > 1 Then 
		response.write "<a href='" & thisname & "?" & AdditonURL & "currentpage=" & (iCurrentPage-1) & "&v=" & sShowCitizens & "&searchvalue=" & request("searchvalue") & "&includeemails=" & request("includeemails") & "'>"
	...
	If iCurrentPage < totalpages Then 
		response.write "&nbsp;&nbsp;<a href='" & thisname & "?" & AdditonURL & "currentpage=" & (iCurrentPage+1) & "&v=" & sShowCitizens & "&searchvalue=" & request("searchvalue") & "&includeemails=" & request("includeemails") & "'>" & langNext & " " & pagesize
	Else 
		response.write "&nbsp;&nbsp;<!a href='" & thisname & "?" & AdditonURL & "currentpage=" & (iCurrentPage+1) & "&v=" & sShowCitizens & "&searchvalue=" & request("searchvalue") & "&includeemails=" & request("includeemails") & "'><font color=#999999>" & langNext & " " & pagesize & "</font>" 
	End If 
	...
	Else 
		If record > 0  Then 
			response.write "&nbsp;&nbsp;<img src=""../images/small_delete.gif"" align=""absmiddle"">&nbsp;<a href=""javascript:document.DeleteMember.submit();"" onClick=""javascript: return confirm('" & langWanttoDeleteMember & "');"">" & langDelete & "</a>"
		End If 
