...
'
'--------------------------------------------------------------------------------------------------
Dim sSql, sLastName, oRs, sResults, sFirstInitial

sLastName = DBsafe(request("userlname"))
sFirstInitial = DBSafe(Left(Trim(request("userfname")),1))
sResults = ""

sSql = "SELECT userfname, userlname, useraddress FROM egov_users WHERE LOWER(userlname) = LOWER('" & sLastName
sSql = sSql & "') AND userfname LIKE '" & sFirstInitial & "%' AND isdeleted = 0 AND headofhousehold = 1 AND orgid = " & session("orgid")
sSql = sSql & " ORDER BY userlname, userfname, useraddress"
