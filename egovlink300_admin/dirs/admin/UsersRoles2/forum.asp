...
response.write "<table border=0 cellpadding=5 class='tablelist' cellspacing=1 align=center width=700 bgcolor='" & cForumBgColor & "'>"
response.write "<tr>"
...
set cDB = new DBClass
	cDB.Open sConnForum
strSQL = "select distinct rolename from UsersGroupsRoles UGP "& _
		" where UGP.userid="&RA(IDCol,i)&" order by rolename"
set rs=cDB.GetRS(strSQL)
while not rs.eof
if strUserGroupRoles="" then strSeparation="" else strSeparation="<br>" 
strUserGroupRoles=rs("rolename") &strSeparation &strUserGroupRoles
'response.write "<br>strSQL="&strSQL2&" role="&rs("rolename")&" strRoles="&strUserGroupRoles
rs.movenext
wend
destroy cDB
response.write "<td align=left><!A HREF=javascript:openWin2('DisplayUserGroupRoles.asp?userid="&RA(IDCol,i)&"','_blank')>&nbsp;"&strUserGroupRoles&"</A></td>"
'---------------------------------------------
  response.write " </tr>"
next     
...
"resizable,width=400,height=350");
}
</script>
