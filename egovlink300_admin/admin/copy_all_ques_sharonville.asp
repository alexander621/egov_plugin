...
iNewOrg = "27"		' The New Orgid						  -- Change this 
iAdminId = "785"	' The id of the initial admin person  -- Change this
iDeptId = "327"	' The City Employees Department id	  -- Change this

' GET ALL FORMS FOR ADMINISTRATIVE ORGINIZATION (0) 
'					except the contact E-GOV form - Steve Loar 4/10/2006
sSQL = "SELECT * FROM egov_action_request_forms WHERE orgid = 27" 
Set oAllForms = Server.CreateObject("ADODB.Recordset")
oAllForms.Open sSQL, "Provider=SQLOLEDB; Data Source=10.1.8.30; User ID=egovsa; Password=egov_4303; Initial Catalog=egovlink300_sharonville;", 3, 2
	
If NOT oAllForms.EOF Then
	Do While NOT oAllForms.EOF 
			Call subCopyForm(oAllForms("action_form_id"),iNewOrg, iAdminId, iDeptId)
			oAllForms.MoveNext
	Loop 
End If
response.write Now() & "<br />"
...
Sub subCopyForm( iFormID,iNewOrg, iAdminId, iDeptId )
	...
	' COPY FORM QUESTIONS
	Call SubCopyFormQuestions( iFormID, iNewOrg, iNewFormID )

	' GET CATEGORY ID 
	iNewCategoryID = GetCategoryID( iFormID )

	'CODE TO ASSIGN FORM TO CATEGORY
	Call subAssignFormtoCategory( iNewFormID, iNewCategoryID, iNewOrg )
...
Function fnCopySQLDataRow(sPrimaryKey,iPrimaryKeyID,sTableName, iNewOrg, iAdminId, iDeptId)
	...
	oSchema.Open sSQL, "Provider=SQLOLEDB; Data Source=10.1.8.30; User ID=egovsa; Password=egov_4303; Initial Catalog=egovlink300_sharonville;" , 3, 1
...
Sub SubCopyFormQuestions(iFormID,iOrgID,iNewFormID)
	...
	oQuestions.Open sSQL, "Provider=SQLOLEDB; Data Source=10.1.8.30; User ID=egovsa; Password=egov_4303; Initial Catalog=egovlink300_sharonville;" , 3, 2
	...
	If NOT oSchema.EOF Then
		...
		Do while NOT  oSchema.EOF 
			...
			If NOT oSchema.EOF Then
				...
				sInsertStatement = "INSERT INTO " & sTableName & " (" & sFieldList & ") VALUES (" & sValueList & ")"
				'DEBUG DATA:
				response.write sInsertStatement & "<BR>"
