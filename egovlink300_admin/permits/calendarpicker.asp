...
.pickerparent {text-align:center;}
  ...
  <!--
	...
	{
		//ClassForm
      parent.document.<%=request("updateform")%>.<%=request("updatefield")%>.value = d;
...
<body topmargin="0" leftmargin="0" marginwidth="0" marginheight="0">
  <form name="frmDate" action="calendarpicker.asp" method="post" style="margin:0;">
  ...
  <table border="0" cellpadding="3" cellspacing="0" bgcolor="#ffffff" class="cal" width="100%" height="100%">
    ...
    Do While iCurrent <= iDIM
      ...
      Response.Write vbTab & vbTab & "<td style=""cursor: hand"" class=""pickerparent"" onclick=""javascript:doDateSelect('" & Month(dDate) & "/" & iCurrent & "/" & Year(dDate) & "');"""
	  If DateDiff("d", Date(), Month(dDate) & "/" & iCurrent & "/" & Year(dDate)) = 0 Then
		response.write " id=""today"" title=""today"""
	  End If 
	  response.write "><span class=""pickerday""><font size=2>" & iCurrent & "</font></span></td>" & vbCrLf
