...
' 1.0   01/19/2009	Steve Loar - Initial Version
'
'--------------------------------------------------------------------------------------------------
'
'--------------------------------------------------------------------------------------------------
Dim iPermitId, sPermitLocation, sPDFPath

iPermitId = CLng(request("permitid"))

GetPermitDocumentInfo iPermitid, sPDFPath
...
Sub ShowForm( ByVal sPDFPath, ByVal iPermitId )
	...
	Dim sAmount1, sPaymentDate2, sMethod2, sAmount2, sPermitJobAddress, sStatus, sStatusDate, sApplicantAddressLabel
	Dim sAttachmentDate, sAttachmentName, sAttachmentDescription, x, sInspectionsDue, sInspectorList
	Dim sReviewList, sReviewStatus, sReviewDate, sReviewer, sPermitTypeDescription, sReviewNotes
...
'--------------------------------------------------------------------------------------------------
' Sub GetPermitDocumentInfo( iPermitid, sPDFPath )
'--------------------------------------------------------------------------------------------------
Sub GetPermitDocumentInfo( ByVal iPermitid, ByRef sPDFPath )
	Dim sSql, oRs 

	sSql = "SELECT document, path FROM egov_permitdocuments D, egov_permitpermittypes P WHERE P.permitid = " & iPermitid
	sSql = sSql & " AND P.documentid = D.documentid"

	Set oRs = Server.CreateObject("ADODB.Recordset")
	oRs.Open sSql, Application("DSN"), 3, 1

	If Not oRs.EOF Then 
		sPDFPath = oRs("path") & oRs("document")
	Else
		sPDFPath = ""
	End If
