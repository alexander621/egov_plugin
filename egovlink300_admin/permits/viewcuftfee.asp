...
'
'--------------------------------------------------------------------------------------------------
' FILENAME: viewcuftfee.asp
' AUTHOR: Steve Loar
' CREATED: 11/06/2012
' COPYRIGHT: Copyright 2012 eclink, inc.
'			 All Rights Reserved.
'
' Description:	Views the calculation for a cubic footage (volume) fee of a permit
'
' MODIFICATION HISTORY
' 1.0   11/06/2012	Steve Loar - INITIAL VERSION
'
'--------------------------------------------------------------------------------------------------
'
'--------------------------------------------------------------------------------------------------
Dim iPermitFeeId, sSql, oRs, sFeeAmount, dblCuFootage, sFeeFormula, iPermitId, sMinFeeAmount, sAppliedFee
Dim sFeeName, sCuFootageType

iPermitFeeId = CLng(request("permitfeeid"))

iPermitId = GetPermitIdByPermitFeeId( iPermitFeeId ) ' Get the permit id

' This is just for volume now. That value is stored in the othersqft field
'sCuFootageType = GetSqFootageType( iPermitFeeId ) ' Get which sq footage to use

dblCuFootage = GetPermitSqFootage( iPermitId, "othersqft" ) ' Pull the correct sq footage

sMinFeeAmount = GetMiniumFeeAmount( iPermitFeeId ) ' Pull out for display

sFeeFormula = GetFeeFormula( iPermitFeeId, dblCuFootage ) ' The formula to display
...
Function GetFeeFormula( iPermitFeeId, dblSqFootage )
	...
	If Not oRs.EOF Then
		...
		If CLng(oRs("unitqty")) > CLng(0) Then 
			sFormula = "Ceiling( " & sFormula & " / " & oRs("unitqty") & " )"
