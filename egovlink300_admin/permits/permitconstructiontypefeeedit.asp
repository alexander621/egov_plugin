...
'
'--------------------------------------------------------------------------------------------------
' FILENAME: permitconstructiontypefeeedit.asp
' AUTHOR: Steve Loar
' CREATED: 04/25/2008
' COPYRIGHT: Copyright 2008 eclink, inc.
'			 All Rights Reserved.
'
' Description:  This allows the creation and edit of construction type fees for permits.
'
...
<head>
	...
	<!--
		...
		function CopyFee( iPermitFeeTypeId )
		{
			...
			{
				location.href="permitfeetypecopy.asp?permitfeetypeid=" + iPermitFeeTypeId + "&redirectpage=permitconstructiontypefeeedit";
			}
		}

		function Another()
		{
			location.href="permitconstructiontypefeeedit.asp?permitfeetypeid=0";
		}
		...
		function EditConstructionTypeRate()
		{
			showModal('constructiontypelist.asp', 'Construction Type Rates', 85, 95);
		}
...
<body>
	...
	<div id="content">
		...
		<div class="gutters">
			...
			<!--BEGIN: PAGE TITLE-->
			<p>
				<font size="+1"><strong><%=sTitle%> Construction Type Fee</strong></font><br /><br />
...
<%		End If %>
		...
		<form name="frmFeeTypes" action="permitconstructiontypefeeupdate.asp" method="post">
		<input type="hidden" name="permitfeetypeid" value="<%=iPermitFeeTypeid%>" />
		<input type="hidden" name="isconstructiontypefee" value="1" />
		<input type="hidden" name="isbuildingpermitfee" value="1" />
		<input type="hidden" name="baseamount" value="0.00" />
		<input type="hidden" name="unitqty" value="0" />
		<input type="hidden" name="unitamount" value="0.00" />
...
<%		End If %>
		...
		<div class="shadow">
			<table cellpadding="3" cellspacing="0" border="0" class="tableadmin" id="feesteptable">
				<tr><th>Calculation Method</th><th>Included<br />Multipliers</th><th>Minimum<br />Amount</th>
<%					If bShowUpFrontAmount Then	%>
						<th>Up Front<br />Amount</th>
<%					End If						%>
				</tr>
				<tr>
					<td align="center"><select name="permitfeemethodid"><% ShowFeeMethods iFeeMethodId %></select><input type="button" value="Edit Construction Type Rates" onClick="EditConstructionTypeRate();" class="button ui-button ui-widget ui-corner-all" /></td>
...
Sub GetPermitFeeType( iPermitFeeTypeid )
	...
	sSql = sSql & " ISNULL(baseamount, 0.00) AS baseamount, ISNULL(unitqty, 0) AS unitqty, ISNULL(unitamount, 0.00) AS unitamount, "
	sSql = sSql & " ISNULL(accountid,0) AS accountid, "
	sSql = sSql & " permitfeecategorytypeid, permitfeemethodid, ISNULL(upfrontamount,0.00) AS upfrontamount, "
	...
	If Not oRs.EOF Then 
		...
		sUnitAmount = FormatNumber(oRs("unitamount"),2,,,0)
...
Sub ShowFeeMethods( iFeeMethodId )
	...
	sSql = sSql & " AND isconstructiontypemethod = 1 "
...
Sub ShowUnusedMultipliers( iPermitFeeTypeid )
	...
	response.write vbcrlf & "<select id=""newfeemultipliertypeid"" name=""newfeemultipliertypeid"">"
	Do While Not oRs.EOF
