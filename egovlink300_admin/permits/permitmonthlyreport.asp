...
Dim sDisplayDateRange, sMonthName, iIncludeJobValue, bUsePermitJobValue
...
<%										If request("isview") <> "" Then		%>
											<input type="button" class="button ui-button ui-widget ui-corner-all" value="Download to Excel" onClick="location.href='permitmonthlyreportexport.asp?selmonth=<%=iStartMonth%>&selyear=<%=iStartYear%>&includejobvalue=<%=iIncludeJobValue%>'" />
<%										End If		%>
									</td>
								</tr>
							</table>
...
<%			' if they choose to view the report, then display the inspections
			If request("isview") <> "" Then	
				DisplayIssuedPermits sStartDate, sEndDate, iInclude, sMonthName, bUsePermitJobValue 
			Else 
...
'--------------------------------------------------------------------------------------------------
' void DisplayIssuedPermits sStartDate, sEndDate, iInclude, sMonthName 
'--------------------------------------------------------------------------------------------------
Sub DisplayIssuedPermits( ByVal sStartDate, ByVal sEndDate, ByVal iInclude, ByVal sMonthName, ByVal bUsePermitJobValue )
	...
	sSql = "SELECT P.permitid, P.issueddate, P.isvoided, P.jobvalue, ISNULL(P.residentialunits,0) AS residentialunits, "
	...
	sSql = sSql & sIsVoided
			'If not bUsePermitJobValue Then
	sSql = sSql & " UNION ALL "
	sSql = sSql & " SELECT DISTINCT P.permitid, P.issueddate, P.isvoided, P.jobvalue, 0 AS residentialunits, "
	...
	If Not oRs.EOF Then 
		...
		response.write vbcrlf & "<tr><th>Permit #</th><th>Issued<br />Date</th><th>Description<br />of Work</th><th>Address</th><th>New Residential<br />Units</th><th>Est. Cost</th>"
		...
		Do While Not oRs.EOF
			...
			Else 
				response.write GetCostEstimate( oRs("permitid"), oRs("isold"), dCostEstimateSubTotal, sEndDate, sStartDate ) & "<!--" & oRs("permitid") & " - " & oRs("isold") & " - " & dCostEstimateSubTotal & " - " & sEndDate & " - " & sStartDate & "-->"
			End If 
			response.write "</td>"
