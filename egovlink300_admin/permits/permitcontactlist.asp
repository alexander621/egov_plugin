...
'
'--------------------------------------------------------------------------------------------------
' FILENAME: permitcontactlist.asp
' AUTHOR: Steve Loar
' CREATED: 01/23/2008
' COPYRIGHT: Copyright 2007 eclink, inc.
'			 All Rights Reserved.
'
' Description:  List of permit contacts
'
...
<body>
	...
	<div id="content">
		...
		<div class="gutters">
			...
			<!--BEGIN: PAGE TITLE-->
			<p>
				<font size="+1"><strong>Permit Contractors</strong></font><br />
			</p>
			<!--END: PAGE TITLE-->
			<form name="frmPermitSearch" method="post" action="permitcontactlist.asp">
				<div>
					...
					<input type="button" name="new" class="button ui-button ui-widget ui-corner-all" value="New Contractor" onclick="location.href='permitcontacttypeedit.asp?permitcontacttypeid=0&isorganization=0';" />
					<br /><br />
				</div>
			</form>
			...
			<table id="categorytypes" cellpadding="0" cellspacing="0" border="0">
				<tr><th>Contractor</th><th>Contractor Type</th></tr>
				<%	
...
'--------------------------------------------------------------------------------------------------
' void ShowPermitContactTypes sSearch, sWhere 
'--------------------------------------------------------------------------------------------------
Sub ShowPermitContactTypes( ByVal sSearch, ByVal sWhere )
	...
	sSql = sSql & " ISNULL(lastname,'') AS lastname, ISNULL(company,'') + ISNULL(lastname,'') + ISNULL(firstname,'') AS sortname, "
	sSql = sSql & " ISNULL(contractortypeid,0) AS contractortypeid "
	sSql = sSql & " FROM egov_permitcontacttypes WHERE isorganization = 0 AND orgid = " & session("orgid") & sWhere
	If sSearch <> "" Then
	...
	If Not oRs.EOF Then
		Do While Not oRs.EOF
			If iPermitContactTypeid <> CLng(oRs("permitcontacttypeid")) Then 
				...
				response.write "</td>"
				response.write "<td align=""center"" title=""click to edit"" onClick=""location.href='permitcontacttypeedit.asp?permitcontacttypeid=" & oRs("permitcontacttypeid") & "';"">"
				If clng(oRs("contractortypeid")) > clng(0) Then 
					response.write GetContractorType( oRs("contractortypeid") )
				Else
					response.write "&nbsp;"
				End If 
				response.write "</td>"

'				If ContactHasPermits( oRs("permitcontacttypeid") ) Then
'					response.write "<td align=""center"" title=""click to view"" onClick=""location.href='contactpermitlist.asp?permitcontacttypeid=" & oRs("permitcontacttypeid") & "';"">&nbsp;"
'					response.write "view permits"
'				Else
'					response.write "<td title=""click to edit"" onClick=""location.href='permitcontacttypeedit.asp?permitcontacttypeid=" & oRs("permitcontacttypeid") & "';"">&nbsp;"
'				End If 
'				response.write "</td>"
			End If 
			oRs.MoveNext 
	...
	Else
		If sSearch <> "" Then
			response.write vbcrlf & "<tr><td colspan=""2"">&nbsp;No Permit Contacts could be found that match your search criteria.</td></tr>"
		Else 
			response.write vbcrlf & "<tr><td colspan=""2"">&nbsp;No Permit Contacts could be found. Click on the New Contact button to start entering data.</td></tr>"
		End If 
	End If  
...
'--------------------------------------------------------------------------------------------------
' boolean ContactHasPermits( iPermitContactTypeId )
'--------------------------------------------------------------------------------------------------
Function ContactHasPermits( ByVal iPermitContactTypeId )
	Dim sSql, oRs
