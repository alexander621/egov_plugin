...
sub displayTeamRosterAccessories(p_orgid, p_classid, p_type )
  ...
  if not oClassAccessories.eof then
     do while not oClassAccessories.eof
        response.write "  <option value=""" & oClassAccessories("accessoryvalue") & """>" & oClassAccessories("accessoryname") & "</option>"  & vbcrlf

        oClassAccessories.movenext
     loop
  end if

'response.write "  <option value="""">" & sSQL & "</option>" & vbcrlf
