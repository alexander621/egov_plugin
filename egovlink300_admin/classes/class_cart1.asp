...
<head>
	...
	<!--
		...
		function validate()
		{
			if (document.cartForm.amount2.value != "" && document.cartForm.checkno.value == "")
			{
				if ( ! confirm('You have a check payment without a check number. \nDo you wish to continue?'))
				{
					document.cartForm.checkno.focus();
					return;
				}
			}
			if (Number(document.cartForm.amount.value) == Number(document.cartForm.purchasetotal.value))
			{
				//alert('OK');
				document.cartForm.submit();
			}
		...
		function addTotal()
		{
			...
			if (document.cartForm.amount1.value != "")
			{
				...
				var rege1 = /^\d*\.?\d{0,2}$/
				var Ok1 = rege1.exec(document.cartForm.amount1.value);
				if ( Ok1 )
				{
			...
			if (document.cartForm.amount2.value != "")
			{
				...
				var rege2 = /^\d*\.?\d{0,2}$/
				var Ok2 = rege2.exec(document.cartForm.amount2.value);
				if ( Ok2 )
				{
			...
			if (document.cartForm.amount3.value != "")
			{
				...
				var rege3 = /^\d*\.?\d{0,2}$/
				var Ok3 = rege3.exec(document.cartForm.amount3.value);
				if ( Ok3 )
				{
			...
			if (bexists)
			{
				if (document.cartForm.amount4.value != "")
				{
					...
					var rege4 = /^\d*\.?\d{0,2}$/
					var Ok4 = rege4.exec(document.cartForm.amount4.value);
					if ( Ok4 )
					{
			...
			if (bexists)
			{
				if (document.cartForm.amount8.value != "")
				{
					...
					var rege8 = /^\d*\.?\d{0,2}$/
					var Ok8 = rege8.exec(document.cartForm.amount8.value);
					if ( Ok8 )
					{
...
Dim sSql, iRowCount, nRowTotal, nTotal, sRowClassName
...
If Not oCart.eof Then 
	...
	Do While Not oCart.EOF
		...
		Select Case sItemType
			...
			Case "regatta team"
				...
				response.write oCart("quantity")
				response.write "</td>"
				fAmount = FormatCurrency(GetCartItemPrice( oCart("cartid") ))
				nTotal = nTotal + fAmount
				response.write "<td align=""right"">" & fAmount & "</td>"
				response.write "</tr>"
			...
			Case "shipping and handling fees"
				...
				' Total amount
				fAmount = FormatCurrency( oCart("amount") )
				nTotal = nTotal + fAmount
				response.write "<td align=""right"">" & fAmount & "</td>"
				response.write "</tr>"
	...
	response.write "<input type=""hidden"" name=""purchasetotal"" value=""" & nTotal & """ />Cart Total: " & FormatCurrency(nTotal)
	response.write "</td>" 
	response.write "</tr>" 
	...
	response.write "<input type=""hidden"" value=""0.00"" name=""amount"" />"
...
Sub ShowPaymentChoices( ByVal iUserId, sBalanceDue )
	...
	If Not oPayments.EOF Then 
		...
		Do While Not oPayments.EOF
			...
			response.write "</td>"
			response.write "<td>"
			response.write "<input type=""text"" value="""" name=""amount" & oPayments("paymenttypeid") & """ size=""10"" maxlength=""9"" onblur=""addTotal()"" />"

			If oPayments("requirescheckno") Then 
				response.write "&nbsp;<strong>Check #: </strong>"
				response.write "<input type=""text"" value="""" name=""checkno"" size=""8"" maxlength=""8"" />"
			End If 
