...
<%
	...
	Dim sStartDate, sEndDate, sPublishStartDate, sPublishEndDate, sRegistrationStartDate

	iClassId = CLng(request("classid"))

	If iClassId > CLng(0) Then 

		sSQL = "SELECT classname, isnull(classseasonid,0) as classseasonid, ISNULL(classdescription,'') AS classdescription, startdate, "
		sSQL = sSQL & " enddate, ISNULL(publishstartdate,'') as publishstartdate, ISNULL(publishenddate,'') as publishenddate, "
		sSQL = sSQL & " registrationstartdate, registrationenddate, "
		...
		sSQL = sSQL & " ISNULL(alternatedate,'') as alternatedate, ISNULL(searchkeywords,'') AS searchkeywords, externalurl, externallinktext, noenddate, "
		...
		sSQL = sSQL & " ISNULL(minage,0) as minage, ISNULL(maxage,99) as maxage, mingrade, maxgrade, ISNULL(pocid,0) as pocid,  "
		sSQL = sSQL & " ISNULL(membershipid,0) as membershipid, isnull(supervisorid,0) as supervisorid, ISNULL(notes,'') AS notes, allowearlyregistration, "
		sSQL = sSQL & " earlyregistrationdate, ISNULL(earlyregistrationclassseasonid,0) AS earlyregistrationclassseasonid, "
		sSQL = sSQL & " ISNULL(earlyregistrationclassid,0) AS earlyregistrationclassid, ISNULL(minageprecisionid, 0) as minageprecisionid, "
		sSQL = sSQL & " ISNULL(maxageprecisionid, 0) as maxageprecisionid, ISNULL(agecomparedate,'') as agecomparedate, "
		...
		If Not oClass.eof Then
			...
			If IsNull(oClass("registrationenddate")) Then
				sPublishEndDate = ""
			Else
				sPublishEndDate = FormatDateTime(oClass("registrationenddate"),2)
			End If 
...
<%
			   'Receipt Notes ---------------------------------------------------------
				...
				response.write "   <textarea name=""notes"" id=""receiptnotes"" maxlength=""1024"" wrap=""soft"">" & sNotes & "</textarea>" & vbcrlf
				response.write "</p>" & vbcrlf
%>
			</div>
...
'--------------------------------------------------------------------------------------------------
' Sub ShowRegattaSignupTypes( iRegattaSignupTypeId )
'--------------------------------------------------------------------------------------------------
Sub ShowRegattaSignupTypes( iRegattaSignupTypeId )
	Dim sSql, oRs
