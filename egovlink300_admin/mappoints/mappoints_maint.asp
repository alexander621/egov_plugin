...
'Retrieve the mappointid to be maintained.
...
'Set up local variables
 ...
 if lcl_screen_mode = "EDIT" then
   'Retrieve all of the data for the mappoint
    sSQL = "SELECT mp.mappointid, mp.mappoint_typeid, mp.orgid, mp.createdbyid, mp.createdbydate, mp.isActive, "
    ...
    sSQL = sSQL & " mp.city, mp.state, mp.zip, mp.validstreet, mp.mappointcolor "
    ...
    if not oMapPoints.eof then
       ...
       lcl_isActive           = oMapPoints("isActive")
       ...
       lcl_mappointcolor      = oMapPoints("mappointcolor")

      'Determine if the checkbox(es) are checked or not
       if not oMapPoints("isActive") then
...
'Determine the MapPointColor
 if (lcl_mappointcolor = "" OR isnull(lcl_mappointcolor)) AND lcl_screen_mode = "EDIT" then
...
function checkAddressButtons() {
  <%
    if lcl_mappointid <> 0 then

       if lcl_orghasfeature_large_address_list then
          response.write "   document.getElementById(""validateAddress"").disabled=true;" & vbcrlf
       end if

       response.write "document.getElementById(""importAddress"").disabled=true;" & vbcrlf

      'If a non-valid address is entered then disable the "Validate Address" and "Import Address" buttons.
      'Determine which fields are valid depending on the which address list the org has
       ...
       if lcl_orghasfeature_large_address_list then
          response.write "   document.getElementById(""validateAddress"").disabled=false;" & vbcrlf
       end if
...
<table border="0" cellspacing="0" cellpadding="10" width="800" class="start">
  <form name="mappoints_maint" id="mappoints_maint" method="post" action="mappoints_action.asp">
    ...
    <input type="<%=lcl_hidden%>" name="f" id="f" value="<%=lcl_feature%>" />
<%
 'Determine if the Map-Point Type is pre-selected or not.  If yes then show the dropdown list.
 'Otherwise, get the "mappoint_typeid" based off of the feature passed in.
 'NOTE: "lcl_canSelectMPType", if set to True, represent BOTH the display and selecting of the Map-Point Type.
 'This means that in the "EDIT" screen if "lcl_canSelectMPType" is true then the value of the mappoint_typeid is displayed.
 'If the screen mode is "ADD"  and "lcl_canSelectMPType" is true then the dropdown list of MapPoint Types is display.
  if lcl_canSelectMPType then
     response.write "  <tr>" & vbcrlf
     response.write "      <td>" & vbcrlf
     response.write "          <font size=""+1""><strong>Map-Points: " & lcl_screen_mode & "</strong></font><br />" & vbcrlf
     response.write "          <input type=""button"" name=""backButton"" id=""backButton"" value=""Back to List"" class=""button"" onclick=""location.href='mappoints_list.asp" & lcl_url_parameters & "';"" />" & vbcrlf
     response.write "      </td>" & vbcrlf
     ...
     response.write "            <tr>" & vbcrlf
     response.write "                <th align=""left"">Map-Point</th>" & vbcrlf
     response.write "                <th align=""right"" colspan=""2"">" & vbcrlf
     response.write "                    <input type=""checkbox"" name=""isActive"" id=""isActive"" value=""Y""" & lcl_checked_isactive & " /> Active" & vbcrlf
     response.write "                </th>" & vbcrlf
     response.write "            </tr>" & vbcrlf
     response.write "            <tr>" & vbcrlf
     response.write "                <td nowrap=""nowrap"">Map-Point Category:</td>" & vbcrlf
     response.write "                <td colspan=""2"">" & vbcrlf
    ...
    'On an "add" the Map-Point Color will be defaulted to the color of the Map-Point Type
     if lcl_screen_mode = "EDIT" then
        response.write "            <tr>" & vbcrlf
        ...
        response.write "                <td style=""color:#800000"" colspan=""2"">" & lcl_displayCreatedByInfo & "</td>" & vbcrlf
        response.write "            </tr>" & vbcrlf
        response.write "            <tr>" & vbcrlf
        response.write "                <td nowrap=""nowrap"">Last Modified By:</td>" & vbcrlf
        response.write "                <td style=""color:#800000"" colspan=""2"">" & lcl_displayLastModifiedByInfo & "</td>" & vbcrlf
        response.write "            </tr>" & vbcrlf
     else
    ...
    'Display the bottom row of buttons
     ...
     response.write "<input type=""" & lcl_hidden & """ name=""mappoint_typeid"" id=""mappoint_typeid"" value=""" & lcl_mappoint_typeid & """ />" & vbcrlf
     response.write "<input type=""" & lcl_hidden & """ name=""mappointcolor"" id=""mappointcolor"" value=""" & lcl_mappointcolor & """ />" & vbcrlf
...
sub displayMapPointFields(iOrgID, iMapPointID, iMapPointTypeID, iOrgHasFeature_IssueLocation, iOrgHasFeature_LargeAddressList, iImportAddressFields, _
 ...
 'Retrieve all of the Map-Point Type Fields
  sSQL = "SELECT mpf.mp_fieldid, mpf.mappoint_typeid, mpf.fieldname, UPPER(mpf.fieldtype) as fieldtype, "
  sSQL = sSQL & " mpf.hasAddLinkButton, mpf.isMultiLine, mpf.displayInResults, mpf.resultsOrder, mpv.fieldvalue "
  ...
  if not oMPTFields.eof then
    ...
    'END: Import address fields -----------------------------------------------
     ...
     do while not oMPTFields.eof
       ...
       'If this is a "specialty" field then we want to expand across all columns.
       'If not then we want to allow for an extra column in case there are "additional features" like a "add a link" button.
        if oMPTFields("fieldtype") <> "" AND not isnull(oMPTFields("fieldtype")) then
           lcl_colspan = " colspan=""2"""
        else
           lcl_colspan = ""
        end if
        ...
        response.write "      <td nowrap=""nowrap"" valign=""top""" & lcl_colspan & ">" & vbcrlf
       ...
       'Check for "specialty" fields
        if oMPTFields("fieldtype") = "ADDRESS" then
          ...
          '- If "yes" then check to see if the org has the "large address list" feature on.
           if iOrgHasFeature_IssueLocation then
              if iValidStreet = "Y" then
                 if iOrgHasFeature_LargeAddressList then
                    ...
                    DisplayLargeAddressList iOrgID, sNumber, sPrefix, sAddress, sSuffix, sDirection
                 else
           ...
           response.write "    <br /><input type=""" & lcl_hidden & """ name=""mp_fieldvalue" & iRowCount & """ id=""mp_fieldvalue" & iRowCount & """ value=""" & lcl_displayAddress & """ size=""50"" maxlength=""" & lcl_field_maxlength & """ />" & vbcrlf
           response.write "</fieldset>" & vbcrlf
           ...
           response.write "If not, you can search for latitude and longitude values <a href=""javascript:openWin('mappoints_geocode_finder.asp?popup=Y&fname=mappoints_maint&lat=latitude&long=longitude', 800, 700);"">here.</a>" & vbcrlf
        ...
        elseif oMPTFields("fieldtype") = "LATITUDE" OR oMPTFields("fieldtype") = "LONGITUDE" then
          ...
          'If this is an import then the LATITUDE and LONGITUDE fields must be overridden
           ...
           response.write "          <input type=""" & lcl_hidden & """ name=""mp_fieldvalue" & iRowCount & """ id=""mp_fieldvalue" & iRowCount & """ value=""" & lcl_fieldvalue & """ size=""50"" maxlength=""" & lcl_field_maxlength & """ onchange=""clearMsg('mp_fieldvalue" & iRowCount & "');"" />" & vbcrlf
          ...
          'Determine if this field is considered a "multi-line"
           if oMPTFields("isMultiLine") then
              response.write "<textarea name=""mp_fieldvalue" & iRowCount & """ id=""mp_fieldvalue" & iRowCount & """ rows=""5"" cols=""49"" maxlength=""" & lcl_field_maxlength & """>" & lcl_fieldvalue & "</textarea>" & vbcrlf
           else
              response.write "<input type=""text"" name=""mp_fieldvalue" & iRowCount & """ id=""mp_fieldvalue" & iRowCount & """ value=""" & lcl_fieldvalue & """ size=""50"" maxlength=""" & lcl_field_maxlength & """ onchange=""clearMsg('mp_fieldvalue" & iRowCount & "');""" & lcl_style & " />" & vbcrlf
           end if
       ...
       'BEGIN: Additional Features --------------------------------------------
        if oMPTFields("fieldtype") = "" OR isnull(oMPTFields("fieldtype")) then
           response.write "      <td align=""left"" valign=""top"" width=""50%"">" & vbcrlf

          'Determine if we are to display the "Add a Link"
           if oMPTFields("hasAddLinkButton") then
              response.write "<input type=""button"" value=""Add a Link"" class=""button"" onclick=""doPicker('mappoints_maint.mp_fieldvalue" & iRowCount & "','Y','Y','Y','Y');"" />" & vbcrlf
           end if
...
sub displayButtons(iTopBottom, iScreenMode, iReturnParameters)
  ...
  response.write "<input type=""button"" name=""cancelButton"" id=""cancelButton"" value=""Cancel"" class=""button"" onclick=""location.href='mappoints_list.asp" & iReturnParameters & "'"" />" & vbcrlf
