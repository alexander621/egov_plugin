...
'			 All Rights Reserved.
'
' Description:  Edit internal security.
'
	...
	<div id="content">
		<div id="centercontent">
			...
			<table border="0" cellpadding="5" cellspacing="0" id="publicaccesscontainer">
				<tr>
					<td>
						<form name="c1" method="post" action="editsecuritydo.asp">
					...
					<td>
						<form name="r1" method="post" action="editsecuritydo.asp">
...
Sub ShowExistingMembers( ByVal sPath )
	...
	sSql = sSql & "FROM DocumentFolders [df] INNER JOIN FeatureAccess [fa] ON fa.AccessID = df.AccessID "
	sSql = sSql & "INNER JOIN Groups [g] ON g.GroupID = fa.GroupID "
	sSql = sSql & "WHERE fa.OrgID = " & Session("OrgID") & " AND df.FolderPath = '" & sPath & "' AND (g.OrgID = " & session("orgid") & ") "
...
Function HasExistingMembers( ByVal iFolderId )
	...
	sSql = sSql & "FROM DocumentFolders [df] INNER JOIN FeatureAccess [fa] ON fa.AccessID = df.AccessID "
	sSql = sSql & "INNER JOIN Groups [g] ON g.GroupID = fa.GroupID "
	sSql = sSql & "WHERE fa.OrgID = " & Session("OrgID") & " AND df.Folderid = " & iFolderId & " AND (g.OrgID = " & session("orgid") & ") "
...
Sub GetAvailableMembers( ByVal sPath )
	...
	sSql = "SELECT g.GroupID, g.GroupName "
	sSql = sSql & "FROM Groups [g] WHERE OrgID = " & Session("OrgID") & " AND GroupID NOT IN "
	sSql = sSql & "(SELECT g2.GroupID FROM DocumentFolders [df] INNER JOIN FeatureAccess [fa] ON "
	sSql = sSql & "fa.AccessID = df.AccessID INNER JOIN Groups [g2] ON g2.GroupID = fa.GroupID "
