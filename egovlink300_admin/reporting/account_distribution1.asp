...
<head>
	...
	<!--
		...
		function viewReport()
		{
			document.frmPFilter.action = 'account_distribution1.asp';
			document.frmPFilter.submit();
		}

		function exportReport()
		{
			//location.href='account_distribution_export.asp?fromDate=<%=fromDate%>&toDate=<%=toDate%>&locationid=<%=iLocationId%>&adminuserid=<%=iAdminUserId%>&paymentlocationid=<%=iPaymentLocationId%>&reporttype=<%=iReportType%>&journalentrytypeid=<%=iJournalEntryTypeId%>&accountid=<%=iAccountNo%>'
...
<form action="account_distribution1.asp" method="post" name="frmPFilter">
