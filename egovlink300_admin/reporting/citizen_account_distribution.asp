...
'
'------------------------------------------------------------------------------
' FILENAME: citizen_account_distribution.asp
' AUTHOR: Steve Loar
' CREATED: 01/17/2013
...
' 1.0   1/17/2013		Steve Loar - INITIAL VERSION
'
'------------------------------------------------------------------------------
'
'------------------------------------------------------------------------------
...
' USER SECURITY CHECK
PageDisplayCheck "citizen account distribution report ", sLevel	' In common.asp
...
<head>
	...
	<!--
		...
		var validate = function( media ) {
			if ( datesAreValid() ) {
				if ( media === 'screen') {
					document.reportFilter.action = 'citizen_account_distribution.asp';
				}
				else {
					// this changes based on the report selected
					document.reportFilter.action = 'citizen_account_distribution_export.asp';
				}
...
<form action="account_distribution.asp" method="post" name="reportFilter">

	<table border="0" cellpadding="10" cellspacing="0" class="start" width="100%">
		<tr>
			<td><font size="+1"><strong>Citizen Account Distribution <%=sRptTitle%></strong></font></td>
		</tr>
		<tr>
			<td>
				<fieldset>
					...
					<p>
						...
						<strong>Entries: </strong><% ShowJournalEntryTypes iJournalEntryTypeId, "citizens" %>
					</p>
		...
		<tr>
			...
			<td colspan="3" valign="top">
				...
				If sRptType = "Detail" Then
					...
					Display_Citizen_Acct_Dist_Details varWhereClause
				Else
					'DisplaySummary varWhereClause
					Display_Citizen_Acct_Dist_Summary varWhereClause
				End If 
...
''	sSql = sSql & "WHERE journalentrytype = 'withdrawl' "
''	sSql = sSql & " OR journalentrytype = 'deposit' ORDER BY displayorder"
''
''	Set oRs = Server.CreateObject("ADODB.Recordset")
''	oRs.Open  sSql, Application("DSN"), 0, 1
''
