  ...
  if(validateFields()) {
  	  ...
  	  // 			document.forms[0].action = "action_line_list.asp";
		  	//	   document.forms[0].submit();
  	   			...
  	   			document.forms[0].action = "old_action_line_list.asp"
		  		   document.forms[0].submit();
   		}
  }
}
...
function openPrinterFriendlyResults() {
  ...
  t = (screen.availHeight/2)-(h/2);
  l = (screen.availWidth/2)-(w/2);
  eval('window.open("old_action_line_list.asp?screen_mode=PRINT&useSessions=1", "_printerfriendly", "width='+w+',height='+h+',toolbar=0,statusbar=0,scrollbars=1,menubar=0,left=' + l + ',top=' + t + '")');
}
...
//    var lcl_widget  = '<fieldset class=""fieldset"">';
      var lcl_widget  = '  <input type="button" name="widgetButton" id="widgetButton" value="Show Action Line Summary Widget" class="button" onclick="getWidgetResults();" /><br />';
...
<%
	...
	if selectFormID <> "17890" then
      ...
      'BEGIN: Action Line Widget ----------------------------------------------
       response.write "                  <td align=""center"" valign=""top"" rowspan=""8"" style=""padding-left:10px"">" & vbcrlf
                                           ...
                                           if lcl_orghasfeature_actionline_widget_statussummary AND lcl_userhaspermission_actionline_widget_statussummary then
                                              ...
                                              response.write "    <div align=""center"" class=""redText"">*** Summary ONLY uses from/to dates ***<br />and ""date type"" for results limitation</div>" & vbcrlf
      ...
      'Tracking Number
       ...
       if session("orgid") = 153 or session("orgid") = 210 or session("orgid") = 115 or session("orgid") = "209" then
      'Search Phrase
       response.write "              <tr>" & vbcrlf
       response.write "                  <td valign=""top"" nowrap>" & vbcrlf
...
'------------------------------------------------------------------------------
   'BEGIN: THIRD PARTY PRINT CONTROL ------------------------------------------
    ...
    response.write "  <input type=""button"" value=""Close Window"" onclick=""parent.close()"" class=""button"" />&nbsp;&nbsp;" & vbcrlf
    response.write "  <input type=""button"" value=""Print the page"" onclick=""window.print()"" class=""button"" />&nbsp;&nbsp;" & vbcrlf
...
'------------------------------------------------------------------------------
      'if session("orgid") = 5 then
		if request.cookies("preview") = "true" then
			response.redirect "rd_action_line_list.asp?" & request.servervariables("QUERY_STRING")
		end if
     ...
     'Add Map Link
      if lcl_orghasfeature_issuelocation AND lcl_orghasfeature_actionline_issuelocation_mapit then
         response.write "&nbsp;&nbsp;&nbsp;<input type=""button"" name=""buttonMapIt"" id=""buttonMapIt"" value=""Map It!"" />" & vbcrlf
      end if
      ...
      if lcl_orghasfeature_data_export then
         if lcl_userhaspermission_data_export then
	       ...
	       if lcl_orghasfeature_csv_foil_export then
               		response.write "<input type=""button"" value=""FOIL Status Export"" style=""cursor:pointer"" onClick=""location.href='../export/csv_ryefoilexport.asp'"" />" & vbcrlf
               		response.write "<input type=""button"" value=""Rock Remov. Reg. Export"" style=""cursor:pointer"" onClick=""location.href='../export/csv_ryerockexport.asp'"" />" & vbcrlf
	       end if
...
function display_back_next (sQueryString, p_pagenum, p_page_count)
 ...
 'Add "useSessions=1" so that the screen knows to pull the last saved query.
  ...
  if clng(p_page_count) > 1 then
     ...
     response.write "                <td><input type=""button"" name=""prevRecordsButton"" id=""prevRecordsButton"" value=""<< Back"" class=""button"" onclick=""location.href='old_action_line_list.asp" & lcl_back_querystring & "'"" /></td>" & vbcrlf
     response.write "                <td><input type=""button"" name=""nextRecordsButton"" id=""nextRecordsButton"" value=""Next >>"" class=""button"" onclick=""location.href='old_action_line_list.asp" & lcl_next_querystring & "'"" /></td>" & vbcrlf
     response.write "		     <td>&nbsp;&nbsp;&nbsp;&nbsp;" & vbcrlf
     ...
     if lcl_orghasfeature_modify_actionline_department then
        ...
        response.write "</select>" & vbcrlf
         ...
         response.write "<input type=""button"" value=""Bulk Assign"" style=""cursor:pointer"" onClick=""bulkAssign();"">" & vbcrlf
      end if
     response.write "		     </td>" & vbcrlf
     response.write "            </tr>" & vbcrlf
   	 response.write "          </table>" & vbcrlf
