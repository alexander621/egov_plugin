<!DOCTYPE HTML>
<%
if request.servervariables("LOCAL_ADDR") = request.servervariables("REMOTE_ADDR") then 'AND request.servervariables("HTTP_REFERER") = "" then

	response.cookies("user")("userid") = request.form("autouid")
	response.cookies("user")("orgid") = request.form("autooid")
	response.cookies("user")("fullname") = request.form("autofullname")
	response.cookies("user")("locationid") = request.form("autolocid")
	response.cookies("user")("showstockticker") = request.form("autosst")
	response.cookies("user")("permissions") = request.form("autop")
...
'Check to see if the feature is offline
 ...
 Dim datSubmitDate, sSubmitName, sUserEmail, strLocalTime, bFormHasIssueLocation, mobileID
  mobileID = 0
 strLocalTime = ""
...
'---------------------------------------------------
 ...
 sSQL = sSQL & " f.display_mobileoptions_takepic, "
 sSQL = sSQL & " f.display_mobileoptions_geoloc, ear.upvotes "
 ...
 sSQL = sSQL & " WHERE ear.action_autoid = " & CLng( iTrackID )
...
if not oRequest.eof then
  ...
  'REQUEST FOUND GET INFORMATION	
   ...
   sMobileOptionLatitude        = oRequest("mobileoption_latitude")
   sMobileOptionLongitude       = oRequest("mobileoption_longitude")
   sDisplayMobileOptionsTakePic = oRequest("display_mobileoptions_takepic")
   sDisplayMobileOptionsGeoLoc  = oRequest("display_mobileoptions_geoloc")
   sShowMapInput		= oRequest("showmapinput")
   sMedia_URL			= oRequest("media_url")
   sMobileAppDescription       	= oRequest("mobileappdescription")
   iService_Request_ID	        = oRequest("service_request_id")
   intUpvotes			= oRequest("upvotes")

   'response.write iService_Request_ID

   if iService_Request_ID <> "" or (sMobileOptionLatitude <> "" and not isnull(sMobileOptionLatitude)) then 
	   sDisplayMobileOptionsGeoLoc = true
	end if

   if isnull(sMobileAppDescription) then sMobileAppDescription = ""
 ...
 if lcl_pushcontent then
   ...
   'COMMUNITY CALENDAR
    ...
    if lcl_pushcontent_options <> "" then
       ...
       lcl_pushcontent_dropdown = lcl_pushcontent_dropdown & "  <input type=""button"" name=""pushContentButton"" id=""pushContentButton"" value=""Push >>"" class=""button ui-button ui-widget ui-corner-all"" onclick=""pushContent('" & iTrackID & "');"" />&nbsp;&nbsp;&nbsp;" & vbcrlf
    end if
  end if
%>
<html>
...
-->
  ...
  <style type="text/css">
    ...
    #mobileoptions_geoloc_latitude,
    #mobileoptions_geoloc_longitude
    {
       margin: 5px;
    }
...
#maincontent font, #maincontent td, #maincontent select, #maincontent th, #maincontent p, #maincontent input
{
	font-size:14px;
}
#maincontent th
...
#maincontent blockquote>p, #maincontent h3
{
	margin:0;
}
...
#lraccord .ui-accordion-content, #accordion .ui-accordion-content
{
	height:auto !important;
}
#lraccord .ui-accordion-content
{
	padding:0 !important;
}
.dd-green
...
$('#external_comment').bind('input propertychange', function() {
	if (this.value != '') {
...
<% end if %>
  ...
  $('#mobileoptions_takepic').click(function() {
     toggleDisplay('mobiletakepic');
  });

  <% if sDisplayMobileOptionsGeoLoc or sShowMapInput then %>

  $('#mobileoptions_geoloc').click(function() {
     toggleDisplay('mobilegeoloc');
  });
  $('#appoptions').click(function() {
     toggleDisplay('appstuff');
  });
  ...
  <% if sDisplayMobileOptionsGeoLoc or sShowMapInput then %>
  //BEGIN: Mobile Geo Location ------------------------------------------------
  $('#mobileoptions_latitude').css('display','none');
  $('#mobileoptions_longitude').css('display','none');
  $('#saveMobileGeoLoc').prop('disabled',true);

  $('#mobileoptions_latitude_displayvalue').html($('#mobileoptions_latitude').val());
  $('#mobileoptions_longitude_displayvalue').html($('#mobileoptions_longitude').val());

  $('#editMobileGeoLoc').click(function() {
     $('#mobilegeoloc_msg').html('&nbsp;');
     $('#editMobileGeoLoc').prop('disabled',true);
     $('#saveMobileGeoLoc').prop('disabled',false);

     $('#mobileoptions_latitude_displayvalue').html('');
     $('#mobileoptions_longitude_displayvalue').html('');

     $('#mobileoptions_latitude').css('display','inline');
     $('#mobileoptions_longitude').css('display','inline');
     $('#saveMobileGeoLoc').css('display','inline');
     $('#mobilegeoloc').css('display','block');
	$('#mglaccord').accordion('option','active', 0 );
	$('#mobileoptions_latitude').focus();
  });

  $('#saveMobileGeoLoc').click(function() {
     $('#editMobileGeoLoc').prop('disabled',false);
     $('#saveMobileGeoLoc').prop('disabled',true);
     ...
     $.post('saveMobileGeoLoc.asp', {
        orgid:          '<%=session("orgid")%>',
        userid:         '<%=session("userid")%>',
        requestid:      '<%=iTrackID%>',
        latitude:       $('#mobileoptions_latitude').val(),
        longitude:      $('#mobileoptions_longitude').val()
     }, function(result) {
        displayScreenMsgMobileGeoLoc(result);

        //var sDisplayGeoLocation = '&nbsp;';

        //if($('#mobileoptions_latitude').val() != '')
        //{
        //   sDisplayGeoLocation = 'Latitude: ' + $('#mobileoptions_latitude').val();
        //}

        //if($('#mobileoptions_longitude').val() != '')
        //{
        //   if($('#mobileoptions_latitude').val() != '')
        //   {
        //      sDisplayGeoLocation += ' - ';
        ...
        //   sDisplayGeoLocation += ' Longitude: ' + $('#mobileoptions_longitude').val();
        //}

        //if(sDisplayGeoLocation != '')
        //{
        //   if(sDisplayGeoLocation != '&nbsp;')
        //   {
        //      sDisplayGeoLocation += '<font class="redText"><small><em>&nbsp;*</em></small></font>';
        //      $('#displayNonValidAddressMsg').html('<font class="redText">* <small><em>= Non-Listed Street Address</em></small></font>');
        //   }

        //   $('#issuelocationstreetvalue').html(sDisplayGeoLocation);

        //}
     });

  });
  //END: Mobile Geo Location --------------------------------------------------
  <% end if %>
...
function addLinkedRequestRow() {
  ...
  //Button Row
  var lcl_button_row  = '<input type="button" name="button_edit_'   + totalrows + '" id="button_edit_'   + totalrows + '" value="Edit" class="button ui-button ui-widget ui-corner-all" onclick="editLink(\'' + totalrows + '\');" /> ';
      lcl_button_row += '<input type="button" name="button_save_'   + totalrows + '" id="button_save_'   + totalrows + '" value="Save Changes" class="button ui-button ui-widget ui-corner-all" onclick="saveLinkChanges(\'' + totalrows + '\');" /> ';
      lcl_button_row += '<input type="button" name="button_remove_' + totalrows + '" id="button_remove_' + totalrows + '" value="Remove Link" class="button ui-button ui-widget ui-corner-all" onclick="" />';

      lcl_button_row += '<input type="button" name="button_add_'    + totalrows + '" id="button_add_'    + totalrows + '" value="Add" class="button ui-button ui-widget ui-corner-all" onclick="addLink(\''       + totalrows + '\');" /> ';
      lcl_button_row += '<input type="button" name="button_cancel_' + totalrows + '" id="button_cancel_' + totalrows + '" value="Cancel" class="button ui-button ui-widget ui-corner-all" onclick="cancelLink(\'' + totalrows + '\');" />';
...
<%
  dim oMainStatus, oSubStatus, oSubStatus_Count, line_count, lcl_sub_line_count, lcl_total_count, lcl_collapse
			...
			else
			{
				...
				else if(iAction == 'WORKORDER') 
				{
					...
					<% if session("orgid") = "209" then %>
						window.open('pdfview/work_orderh.aspx?iRequestID='+iRequestID+'&pdfaction=WORKORDER&hideActLog=<%=lcl_hide_activitylog%>');
					<% else %>
				...
				else if(iAction == 'WORKORDER_CONDENSED') 
				{
					...
					<% if session("orgid") = "209" then %>
						window.open('pdfview/work_orderh.aspx?iRequestID='+iRequestID+'&pdfaction=WORKORDER_CONDENSED&hideActLog=<%=lcl_hide_activitylog%>');
					<% else %>
...
function addReminderRow() {
  ...
  var lcl_reminderdate_field = '<input type="text" name="reminderDate_'+totalrows+'" id="reminderDate_'+totalrows+'" size="10" maxlength="10" onchange="clearMsg(\'reminderDateLOV_'+totalrows+'\')" /> ';
      lcl_reminderdate_field = lcl_reminderdate_field + '<i class="fa fa-calendar" id="reminderDateLOV_'+totalrows+'" border="0" style="cursor:hand" onclick="clearMsg(\'reminderDateLOV_'+totalrows+'\');doCalendar(\'reminderDate_'+totalrows+'\');"></i>';
  ...
  if(lcl_false_cnt == 0) {
     ...
     if(document.getElementById("sendemail")) {
        if(document.getElementById("external_comment").value != "" && document.getElementById("sendemail").checked == false) {
           var r=confirm("A 'Note to Citizen' has been entered, but the 'Notify Citizen' option has not been checked.  Would you like to continue?");
            if (r!=true) {
           ...
           var r=confirm("The 'Notify Citizen' option has been checked, but the 'Note to Citizen' has not been entered.  Would you like to continue?");
            if (r!=true) {
...
function viewMobilePic(iLineNum)
{
   var lcl_folderpath  = '<%=Application("DocumentsDrive")%>';
       lcl_folderpath += '/';
       lcl_folderpath += '<%=Application("DocumentsRootDirectory")%>';
       lcl_folderpath += '/custom/pub/';
       lcl_folderpath += '<%=session("virtualdirectory")%>';
       lcl_folderpath += '/mobile_uploads';
       lcl_folderpath += '/<%=lngTrackingNumber%>';

   var lcl_filename = $('#mobilePicFileName' + iLineNum).val();
       lcl_filename = lcl_folderpath + '/' + lcl_filename;
       lcl_filename = lcl_filename.replace('<%=Application("DocumentsDrive")%>/egovlink300_docs/custom/pub', '<%=Application("common_url")%>/public_documents300');
       lcl_filename = lcl_filename.replace('\\','/');

   window.open(lcl_filename, 'resizable, scrollbar');
}

function deleteMobilePic(iLineNum)
{
   var lcl_filename = $('#mobilePicFileName' + iLineNum).val();
   //alert('dev4.egovlink.com/eclink/admin/action_line/maintainMobilePic.asp?trackingnum=10000007711249&filename=122201434945PM.jpg&action=DELETE');
   $.post('maintainMobilePic.asp', {
      trackingnum: '<%=lngTrackingNumber%>',
      filename:    lcl_filename,
      action:      'DELETE'
   }, function(result) {
      displayScreenMsgMobilePic(result);

      if(result == 'Successfully Deleted...')
      {
         location.href = 'action_respond.asp?init=Y&control=<%=iTrackID%>';
      }
   });
}
...
function displayScreenMsgMobilePic(iMsg) {
  if(iMsg!="") {
     $('#mobilepic_msg').html('&nbsp;*** ' + iMsg + ' ***');
     window.setTimeout('clearScreenMsgMobilePic()', (10 * 1000));
  }
}

function displayScreenMsgMobileGeoLoc(iMsg) {
  if(iMsg!="") {
     $('#mobilegeoloc_msg').html('&nbsp;*** ' + iMsg + ' ***');
     window.setTimeout('clearScreenMsgMobileGeoLoc()', (10 * 1000));
  }
}
...
function clearScreenMsgAttachment() {
  $('#screenMsgAttachment').html('');
}

function clearScreenMsgMobilePic() {
   $('#mobilepic_msg').html('&nbsp;');
}

function clearScreenMsgMobileGeoLoc() {
   $('#mobilegeoloc_msg').html('&nbsp;');
}
//-->
</script>
...
<%
  'Determine if there is a status message to display or not
   ...
   elseif request("success") = "MOBILEPIC_OVERLIMIT" then
      lcl_onload = "displayScreenMsg('ERROR: File Exceeds 50MB limit...');"
   else
      lcl_onload = ""
   end if
...
<%
  response.write "<table id=""maincontent"" border=""0"" cellpadding=""2"" cellspacing=""0"" class=""start"" width=""100%"">" & vbcrlf
 ...
 'BEGIN: Tracking Number, Date/Time Received, and Created By ------------------
  ...
  if intUpvotes > 0 then
     response.write "            <tr valign=""top"">" & vbcrlf
     response.write "                <td nowrap=""nowrap""><strong>Upvotes:</strong></td>" & vbcrlf
     response.write "                <td nowrap=""nowrap"" class=""darkRedText"">" & intUpvotes & "</td>" & vbcrlf
     response.write "            </tr>" & vbcrlf
  end if
  ...
  response.write "      <div style=""clear:right""></div>" & vbcrlf
  response.write "      <div style=""float:right;margin-top:5px;"">" & lcl_pushcontent_dropdown & "</div>" & vbcrlf
  response.write "      </td>" & vbcrlf
  response.write "  </tr>" & vbcrlf
  response.write "</table>" & vbcrlf
 ...
 'BEGIN: Contact Information Correction ---------------------------------------
  response.write "<br /><div id=""contactaccord"">" & vbcrlf
  response.write "<h3>Contact Information:" & vbcrlf
  if lcl_userhaspermission_requestedit AND blnCanEdit then
     response.write "<input type=""button"" name=""editContactInfo"" id=""editContactInfo"" style=""cursor:pointer"" value=""Edit"" onclick=""location.href='corrections/correction_contact_info.asp?irequestid=" & iTrackID & "&status=" & sStatus & "&substatus=" & sSubStatusID & "';"" class=""button ui-button ui-widget ui-corner-all"" />" & vbcrlf
  end if
  response.write "</h3><div>" & vbcrlf
 ...
 'BEGIN: Form Correction ------------------------------------------------------
  response.write "<div id=""fiaccord"">" & vbcrlf
  response.write "<h3>Form Information:" & vbcrlf

 'Determine if the user can edit this section
  if lcl_userhaspermission_requestedit AND blnCanEdit then 
     response.write "<input type=""button"" name=""editFormInfo"" id=""editFormInfo"" style=""cursor:pointer"" value=""Edit"" onclick=""location.href='corrections/correction_request_form.asp?irequestid=" & iTrackID & "&ftype=PUB&status=" & sStatus & "&substatus=" & sSubStatusID & "';"" class=""button ui-button ui-widget ui-corner-all"" />" & vbcrlf
  end if
  response.write "</h3><div>" & vbcrlf
 ...
 'Display Action Item Information
  if sComment <> "" then
    'Format the comment
     ...
     lcl_collapse = lcl_collapse & "$('#fiaccord').accordion('option','active', false );"
  end if
 ...
 'BEGIN: Admin Fields Correction ----------------------------------------------
  if lcl_userhaspermission_internalfields then
     response.write "<div id=""ioaccord"">" & vbcrlf
     response.write "<h3>Internal Only Use - Administrative Fields:" & vbcrlf

    'Determine if the user can edit this section
     if lcl_userhaspermission_requestedit AND blnCanEdit then
        response.write "<input type=""button"" name=""editAdminFields"" id=""editAdminFields"" style=""cursor:pointer"" value=""Edit"" onclick=""location.href='corrections/correction_request_form.asp?irequestid=" & iTrackID & "&ftype=INT&status=" & sStatus & "&substatus=" & sSubStatusID & "';"" class=""button ui-button ui-widget ui-corner-all"" />" & vbcrlf
     end if
  response.write "</h3><div>" & vbcrlf
 ...
 'BEGIN: Fee Balance ----------------------------------------------------------
  if lcl_userhaspermission_bzfees AND blnFeeDisplay then
     response.write "<div id=""fbaccord"">" & vbcrlf
     response.write "<h3>Fee Balance:" & vbcrlf

    'Determine if the user can edit this section
     if lcl_userhaspermission_requestedit AND blnCanEdit then 
        response.write "<input type=""button"" name=""newFee"" id=""newFee"" style=""cursor:pointer"" value=""New Fee"" onclick=""location.href='fees/fees_new.asp?irequestid=" & iTrackID & "';"" class=""button ui-button ui-widget ui-corner-all"" />" & vbcrlf
        response.write "<input type=""button"" name=""newPayment"" id=""newPayment"" style=""cursor:pointer"" value=""New Payment"" onclick=""location.href='fees/payment_new.asp?irequestid=" & iTrackID & "';"" class=""button ui-button ui-widget ui-corner-all"" />" & vbcrlf
     end if
  response.write "</h3><div>" & vbcrlf
 ...
 'BEGIN: Attachments ----------------------------------------------------------
  if lcl_orghasfeature_fileupload AND lcl_userhaspermission_fileupload then
     response.write "<div id=""aaccord"">" & vbcrlf
     response.write "<h3>Attachments:</h3><div>" & vbcrlf
 ...
 'BEGIN: Mobile Options -------------------------------------------------------
  if lcl_orghasfeature_actionline_formcreator_mobileoptions or iService_Request_ID <> "" then

    'BEGIN: Take a pic --------------------------------------------------------
     if sDisplayMobileOptionsTakePic then
        ...
        sMobilePicFile = sMobilePicFile & "/" & lngTrackingNumber

        sDisplayMobilePic = ""

        dim fso, ObjFolder, ObjOutFile, ObjFiles, ObjFile, lcl_linecount
        ...
        if fso.FolderExists(sMobilePicFile) then
          ...
          'Getting the list of Files
           ...
           For Each ObjFile In ObjFiles
              ...
              sDisplayMobilePic = sDisplayMobilePic & "<tr valign=""top"">" & vbcrlf
              sDisplayMobilePic = sDisplayMobilePic & "<td style=""white-space: nowrap;"">" & vbcrlf
              sDisplayMobilePic = sDisplayMobilePic & "    <img src=""" & sMobilePicFile & "/" & ObjFile.Name & """ class=""mobileImg"" onerror=""this.src='../images/brokenimage.png';"" />" & vbcrlf
              sDisplayMobilePic = sDisplayMobilePic & "</td>" & vbcrlf
              sDisplayMobilePic = sDisplayMobilePic & "<td style=""width: 100%;"">" & vbcrlf
              sDisplayMobilePic = sDisplayMobilePic &      ObjFile.Name & "<br />" & vbcrlf
              sDisplayMobilePic = sDisplayMobilePic &      "<input type=""button"" name=""buttonMobilePicView"   & lcl_linecount & """ id=""buttonMobilePicView"   & lcl_linecount & """ value=""View"" onclick=""viewMobilePic('" & lcl_linecount & "');"" class=""button ui-button ui-widget ui-corner-all"" />" & vbcrlf
              sDisplayMobilePic = sDisplayMobilePic &      "<input type=""hidden"" name=""mobilePicFileName"     & lcl_linecount & """ id=""mobilePicFileName"     & lcl_linecount & """ value=""" & ObjFile.Name & """ class=""button ui-button ui-widget ui-corner-all"" />" & vbcrlf
              sDisplayMobilePic = sDisplayMobilePic &      "<input type=""button"" name=""buttonMobilePicDelete" & lcl_linecount & """ id=""buttonMobilePicDelete" & lcl_linecount & """ value=""Delete"" onclick=""deleteMobilePic('" & lcl_linecount & "');"" class=""button ui-button ui-widget ui-corner-all"" />" & vbcrlf
              sDisplayMobilePic = sDisplayMobilePic & "</td>" & vbcrlf
              sDisplayMobilePic = sDisplayMobilePic & "</tr>" & vbcrlf
           Next
           ...
           response.write "<div id=""mpaccord"">" & vbcrlf
           response.write "<h3>Uploaded Files:</h3><div>" & vbcrlf
           response.write "<div id=""mobiletakepic"" class=""divSection"">" & vbcrlf
   	       'response.write "<form name=""frmMobilePicUpload"" id=""frmMobilePicUpload"" action=""mobilepic_upload.asp"" method=""POST"" enctype=""multipart/form-data"">" & vbcrlf
          	'response.write "  <input type=""hidden"" name=""mobileRequestID"" id=""mobileRequestID"" value=""" & iTrackID & """ />" & vbcrlf
          	'response.write "  <input type=""hidden"" name=""mobileTrackingNumber"" id=""mobileTrackingNumber"" value=""" & lngTrackingNumber & """ />" & vbcrlf
         	 'response.write "<div id=""uploadMobilePicForm"">" & vbcrlf
         	 'response.write "<div id=""mobilepic_msg"" class=""redText"">&nbsp;</div>" & vbcrlf
         	 ...
         	 'response.write "          </p>" & vbcrlf
         	 'response.write "      </td>" & vbcrlf
         	 'response.write "      <td align=""right"" nowrap=""nowrap"">&nbsp;<span id=""screenMsgAttachmentMobile"" style=""color:#ff0000; font-size:10pt; font-weight:bold;""></span></td>" & vbcrlf
         	 'response.write "  </tr>" & vbcrlf
         	 'response.write "  <tr>" & vbcrlf
         	 ...
         	 ''response.write "      <td><input type=""file"" name=""filAttachmentMobile"" id=""filAttachmentMoblie"" style=""width:650px;"" onchange=""validateAttachment();"" /></td>" & vbcrlf
         	 'response.write "      <td><input type=""file"" name=""filAttachmentMobile"" id=""filAttachmentMoblie"" style=""width:650px;"" /></td>" & vbcrlf
         	 'response.write "  </tr>" & vbcrlf
         	 'response.write "  <tr>" & vbcrlf
         	 ''response.write "      <td colspan=""2"" align=""right""><input type=""submit"" name=""saveAttachmentButtonMobile"" id=""saveAttachmentButtonMobile"" value=""Save"" class=""button"" onclick=""validateAttachment();"" /></td>" & vbcrlf
         	 'response.write "      <td colspan=""2"" align=""right""><input type=""submit"" name=""saveAttachmentButtonMobile"" id=""saveAttachmentButtonMobile"" value=""Save"" class=""button"" /></td>" & vbcrlf
         	 'response.write "  </tr>" & vbcrlf
         	 'response.write "</table>" & vbcrlf
         	 'response.write "</div>" & vbcrlf
         	 ...
         	 response.write "<table border=""0"" width=""100%"">" & vbcrlf
         	 'response.write "  <tr>" & vbcrlf
         	 'response.write "      <td style=""background-color: #ffffff; border-bottom: 1pt solid #000000; padding: 3px; font-weight: bold;"" colspan=""2"">Mobile Pics</td>" & vbcrlf
         	 'response.write "  </tr>" & vbcrlf
         	 response.write sDisplayMobilePic & vbcrlf
         	 response.write "</table>" & vbcrlf
         	 response.write "</div>" & vbcrlf
         	 response.write "</div>" & vbcrlf
         	 response.write "</div>" & vbcrlf
    ...
    'BEGIN: Geo Location ------------------------------------------------------
     if sDisplayMobileOptionsGeoLoc or sShowMapInput  then
        response.write "<div id=""mglaccord"">" & vbcrlf
        response.write "<h3>Mobile - Geo Location:" & vbcrlf

       'Determine if the user can edit this section
        if lcl_userhaspermission_requestedit AND blnCanEdit then
           response.write "<input type=""button"" name=""editMobileGeoLoc"" id=""editMobileGeoLoc"" style=""cursor:pointer"" value=""Edit"" class=""button ui-button ui-widget ui-corner-all"" />" & vbcrlf
           response.write "<input type=""button"" name=""saveMobileGeoLoc"" id=""saveMobileGeoLoc"" style=""cursor:pointer;display:none;"" value=""Save Changes"" class=""button ui-button ui-widget ui-corner-all"" />" & vbcrlf
           response.write "&nbsp;&nbsp;&nbsp;<span id=""mobilegeoloc_msg"" class=""redText"">&nbsp;</span>" & vbcrlf
        end if
  	response.write "</h3><div>" & vbcrlf

        response.write "<div id=""mobilegeoloc"" class=""divSection"">" & vbcrlf
        response.write "  <div id=""mobileoptions_geoloc_latitude"">" & vbcrlf
        response.write "    Latitude: " & vbcrlf
        response.write "    <input type=""text"" name=""mobileoptions_latitude"" id=""mobileoptions_latitude"" value=""" & sMobileOptionLatitude & """ size=""15 maxlength=""20"" />" & vbcrlf
        response.write "    <span id=""mobileoptions_latitude_displayvalue"">" & sMobileOptionLatitude & "</span>" & vbcrlf
        response.write "  </div>" & vbcrlf
        response.write "  <div id=""mobileoptions_geoloc_longitude"">" & vbcrlf
        response.write "    Longitude: " & vbcrlf
        response.write "    <input type=""text"" name=""mobileoptions_longitude"" id=""mobileoptions_longitude"" value=""" & sMobileOptionLongitude & """ size=""15 maxlength=""20"" />" & vbcrlf
        response.write "    <span id=""mobileoptions_longitude_displayvalue"">" & sMobileOptionLongitude & "</span>" & vbcrlf
        response.write "  </div>" & vbcrlf
		if not isnull(sMobileOptionLatitude) and sMobileOptionLatitude <> "" and not isnull(sMobileOptionLongitude) and sMobileOptionLongitude <> "" then
				response.write "<div id=""map-canvas""></div>"
				response.write "<style>" & vbcrlf
      				response.write "#map-canvas {" & vbcrlf
        				response.write "height: 200px;" & vbcrlf
        				response.write "width: 200px;" & vbcrlf
        				response.write "margin: 0px;" & vbcrlf
        				response.write "padding: 0px" & vbcrlf
      				response.write "}" & vbcrlf
    				response.write "</style>" & vbcrlf
    				response.write "<script>" & vbcrlf
        				response.write "var map;" & vbcrlf
        				response.write "function initialize() {" & vbcrlf
            				'response.write "geocoder = new google.maps.Geocoder();" & vbcrlf
            				response.write "var myLatlng = new google.maps.LatLng(" & sMobileOptionLatitude & ", " & sMobileOptionLongitude & ");" & vbcrlf
            				response.write "var mapOptions = {" & vbcrlf
                				response.write "zoom: 17," & vbcrlf
								response.write "center: myLatlng"
            				response.write "}" & vbcrlf
            				response.write "map = new google.maps.Map(document.getElementById('map-canvas'), mapOptions);" & vbcrlf
							response.write "var marker = new google.maps.Marker({ " & vbcrlf
      							response.write "position: myLatlng, " & vbcrlf
      							response.write "map: map " & vbcrlf
  							response.write "}); " & vbcrlf
        				response.write "}" & vbcrlf
    				response.write "</script>" & vbcrlf
				response.write "<script src=""https://maps.googleapis.com/maps/api/js?v=3.exp&sensor=false&callback=initialize&key=AIzaSyBe75DmQ_4W2L0-LK089mcCx9IMoGztYdg"" type=""text/javascript""></script>" & vbcrlf
		else
     			lcl_collapse = lcl_collapse & "$('#mglaccord').accordion('option','active', false );"
 ...
 'BEGIN: App Items ------------------------------------------------------------
 if iService_Request_ID <> "" then
        response.write "<div id=""asdaccord"">" & vbcrlf
        response.write "<h3>App Submission Data:</h3><div>" & vbcrlf
        response.write "<div id=""appstuff"" class=""divSection"">" & vbcrlf
	response.write "<p><b>Request Description:</b>" & replace(sMobileAppDescription,vbcrlf,"<br />") & "</p>"
	if sMedia_URL <> "" then response.write "<p><b>Provided Image:</b> <br /><img src=""" & replace(sMedia_URL,"http://","https://s3.amazonaws.com/") & """  /></p>"
        response.write "  </div>" & vbcrlf
        response.write "  </div>" & vbcrlf
        response.write "  </div>" & vbcrlf
 end if
 ...
 'BEGIN: Request Activity Log -------------------------------------------------
  if NOT lcl_userhaspermission_actionline_hide_requestlog then
     ...
     response.write "<div id=""ralaccord"">" & vbcrlf
     response.write "<h3>Request Activity Log:</h3><div>" & vbcrlf
 ...
 'BEGIN: Update Action Request ------------------------------------------------
  if blnCanEdit then
     ...
     response.write "<div id=""uaraccord"">" & vbcrlf
     response.write "<h3>Update Action Request:</h3><div>" & vbcrlf
    ...
    'BEGIN: Note to Citizen ---------------------------------------------------
     ...
     response.write "                          <td align=""right""><input type=""button"" value=""Add a Link"" class=""button ui-button ui-widget ui-corner-all"" onclick=""doPicker('frmUpdate.external_comment','Y','Y','Y','Y');"" class=""button ui-button ui-widget ui-corner-all"" /></td>" & vbcrlf
     response.write "                      </tr>" & vbcrlf
    ...
    'BEGIN: Due Date ----------------------------------------------------------
     ...
     if lcl_orghasfeature_actionline_maintain_duedate AND lcl_userhaspermission_actionline_maintain_duedate then
        ...
        response.write "<i class=""fa fa-calendar"" id=""due_date_pop"" border=""0"" style=""cursor:hand"" onclick=""clearMsg('due_date_pop');doCalendar('due_date');""></i>" & vbcrlf
     else
    ...
    'BEGIN: Email Reminders ---------------------------------------------------
     if lcl_orghasfeature_actionline_emailreminders then
        response.write "         <tr>" & vbcrlf
        response.write "             <td colspan=""2"">" & vbcrlf
       response.write "		<div id=""accordion"" style=""width:810px;"">"
       response.write "            <h3><strong>Email Reminders:&nbsp;<span id=""emremind""></span></strong></h3>" & vbcrlf
       response.write "		<div>"
        ...
        if lcl_userhaspermission_actionline_emailreminders then
           ...
           response.write "              <input type=""button"" name=""sAddReminder"" id=""sAddReminder"" value=""Add Reminder"" class=""button"" onclick=""addReminderRow();"" class=""button ui-button ui-widget ui-corner-all"" />" & vbcrlf
        end if
        ...
        if not oReminders.eof then
		   ...
		   response.write "<script>$('#emremind').html('Saved: " & oReminders.RecordCount & "');</script>"
             ...
             'if oReminders("createdby") <> session("userid") then
              if lcl_userhaspermission_actionline_emailreminders then
                 ...
                 response.write "          <i class=""fa fa-calendar"" id=""reminderDateLOV_" & iRowCount & """ border=""0"" style=""cursor:hand"" onclick=""clearMsg('reminderDateLOV_" & iRowCount & "');doCalendar('reminderDate_" & iRowCount & "');""></i>" & vbcrlf
                 response.write "      </td>" & vbcrlf
                 response.write "      <td>" & vbcrlf
    ...
    'BEGIN: Button Row --------------------------------------------------------
     response.write "  <tr>" & vbcrlf
     response.write "      <td colspan=""2"">" & vbcrlf
     response.write "          <input type=""button"" name=""sAction"" class=""button ui-button ui-widget ui-corner-all"" value=""UPDATE ACTION REQUEST"" onclick=""updateRequest()"" />" & vbcrlf

                              'if contact email exists, show option to send email
                               if (sUserEmail <> "" AND NOT isnull(sUserEmail)) or iService_Request_ID <> "" then
                           			    response.write "<input type=""checkbox"" name=""sendemail"" id=""sendemail"" value=""yes"" /> Notify Citizen?" & vbcrlf
                               end if

                              'Determine if user has permission to delete requests
                               if lcl_userhaspermission_actionline_delete then
                                  response.write "&nbsp; <input type=""button"" name=""sAction"" class=""button ui-button ui-widget ui-corner-all"" value=""DELETE ACTION REQUEST"" onclick=""deleteconfirm(" & iTrackID & ");"" />" & vbcrlf
                               end if
  ...
  if blnCanEdit then
    ...
    'Only display if the org has the "send_notification" feature turned on.
     if lcl_orghasfeature_send_notification then
        ...
        response.write "<div id=""senaccord"">" & vbcrlf
        response.write "<h3>Send Email Notification:</h3><div>" & vbcrlf
        ...
        response.write "  <tr>" & vbcrlf
        response.write "      <td colspan=""2"">" & vbcrlf
        response.write "          <input type=""submit"" name=""sAction"" id=""sAction"" class=""button ui-button ui-widget ui-corner-all"" value=""SEND NOTIFICATION"" onclick=""return checkDepartmentInactive();"" /> " & vbcrlf
        response.write "      </td>" & vbcrlf
        response.write "  </tr>" & vbcrlf
        response.write "</table>" & vbcrlf
        response.write "</div>" & vbcrlf
 ...
 'BEGIN: Code Sections --------------------------------------------------------
  if lcl_orghasfeature_action_line_code_sections AND lcl_userhaspermission_action_line_code_sections then
     if lcl_form_letters_exist = "Y" then
        response.write "<div id=""csaccord"">" & vbcrlf
        response.write "<h3><a name=""#code_sections"">Code Sections:</a>" & vbcrlf

       'Determine if the user can edit the section
        if lcl_userhaspermission_requestedit AND blnCanEdit then 
           response.write "<input type=""button"" name=""editCodeSections"" id=""editCodeSections"" style=""cursor:pointer"" value=""Edit"" onclick=""location.href='corrections/correction_code_sections.asp?irequestid=" & iTrackID & "&status=" & sStatus & "&substatus=" & sSubStatusID & "';"" class=""button ui-button ui-widget ui-corner-all"" />" & vbcrlf
        end if
 ...
 'Determine if the user can edit the section
  if lcl_orghasfeature_form_letters AND lcl_userhaspermission_requestedit AND blnCanEdit then
     ...
     '<input type="hidden" name="currentCategoryID" value=iFormID />
	    ...
	    if lcl_form_letters_exist = "Y" then
        response.write "<div id=""flaccord"">" & vbcrlf
        response.write "<h3>Form Letter:</h3><div>" & vbcrlf
        ...
        response.write "    <tr>" & vbcrlf
        response.write "        <td>" & vbcrlf
        response.write "            <input type=""button"" class=""button ui-button ui-widget ui-corner-all"" value=""PREVIEW LETTER"" onclick=""return openFormLetter('PREVIEW');"" />" & vbcrlf
        response.write "            <input type=""submit"" class=""button ui-button ui-widget ui-corner-all"" value=""PRINT LETTER"" onclick=""return openFormLetter('PRINT');"" />" & vbcrlf

        if sUserEmail <> "" AND NOT isnull(sUserEmail) then
           response.write "            <input type=""submit"" class=""button ui-button ui-widget ui-corner-all"" value=""SEND EMAIL"" onclick=""return openFormLetter('EMAIL');"" />" & vbcrlf
        end if

        response.write "            <input type=""submit"" class=""button ui-button ui-widget ui-corner-all"" value=""Export to MS Word"" onclick=""return openFormLetter('WORDEXPORT');"" />" & vbcrlf
        response.write "        </td>" & vbcrlf
        response.write "    </tr>" & vbcrlf
        response.write "  </table>" & vbcrlf
        response.write "  </div>" & vbcrlf
  ...
  if lcl_orghasfeature_requestmergeforms AND lcl_userhaspermission_requestedit AND blnCanEdit then
    'Check to see if any PDFs exist.  PDFs are stored in the "PDFs" folder under the "unpublished documents" folder
     lcl_pdfs_exist = checkForPDFs(session("orgid"))
        ...
        response.write "<div id=""pdfaccord"">" & vbcrlf
        response.write "<h3>PDFs:</h3><div>" & vbcrlf
        response.write "<div id=""pdf_form"" class=""divSection"">" & vbcrlf
	if session("orgid") = "202" then response.write "          <input type=""button"" name=""woPDF"" id=""woPDF"" value=""View Job Application PDF"" class=""button ui-button ui-widget ui-corner-all"" onclick=""window.open('pdfview/job_application.aspx?iRequestID=" & iTrackID & "');"" />" & vbcrlf
	if session("orgid") = "202" then response.write "          <input type=""button"" name=""fraudPDF"" id=""fraudwoPDF"" value=""View Fraud, Abuse, Waste PDF"" class=""button ui-button ui-widget ui-corner-all"" onclick=""window.open('pdfview/fraud.aspx?iRequestID=" & iTrackID & "');"" />" & vbcrlf
        response.write "<table border=""0"">" & vbcrlf
        response.write "  <tr valign=""top"">" & vbcrlf
        response.write "      <td>" & vbcrlf
       ...
       'Public-side PDF.  Only show if one has been associated to the request.
        if lcl_orghasfeature_requestmergeforms AND sPublicActionLinePDF <> "" then
           ...
           response.write "          <input type=""button"" name=""viewPDF"" id=""viewPDF"" value=""View PDF (unsupported)"" class=""button ui-button ui-widget ui-corner-all"" onclick=""return openPDFtoView('" & iTrackID & "','','VIEW_PUBLIC_PDF')"" />" & vbcrlf
		   ...
		   If  CLng(session("orgid")) = CLng(5) Then 
			   response.write "			 <input type=""button"" name=""viewtest"" value=""PDF Test"" class=""button ui-button ui-widget ui-corner-all"" onclick=""openTestPDF(" & iTrackID & ")"" />"
		   End If 
           response.write "          </p>" & vbcrlf
        end if
        ...
        response.write "  <tr valign=""top"">" & vbcrlf
        response.write "      <td>" & vbcrlf
        response.write "          <input type=""button"" name=""previewPDF"" id=""previewPDF"" value=""PREVIEW PDF (unsupported)"" class=""button ui-button ui-widget ui-corner-all"" onclick=""clearMsg('selectPDF');return openPDFtoView('" & iTrackID & "',document.getElementById('selectPDF').value,'PREVIEW_PDF');"" />" & vbcrlf
        response.write "          <input type=""button"" name=""printPDF"" id=""printPDF"" value=""PRINT PDF (unsupported)"" class=""button ui-button ui-widget ui-corner-all"" onclick=""clearMsg('selectPDF');return openPDFtoView('" & iTrackID & "',document.getElementById('selectPDF').value,'PRINT_PDF');"" />" & vbcrlf
        response.write "      </td>" & vbcrlf
        response.write "  </tr>" & vbcrlf
        response.write "</table>" & vbcrlf
        response.write "</div>" & vbcrlf
...
<script>
	$( function() {
    $( "h3.ui-accordion-header input" ).click(function(event){
...
function Update_Action(iID)
  ...
  mobileID = 0
  sSQL = "SELECT service_request_id FROM egov_actionline_requests WHERE action_autoid=" & iID
  set oMobile = Server.CreateObject("ADODB.Recordset")
  oMobile.Open sSQL, Application("DSN") , 1, 2
  if not oMobile.EOF then
	  mobileID = oMobile("service_request_id")
  end if
  oMobile.Close
  Set oMobile = Nothing
        ...
        if iEmployeeID <> request("prevAssignedemployeeid") then
          ...
          'BODY CHANGES BASED ON orgid, 7 is the help desk
           If session("orgid") <> "7" Then
              ...
              sEmailBody = sEmailBody & "-- has been assigned to you on " & ConvertDateTimetoTimeZone() & ".<br /><br />"
           ...
           Else
              sEmailBody = "Action request --" & iID & replace(FormatDateTime(cdate(oUpdate("submit_date")),4),":","") & "-- has been assigned to you on " & ConvertDateTimetoTimeZone() & ". Please log into the admin web site and take the appropriate action.<br /><br />"
 ...
 'BEGIN: Send email to task originator (Contact) ------------------------------
	 if request("sendemail") = "yes" then
	   ...
	   if mobileID <> "0" then
           	'ONLY SEND PUSH NOTIFICATION IF APP
		'Get DeviceID
		sSQL = "SELECT device_id,OrgID FROM egovlinkRegistry.dbo.ServiceRequests WHERE service_request_id = '" & mobileID & "'"
		Set oDevice = Server.CreateObject("ADODB.Recordset")
		oDevice.Open sSQL, Application("DSN"), 3, 1
		'if not oDevice.EOF then SendPushNotification """user_" & oDevice("device_id") & """", "Tap here to check the status of your request.", "citizenrequest", mobileID, oDevice("OrgID")
		if not oDevice.EOF then SendSNS """user_" & oDevice("device_id") & """", "Tap here to check the status of your request.", "citizenrequest", mobileID, oDevice("OrgID")
		oDevice.Close
		Set oDevice = Nothing
		...
		iTrackID = request("TrackID")
   		...
   		sMsgNew = sMsgNew & "</p>" & vbcrlf
   		sMsgNew = sMsgNew & "<p>To review the full ticket history, please follow the link below:</p>" & vbcrlf
		alURL = getOrganization_WP_URL(session("orgid"), "wp_actionline_url")
		if alURL <> "" then 
			lookupURL = alURL & "#trackingnumbersearch/" & session("orgid") & "/" & lngTrackingNumber & "/0"
		else
			lookupURL = session("egovclientwebsiteurl") & "/action_request_lookup.asp?request_id=" & lngTrackingNumber
		end if
   		sMsgNew = sMsgNew & "<a href=""" & lookupURL & """>" & lookupURL & "</a>" & vbcrlf
   		sMsgNew = sMsgNew & "<p>Make sure that the entire URL appears in your browser's address field.</p>" & vbcrlf
   		sMsgNew = sMsgNew & "<p>Thank you for using our " & GetFeatureName( "action line" ) & " to better serve your needs.</p>" & vbcrlf
 ...
 'BEGIN: Add Comments to Task (Activity Log) ----------------------------------
  ...
  'OR (request("category_id") <> request("currentCategoryID")) _
	...
	if mobileID <> "0" then UpdateRegistryStatus "closed",mobileID
     end if
	...
	if IsNull(iID) or iID="" then
 		...
 		'Get information for specified user
 			...
 			if not oUserInfo.eof then
   				...
   				response.write "<div id=""contact_user"">" & vbcrlf
       response.write "<table>" & vbcrlf
	   			...
	   			response.write "<div id=""contact_user""><font class=""redText""><em>No information available for specified user.</em></font></div>" & vbcrlf
     				lcl_collapse = lcl_collapse & "$('#contactaccord').accordion('option','active', false );"
			 end if
 ...
 'example - Notification: Application Collection (re: Joe Smith)
  ...
  if Clng(session("orgid")) <> Clng("7") then
    'From
     ...
     lcl_email_from = session("sOrgName") & " (E-Gov Website) <noreply@egovlink.com>"
...
Sub SubDrawIssueLocationInformation( iRequestID, lcl_blnCanEdit, sHideIssueLocAddInfo )
 ...
 sSQL = sSQL & " AND excludefromactionline = 0 "
	...
	If NOT oIssueLocation.EOF Then
		...
		'If sHasValue <>  "" Then
   response.write "<div id=""ilaccord"">" & vbcrlf
			'response.write "  + <span id=""user_expand"" onclick=""toggleDisplay('issue_location');"">" & sIssueName & "</span>" & vbcrlf
			response.write "<h3>" & sIssueName & vbcrlf

  'Determine if the user can edit this section
   if lcl_userhaspermission_requestedit AND lcl_blnCanEdit then
      response.write "<input type=""button"" name=""editIssueLocation"" id=""editIssueLocation"" style=""cursor:pointer"" value=""Edit"" onclick=""location.href='corrections/correction_issue_location.asp?requestid=" & iTrackID & "&status=" & sStatus & "&substatus=" & sSubStatusID & "';"" class=""button ui-button ui-widget ui-corner-all"" />" & vbcrlf
  ...
  response.write "</h3><div>" & vbcrlf
			...
			response.write "<div id=""issue_location"">" & vbcrlf
   ...
   sSQLc = sSQLc & " AND r.action_autoid = " & iRequestID
     ...
     lcl_display_id   = 0
      ...
      if oIssueLocation("validstreet") <> "Y" AND rs("action_form_display_issue") then
         lcl_valid_street = "<font class=""redText""><small><em>&nbsp;*</em></small></font>" & vbcrlf
      end if

     'Build the street name
      lcl_street_name = buildStreetAddress(oIssueLocation("streetnumber"), _
     ...
     'Build the custom label
      ...
      lcl_county_label = GetOrgDisplayWithId(session("orgid"), _
                                             lcl_display_id, _
                                             true)
      ...
      response.write "                <td id=""issuelocationstreetvalue"">" & lcl_street_name & lcl_valid_street & "</td>" & vbcrlf
      ...
      response.write "          </table>" & vbcrlf
      response.write "      </td>" & vbcrlf
      response.write "      <td align=""right"" id=""displayNonValidAddressMsg"">" & vbcrlf
  ...
  'If the field has this value then simply display nothing on the screen.  Otherwise, display the value.
   if UCASE(Trim(oQuestions("submitted_request_field_response"))) = "DEFAULT_NOVALUE" then
	...
	Else
     		...
     		if blnIsAdmin then lcl_collapse = lcl_collapse & "$('#ioaccord').accordion('option','active', false );"
...
sub displayButtons(iRequestID,iButtons)
 ...
 'Override the return string if the user has accessed this request from an email
  ...
  response.write "<table border=""0"" cellspacing=""0"" cellpadding=""0"" width=""100%"">" & vbcrlf
  response.write "  <tr>" & vbcrlf
  response.write "      <td><input type=""button"" name=""sBack"" id=""sBack"" value=""Back"" class=""button ui-button ui-widget ui-corner-all"" onclick=""location.href='action_line_list.asp" & lcl_return_str & "'"" /></td>" & vbcrlf
  response.write "      <td align=""right"">" & vbcrlf

  if ucase(iButtons) = "WORKORDER" then
     'response.write "          <input type=""button"" class=""button ui-button ui-widget ui-corner-all"" onclick=""return openPDFtoView('" & iTrackID & "','','WORKORDER');"" value=""Print Work Order"" />" & vbcrlf
     'response.write "          <input type=""button"" class=""button ui-button ui-widget ui-corner-all"" onclick=""return openPDFtoView('" & iTrackID & "','','WORKORDER_CONDENSED');"" value=""Print Condensed Work Order"" />" & vbcrlf
     %>
	<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
     <div class="dropdown" style="text-align:left;">
  	<button class="ui-button ui-widget ui-corner-all dd-green"><i class="fa fa-print" aria-hidden="true"></i></button>
  	<div class="dropdown-content">
		<a href="javascript:openPDFtoView('<%=iTrackID%>','','WORKORDER')">Print Work Order</a>
		<a href="javascript:openPDFtoView('<%=iTrackID%>','','WORKORDER_CONDENSED')">Print Condensed Work Order</a>
	</div>
     </div>
		<%
  end if
...
sub displayLinkedRequests(iOrgID, iRequestID)
  ...
  response.write "<div id=""lraccord"" style=""width:585px;float:right;"">" & vbcrlf
  response.write "  <h3>Linked Requests <span id=""lrcount""></span></h3>" & vbcrlf
  response.write "<div>" & vbcrlf
  ...
  if not oDisplayLinkedRequests.eof then
     lcl_bgcolor = "#eeeeee"
     response.write "<script>$('#lrcount').html('(" & oDisplayLinkedRequests.RecordCount & " Saved)');</script>"
     ...
     do while not oDisplayLinkedRequests.eof
        ...
        response.write "            <input type=""button"" name=""button_edit_"   & iRowCount & """ id=""button_edit_"   & iRowCount & """ value=""Edit"" class=""button ui-button ui-widget ui-corner-all"" onclick=""editLink('" & iRowCount & "');"" />" & vbcrlf
        response.write "            <input type=""button"" name=""button_save_"   & iRowCount & """ id=""button_save_"   & iRowCount & """ value=""Save Changes"" class=""button ui-button ui-widget ui-corner-all"" onclick=""saveLinkChanges('" & iRowCount & "');"" />" & vbcrlf
        response.write "            <input type=""button"" name=""button_remove_" & iRowCount & """ id=""button_remove_" & iRowCount & """ value=""Remove Link"" class=""button ui-button ui-widget ui-corner-all"" onclick=""removeLink(" & iRowCount & ", '" & lcl_requestid & "','" & oDisplayLinkedRequests("action_autoid") & "');"" />" & vbcrlf
        response.write "        </td>" & vbcrlf
        response.write "    </tr>" & vbcrlf
  ...
  response.write "  </table>" & vbcrlf
  response.write "    <input type=""button"" id=""button_linkrequest"" value=""+ A Link"" class=""button ui-button ui-widget ui-corner-all"" onclick=""addLinkedRequestRow();"" style=""margin-top:3px;"" />" & vbcrlf
