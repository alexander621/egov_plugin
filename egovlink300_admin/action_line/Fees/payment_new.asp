...
If Request.ServerVariables("REQUEST_METHOD") = "POST" THEN
	...
	If NOT oUser2.EOF Then
		...
		'CALCULATE BALANCE
		BALANCE = CalculateBalance(ActionID)
		...
		sMsgNew = sMsgNew & "<p>A new payment has been added to the Action Line with the tracking number of: " & lngTrackingNumber
		sMsgNew = sMsgNew & "<br>Description: " & Description
		sMsgNew = sMsgNew & "<br>Payment: $" & request.form("Amount")
		if Balance > 0 then
			sMsgNew = sMsgNew & "<br>Balance: $" & BALANCE & " (overpayment)"
		else
			sMsgNew = sMsgNew & "<br>Balance: $" & BALANCE * -1 & " (remaining to be paid)"
		end if

		sendEmail "", sUserEmail, "", Ucase(sOrgName) & " E-GOV MSG - ACTION LINE NEW PAYMENT", sMsgNew, "", "N"
...
<div id="content">
	<div id="centercontent">
		...
		<form action=# method=post name=addpayment>
			Payment Description: <input type=text name=Description value=""><br>
			Payment Amount: $<input type=text name=Amount value=""><br>
			<input type=hidden name=ef:Description-text/req value="Payment Description">
			<input type=hidden name=ef:Amount-text/currency/req value="Payment Amount">
			<input type=button class=button onClick="if(validateForm('addpayment')) {document.addpayment.submit();};" value="Add Payment">
		</form>
...
Function CalculateBalance(iRequestID) 
