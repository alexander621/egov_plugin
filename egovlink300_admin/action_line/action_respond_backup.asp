...
' 4.3	 05/06/13	 Steve Loar  - Pointed some PDF outputs to new viewXMLPDF.asp script
'
'------------------------------------------------------------------------------
'
'------------------------------------------------------------------------------
...
<% end if %>
  ...
  $('#mobileOptions').click(function() {
     toggleDisplay('mobileOptionsInfo');
  });
  ...
  $('#uploadAttachmentForm').prop('display','none');

  $('#buttonUploadAttachment').click(function() {
     $('#uploadAttachmentForm').slideDown('slow');
  });
...
<%
  ...
  if lcl_pushcontent then
     response.write "          <span class=""darkRedText"">Push request to </span>" & vbcrlf
    ...
    'COMMUNITY CALENDAR
     ...
     then
        response.write "            <option value=""COMMUNITYCALENDAR"">Community Calendar</option>" & vbcrlf
     end if
 ...
 'BEGIN: Mobile Options -------------------------------------------------------
  if lcl_orghasfeature_actionline_formcreator_mobileoptions then
     response.write "<p>" & vbcrlf
     response.write "  + <span id=""mobileOptions"" class=""user_expand"">Mobile Options:</span>" & vbcrlf

    'Determine if the user can edit this section
     if lcl_userhaspermission_requestedit AND blnCanEdit then 
        response.write "<input type=""button"" name=""editMobileOptions"" id=""editMobileOptions"" style=""cursor:pointer"" value=""Edit"" onclick=""location.href='corrections/correction_mobileoptions.asp?irequestid=" & iTrackID & "&status=" & sStatus & "&substatus=" & sSubStatusID & "';"" />" & vbcrlf
     end if

     response.write "<div id=""mobileOptionsInfo"" class=""divSection"">" & vbcrlf
     response.write "<p>" & vbcrlf
     response.write "<fieldset class=""fieldset""> " & vbcrlf
     response.write "  <legend>Geolocation</legend> " & vbcrlf
     response.write "  <table border=""0"">" & vbcrlf
     response.write "    <tr>" & vbcrlf
     response.write "        <td><strong>Latitude: </strong> " & sMobileOptionLatitude & "</td>" & vbcrlf
     response.write "    </tr>" & vbcrlf
     response.write "        <td><strong>Longitude: </strong> " & sMobileOptionLongitude & "</td>" & vbcrlf
     response.write "    </tr>" & vbcrlf
     response.write "  </table>" & vbcrlf
     ...
     response.write "<p>" & vbcrlf
     response.write "<fieldset class=""fieldset""> " & vbcrlf
     response.write "  <legend>Pictures</legend> " & vbcrlf

     lcl_isMobile  = "Y"
     lcl_form_name = "MOBILEOPTIONS"

     response.write "<p>" & vbcrlf

'     if lcl_userhaspermission_requestedit AND blnCanEdit then 
'        subDisplayAttachments "E", _
'                              iTrackID, _
'                              sStatus, _
