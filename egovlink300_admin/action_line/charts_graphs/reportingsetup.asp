...
' 1.0 09/01/10 David Boyer - Initial Version
'
'------------------------------------------------------------------------------
'
'------------------------------------------------------------------------------
...
'If empty then default to the current date
 ...
 if lcl_sc_selectedchart = "1" then
    lcl_selected_charttype1 = " selected=""selected"""
 elseif lcl_sc_selectedchart = "2" then
    lcl_selected_charttype2 = " selected=""selected"""
 elseif lcl_sc_selectedchart = "3" then
    lcl_selected_charttype3 = " selected=""selected"""
 elseif lcl_sc_selectedchart = "4" then
    lcl_selected_charttype4 = " selected=""selected"""
 elseif lcl_sc_selectedchart = "5" then
    lcl_selected_charttype5 = " selected=""selected"""
 elseif lcl_sc_selectedchart = "6" then
    lcl_selected_charttype6 = " selected=""selected"""
 elseif lcl_sc_selectedchart = "7" then
    lcl_selected_charttype7 = " selected=""selected"""
 end if
...
function checkChartTypes() {
  ...
  if (lcl_chartselected == "4" || lcl_chartselected == "5" || lcl_chartselected == "6" || lcl_chartselected == "7") {
...
//     doAjax('getchartgraph.asp', sParameter, 'displayChart', 'post', '0');
     document.getElementById("chartGraphSearchCriteria").action = "reportingsetup.asp";
...
<p><font size="+1"><strong>Action Line: Charts</strong></font></p>
<%
 'BEGIN: Tabs (headers) -------------------------------------------------------
  response.write "<div id=""demo"" class=""yui-navset"" style=""width:800px"">" & vbcrlf
...
%>
<table border="0" cellpadding="0" cellspacing="0" class="start" style="width:800px">
  <form name="chartGraphSearchCriteria" id="chartGraphSearchCriteria" method="POST" action="">
    <input type="hidden" name="cid" id="cid" value="<%=lcl_chartid%>" size="5" maxlength="10" />
  <tr>
      <td>
          <fieldset>
            ...
            <table border="0" cellspacing="0" cellpadding="2">
              <tr valign="top">
                  <td nowrap="nowrap">Request Submission Date:</td>
                  <td>
                      <input type="text" name="fromDate" id="fromDate" value="<%=lcl_sc_fromDate%>" size="10" maxlength="10" onchange="clearMsg('fromDateCalPop');" />
                      <a href="javascript:void doCalendar('From');"><img src="../../images/calendar.gif" id="fromDateCalPop" border="0" style="cursor:pointer" onclick="clearMsg('fromDateCalPop');" /></a>
                  </td>
                  <td>
                      To: 
                      <input type="text" name="toDate" id="toDate" value="<%=lcl_sc_toDate%>" size="10" maxlength="10" onchange="clearMsg('toDateCalPop');" />
                      <a href="javascript:void doCalendar('To');"><img src="../../images/calendar.gif" id="toDateCalPop" border="0" style="cursor:pointer" onclick="clearMsg('toDateCalPop');" /></a>
                  </td>
                  <td><% DrawDateChoices "Date","" %></td>
              </tr>
              <tr valign="top">
                  <td colspan="2">
                      <input type="checkbox" name="sc_includedates" id="sc_includedates" value="Y"<%=lcl_sc_includedates_checked%> />Include search dates in chart/graph title
                  </td>
                  <td colspan="2">
                      Form Types:
                      <select name="sc_formtype" id="sc_formtype">
                        <option value=""<%=lcl_selected_formtype_all%>>&nbsp;</option>
                        <option value="PUBLIC"<%=lcl_selected_formtype_public%>>Public Only</option>
                        <option value="INTERNAL"<%=lcl_selected_formtype_internal%>>Internal Only</option>
                      </select>
                  </td>
              </tr>
            </table>
          ...
          <fieldset>
            <legend>Chart/Graph Options&nbsp;</legend>
            <p>
            <table border="0" cellspacing="0" cellpadding="2">
              <tr>
                  <td nowrap="nowrap">Charts:</td>
                  <td>
                      <select name="sc_selectedchart" id="sc_selectedchart" onchange="checkChartTypes()">
<!--                        <option value="1"<%'lcl_selected_charttype1%>>Monthly Status</option> -->
<!--                        <option value="2"<%'lcl_selected_charttype2%>>Monthly Status by Department</option> -->
                        <option value="3"<%=lcl_selected_charttype3%>>Most Submitted Requests</option>
                        <option value="6"<%=lcl_selected_charttype6%>>Open Items by Department</option>
                        <option value="5"<%=lcl_selected_charttype5%>>Open Items Activity by Department</option>
                        <option value="7"<%=lcl_selected_charttype7%>>Open Items by Form</option>
                        <option value="4"<%=lcl_selected_charttype4%>>Open Items Activity by Form</option>
                      </select>
                  </td>
                  <td id="sc_charttype_label">Chart Type:</td>
                  <td>
                      <span id="sc_charttype_display"></span>
                      <select name="sc_charttype" id="sc_charttype">
              ...
              <tr>
                  <td>Chart Width:</td>
                  <td>
                      <select name="sc_chartwidth" id="sc_chartwidth">
                        <%
                  ...
                  <td id="sc_showlegend_label">Show Legend:</td>
                  <td>
                      <input type="checkbox" name="sc_showlegend" id="sc_showlegend" value="Y"<%=lcl_checked_showlegend%> />
                  </td>
              </tr>
              <tr>
                  <td>Chart Height:</td>
                  <td>
                      <select name="sc_chartheight" id="sc_chartheight">
                        <%
                  ...
                  <td id="sc_legendtitle_label">Legend Title:</td>
                  <td>
                      <input type="text" name="sc_legendtitle" id="sc_legendtitle" value="<%=lcl_sc_legendtitle%>" size="30" maxlength="50" />
                  </td>
              </tr>
              <tr>
                  <td colspan="2" style="padding-left:6px">
                      <table border="0" cellspacing="0" cellpadding="0">
                        <tr>
                            <td id="sc_collectedthreshold_label" nowrap="nowrap">Group Results with Minimum Value:</td>
                            <td width="100%" style="padding-left:2px;"><input type="text" name="sc_collectedthreshold" id="sc_collectedthreshold" value="<%=lcl_sc_collectedthreshold%>" size="4" maxlength="5" onchange="clearMsg('sc_collectedthreshold');" /></td>
                        </tr>
                      </table>
                  </td>
                  <td nowrap="nowrap" id="sc_collectedlabel_label">Minimum Value Label:</td>
                  <td>
                      <input type="text" name="sc_collectedlabel" id="sc_collectedlabel" value="<%=lcl_sc_collectedlabel%>" size="30" maxlength="50" />
                  </td>
              </tr>
            </table>
            </p>
          </fieldset>
          <table border="0" cellspacing="0" cellpadding="2" width="100%" style="margin-top:5pt; margin-bottom:5pt; margin-left:5pt;">
            <tr>
                <td><input type="button" name="viewChartButton" id="viewChartButton" value="View Chart" class="button" onclick="viewChart();" /></td>
                <td align="right">
                <% if CLng(lcl_chartid) > 0 then %>
                    <input type="button" name="printChartButton" id="printChartButton" value="Print Chart" class="button" onclick="printChart('<%=lcl_chartid%>');" />
                <% end if %>
                </td>
            </tr>
          </table>
