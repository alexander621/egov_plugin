...
'---------------------------------------------------
 ...
 sSQL = sSQL & " f.display_mobileoptions_geoloc "
...
<% end if %>
  ...
  <% if sDisplayMobileOptionsGeoLoc then %>
...
<%
 ...
 'END: Tracking Number, Date/Time Received, and Created By --------------------
      'if session("orgid") = 5 then
		if request.cookies("preview") = "true" then
			response.redirect "rd_action_respond.asp?" & request.servervariables("QUERY_STRING")
		end if
		response.write "<style> .ui-button-live, .ui-button-live:hover { background-color: #2C5F93; color:white; margin-bottom:5px; padding:5px; width:125px; } </style>"
	      response.write "<div style=""position:fixed;bottom:0;right:0;background:white;padding:5px;"">"
	      response.write "<input type=""button""  class=""ui-button ui-button-live ui-widget ui-corner-all btn-red"" onclick=""window.location='cookiechange.asp'"" value=""Preview New Look!"" /><br />"
	      response.write "<a href=""action_line_updates.pdf"" target=""_newtab"">What is this?</a>"
	      response.write "</div>"
      'end if
 ...
 'BEGIN: Mobile Options -------------------------------------------------------
  if lcl_orghasfeature_actionline_formcreator_mobileoptions or iService_Request_ID <> "" then
        ...
        if fso.FolderExists(sMobilePicFile) then
          ...
          'Getting the list of Files
           ...
           For Each ObjFile In ObjFiles
              ...
              sDisplayMobilePic = sDisplayMobilePic &      "<input type=""button"" name=""buttonMobilePicView"   & lcl_linecount & """ id=""buttonMobilePicView"   & lcl_linecount & """ value=""View"" onclick=""viewMobilePic('" & lcl_linecount & "');"" />" & vbcrlf
              sDisplayMobilePic = sDisplayMobilePic &      "<input type=""hidden"" name=""mobilePicFileName"     & lcl_linecount & """ id=""mobilePicFileName"     & lcl_linecount & """ value=""" & ObjFile.Name & """ />" & vbcrlf
              sDisplayMobilePic = sDisplayMobilePic &      "<input type=""button"" name=""buttonMobilePicDelete" & lcl_linecount & """ id=""buttonMobilePicDelete" & lcl_linecount & """ value=""Delete"" onclick=""deleteMobilePic('" & lcl_linecount & "');"" />" & vbcrlf
              sDisplayMobilePic = sDisplayMobilePic & "</td>" & vbcrlf
           ...
           response.write "<p>" & vbcrlf
           response.write "  + <span id=""mobileoptions_takepic"" class=""user_expand"">Mobile Pics:</span>" & vbcrlf
    ...
    'BEGIN: Geo Location ------------------------------------------------------
     if sDisplayMobileOptionsGeoLoc  then
        response.write "<p>" & vbcrlf
        response.write "  + <span id=""mobileoptions_geoloc"" class=""user_expand"">Mobile - Geo Location:</span>" & vbcrlf

       'Determine if the user can edit this section
        if lcl_userhaspermission_requestedit AND blnCanEdit then
           response.write "<input type=""button"" name=""editMobileGeoLoc"" id=""editMobileGeoLoc"" style=""cursor:pointer"" value=""Edit"" />" & vbcrlf
           response.write "<input type=""button"" name=""saveMobileGeoLoc"" id=""saveMobileGeoLoc"" style=""cursor:pointer"" value=""Save Changes"" />" & vbcrlf
 ...
 if iService_Request_ID <> "" then
        response.write "<p>" & vbcrlf
        response.write "  + <span id=""appoptions"" class=""user_expand"">App Submission Data:</span>" & vbcrlf
 ...
 'BEGIN: PDF Forms ------------------------------------------------------------
  if lcl_orghasfeature_requestmergeforms AND lcl_userhaspermission_requestedit AND blnCanEdit then
	...
	if session("orgid") = "202" then response.write "          <input type=""button"" name=""woPDF"" id=""woPDF"" value=""View Job Application PDF"" class=""button"" onclick=""window.open('pdfview/job_application.aspx?iRequestID=" & iTrackID & "');"" />" & vbcrlf
	if session("orgid") = "202" then response.write "          <input type=""button"" name=""fraudPDF"" id=""fraudwoPDF"" value=""View Fraud, Abuse, Waste PDF"" class=""button"" onclick=""window.open('pdfview/fraud.aspx?iRequestID=" & iTrackID & "');"" />" & vbcrlf
        response.write "<table border=""0"">" & vbcrlf
        response.write "  <tr valign=""top"">" & vbcrlf
        response.write "      <td>" & vbcrlf
       ...
       'Public-side PDF.  Only show if one has been associated to the request.
        if lcl_orghasfeature_requestmergeforms AND sPublicActionLinePDF <> "" then
           ...
           response.write "          <input type=""button"" name=""viewPDF"" id=""viewPDF"" value=""View PDF (unsupported)"" class=""button"" onclick=""return openPDFtoView('" & iTrackID & "','','VIEW_PUBLIC_PDF')"" />" & vbcrlf
        ...
        response.write "  <tr valign=""top"">" & vbcrlf
        response.write "      <td>" & vbcrlf
        response.write "          <input type=""button"" name=""previewPDF"" id=""previewPDF"" value=""PREVIEW PDF (unsupported)"" class=""button"" onclick=""clearMsg('selectPDF');return openPDFtoView('" & iTrackID & "',document.getElementById('selectPDF').value,'PREVIEW_PDF');"" />" & vbcrlf
        response.write "          <input type=""button"" name=""printPDF"" id=""printPDF"" value=""PRINT PDF (unsupported)"" class=""button"" onclick=""clearMsg('selectPDF');return openPDFtoView('" & iTrackID & "',document.getElementById('selectPDF').value,'PRINT_PDF');"" />" & vbcrlf
        response.write "      </td>" & vbcrlf
        response.write "  </tr>" & vbcrlf
        response.write "</table>" & vbcrlf
        response.write "</div>" & vbcrlf
