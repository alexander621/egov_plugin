...
'If user had set search options for this session then get the session values
 ...
 if lcl_useSessions = 1 then
   'Determine if user has a "Last Queried" record.  If any exist then retreive the "customreportid" and "isuserdefault" values
    getCustomReportInfo "ACTIONLINE - LAST QUERIED", False, session("orgid"), session("userid"), False, lcl_customreportid, _
                        lcl_reporttypeid, lcl_reportname, lcl_isuserdefault

   'Get the Org/System Defaults
    if lcl_customreportid = "" then
       getCustomReportInfo "ACTIONLINE - DEFAULTS", True, session("orgid"), session("userid"), False, lcl_customreportid, _
                           lcl_reporttypeid, lcl_reportname, lcl_isuserdefault
    end if
 ...
 if lcl_init = "Y" then
    ...
    if lcl_orghasfeature_savesearchoptions_actionline then
      'Determine if user has saved search options.  If any exist then retreive the "customreportid" and "isuserdefault" values
       getCustomReportInfo "ACTIONLINE - USER", False, session("orgid"), session("userid"), True, lcl_customreportid, lcl_reporttypeid, _
...
<%
   ...
   'Order By
    ...
    displayOrderByList orderBy, lcl_orghasfeature_issuelocation
   ...
   'Status
    ...
    displaySubStatusOptions lcl_userhaspermission_action_line_substatus, substatus_hidden, show_hide_substatus
   ...
   'Report Type
    ...
    displayReportTypesList reporttype, lcl_orghasfeature_actionline_listfull, lcl_orghasfeature_responsetimereporting
   ...
   'BEGIN: Action Line Widget -------------------------------------------------
    response.write "                  <td align=""center"" valign=""top"" rowspan=""8"" style=""padding-left:10px"">" & vbcrlf
                                          if lcl_orghasfeature_actionline_widget_statussummary AND lcl_userhaspermission_actionline_widget_statussummary then
                                             'displayActionLineWidget session("orgid"), session("userid"), selectDateType, fromDate, toDate
                                             response.write "<span id=""actionlinewidget"">" & vbcrlf
                                             response.write "<fieldset>" & vbcrlf
                                             response.write "<input type=""button"" name=""widgetButton"" id=""widgetButton"" value=""Show Action Line Summary Widget"" class=""button"" onclick=""getWidgetResults();"" /><br />" & vbcrlf
                                             response.write "<div align=""center"" style=""color:#ff0000"">*** Summary ONLY uses from/to dates ***<br />and ""date type"" for results limitation</div>" & vbcrlf
                                             response.write "</fieldset>" & vbcrlf
...
'BEGIN: Action Line Request List ----------------------------------------------
 ...
 List_Action_Requests sSortBy, request.querystring
...
'BEGIN: Google Map ------------------------------------------------------------
 response.write "<form name=""frmgooglemap"" action=""" & Application("MAP_URL") & "admin/action_line/action_line_map.asp"" method=""post"" target=""GOOGLE_MAP_WINDOW"">" & vbcrlf
...
'------------------------------------------------------------------------------
sub List_Action_Requests( sSortBy, sQueryString )
...
'restrict what they can view by the permission level they have
 ...
 if selectUserFName <> "all" AND selectUserFName <> "" then
    varWhereClause = varWhereClause & " AND UserFName LIKE '" & dbsafe(selectUserFName) & "%'"
 end if

 if selectUserLName <> "all" AND selectUserLName <> "" then
    varWhereClause = varWhereClause & " AND UserLName LIKE '" & dbsafe(selectUserLName) & "%'"
 end if
...
if selectTicket <> "" then
   if IsNumeric(selectTicket) AND len(selectTicket) > 4 then 
      sTicketNo = Left( selectTicket, (Len(selectTicket) - 4))

      iTrackID  = CStr(CDbl(selectTicket))
...
'   sSQL = sSQL & "[dbo].[checkDateDiff]('DAY',adjustedsubmitdate,submit_date,complete_date,'" & today &"') as lcl_subTotal_totalDaysOpen_adjusted2, "
  ...
  'Are we searching on "Open" or "Past" dates
   if searchDaysType = "PAST" AND pastDays <> "all" then
...
'------------------------------------------------------------------------------
      response.write "<br /><font size=""3"" color=""#3399ff""><i><strong>" & lcl_reportlabel & " REPORT</strong></i></font>" & vbcrlf

     'Add Map Link
      if lcl_orghasfeature_issuelocation AND lcl_orghasfeature_actionline_issuelocation_mapit then
         response.write "&nbsp;&nbsp;&nbsp;<input type=""button"" value=""Map It!"" style=""cursor: hand"" onclick=""frmgooglemap.submit();"" />" & vbcrlf
      end if

      response.write "<br /><strong>Page <font color=""#0000ff"">" & oRequests.AbsolutePage & "</font>  " & vbcrlf
      response.write "of <font color=""#0000ff""> " & oRequests.PageCount & "</font></strong> &nbsp;|&nbsp; " & vbcrlf
      response.write "<strong><font color=""#0000ff"">" & oRequests.RecordCount & "</font> total Action Item Requests</strong>" & vbcrlf
         ...
         if lcl_userhaspermission_data_export then
            if request.querystring("reporttype") = "" or LCASE(request.querystring("reporttype")) = "list" Then
              'DISPLAY EXPORT BUTTON ONLY FOR LIST VIEW
               response.write "<input type=""button"" value=""Download as CSV"" style=""cursor:pointer"" onClick=""location.href='../export/csv_export.asp'"" />" & vbcrlf
              ...
              'Check to see if the org and the user have the "Action Line - CSV Export (parsed)" feature assigned.
               if lcl_orghasfeature_csv_export_parsed AND lcl_userhaspermission_csv_export_parsed then
                  response.write "<input type=""button"" value=""Download as CSV (parsed)"" style=""cursor:pointer"" onClick=""location.href='../export/csv_export_parsed.asp'"" />" & vbcrlf
               end if

               if lcl_orghasfeature_activity_log_download AND lcl_userhaspermission_activity_log_download then
                  response.write "<input type=""button"" value=""Download Activity Log"" style=""cursor:pointer"" onClick=""location.href='../export/csv_export_activitylog.asp'"" />" & vbcrlf
               end if
            End If
         End If
      End If
     ...
     'Print Multiple Work Orders
      if lcl_orghasfeature_display_multiple_workorders then
         response.write "<input type=""button"" value=""Print Work Order(s)"" style=""cursor:pointer"" onClick=""printWorkOrders();"">" & vbcrlf
      end if
  ...
  'BEGIN: Show column headers -------------------------------------------------
   if UCASE(reporttype) = "LISTFULL" then
      ...
      if lcl_screen_mode <> "PRINT" then
         ...
         display_back_next sQueryString, session("pagenum"), oRequests.PageCount
   ...
   for intRec=1 to oRequests.PageSize
       if not oRequests.EOF then
         ...
         'The department has been deleted
          ...
          if (searchDaysType = "PAST" AND pastDays <> "all") OR _
             (searchDaysType = "OPEN" AND pastDays <> "all" AND pastDays <> "0") then
         ...
         'BEGIN: Display data rows --------------------------------------------
          if UCASE(reporttype) = "LISTFULL" then
            ...
            'Setup the javascript events for the row.
             ...
             response.write "<tr valign=""top"" bgcolor=""" & bgcolor & """ id=""row_" & oRequests("action_autoid") & """" & lcl_row_onmouseover & lcl_row_onmouseout & ">" & vbcrlf
             if lcl_screen_mode <> "PRINT" then
                if lcl_orghasfeature_display_multiple_workorders then
...
'------------------------------------------------------------------------------
            'BEGIN: Insert sub-total row if Detail Report ---------------------
             ...
             response.write "<tr bgcolor=""" & bgcolor & """ id=""row_" & oRequests("action_autoid") & """ align=""center""" & lcl_row_onmouseover & lcl_row_onmouseout & lcl_row_onclick & ">" & vbcrlf
         ...
         '  subtotal row for the group.
          if UCASE(reporttype) = "DETAIL" OR UCASE(reporttype) = "DRILLTHRU" then
            ...
            'Track each row in a group and calculate the subtotals
             trackSubTotals orderBy, sStatus, lcl_currentvalue, lcl_previousvalue, lcl_subTotal_totalRequests, _
                            lcl_subTotal_totalDaysOpen, lcl_subTotal_totalRequestsOpen, lcl_subTotal_totalDaysClosed, _
                            lcl_subTotal_totalRequestsClosed, countDays, lcl_subTotal_totalRequests, lcl_subTotal_totalDaysOpen, _
                            lcl_subTotal_totalRequestsOpen, lcl_subTotal_totalDaysClosed, lcl_subTotal_totalRequestsClosed
          end if
 ...
 sBGColor = "#E0E0E0"
	...
	if not oCommentList.eof then
       ...
       lcl_comments_lastname  = oCommentList("lastname")
  		  	...
  		  	response.write "  <tr><td>" & lcl_comments_firstname & " " & lcl_comments_lastname & " - " & UCASE(oCommentList("action_status")) & lcl_substatus_name & " - " &  oCommentList("action_editdate") & vbcrlf
         ...
         'Determine if an email was sent to the citizen.
          ...
          response.write "<br />&nbsp;&nbsp;&nbsp;<strong>Note to Citizen: </strong><i>" & lcl_action_externalcomment  & "</i>" & vbcrlf
  		  	end if

    			if oCommentList("action_citizen") <> "" then
          response.write "<br />&nbsp;&nbsp;&nbsp;<strong>" & oCommentList("userfname")  & " " & oCommentList("userlname") & " : </strong><i>" & oCommentList("action_citizen") & "</i>" & vbcrlf
  		  	end if

    			if oCommentList("action_internalcomment") <> "" then
          response.write "<br />&nbsp;&nbsp;&nbsp;<strong>Internal Note: </strong><i>" & oCommentList("action_internalcomment")  & "</i>" & vbcrlf
  		  	end if
...
function display_back_next (sQueryString, p_pagenum, p_page_count)
 ...
 'Add "useSessions=1" so that the screen knows to pull the last saved query.
  ...
  if clng(p_page_count) > 1 then
     ...
     response.write "                <td><input type=""button"" name=""prevRecordsButton"" id=""prevRecordsButton"" value=""<< Back"" class=""button"" onclick=""location.href='action_line_list.asp" & lcl_back_querystring & "'"" /></td>" & vbcrlf
     response.write "                <td><input type=""button"" name=""nextRecordsButton"" id=""nextRecordsButton"" value=""Next >>"" class=""button"" onclick=""location.href='action_line_list.asp" & lcl_next_querystring & "'"" /></td>" & vbcrlf
     response.write "            </tr>" & vbcrlf
   	 response.write "          </table>" & vbcrlf
     response.write "      </td>" & vbcrlf
 ...
 'Show the Custom Reports - Code Sections button if the org and user have the feature assigned
  ...
  'AND lcl_userhaspermission_customreports _
  if lcl_orghasfeature_customreports_codesections AND lcl_userhaspermission_customreports_codesections then
     response.write "                <td><input type=""button"" name=""sCustomReports_CodeSections"" id=""sCustomReports_CodeSections"" value=""Custom Report - Code Violations"" style=""cursor:pointer"" onclick=""openCustomReports('codesections')"" /></td>" & vbcrlf
  end if

	 'response.write "                <td nowrap=""nowrap""><a href=""action_line_list.asp?screen_mode=PRINT&useSessions=1"" target=""new"">Open New Printer Friendly Results Window</a></td>" & vbcrlf
  response.write "                <td><input type=""button"" name=""printerFriendlyButton"" id=""printerFriendlyButton"" value=""Printer Friendly Results"" class=""button"" onclick=""openPrinterFriendlyResults()"" /></td>" & vbcrlf
  response.write "            </tr>" & vbcrlf
  response.write "          </table>" & vbcrlf
  response.write "      </td></tr>" & vbcrlf
  response.write "</table>" & vbcrlf
...
'------------------------------------------------------------------------------
 sub displayTotalRow (p_total_type, p_record_count, p_lastDate, p_subTotal, p_numOpen, p_avgOpenTotal, p_avgClosedTotal)
   ...
   'Determine if this is a Grand Total or Sub-Total row.
    if UCASE(p_total_type) = "GRANDTOTAL" then
       ...
       lcl_row_info = "Subtotal: " & p_lastDate
    end if
...
'------------------------------------------------------------------------------
sub trackSubTotals(ByVal iOrderBy, ByVal iStatus, ByVal iCurrentValue, ByVal iPreviousValue, ByVal iSubTotal_TotalRequests, _
                   ByVal iSubTotal_TotalDaysOpen, ByVal iSubTotal_TotalRequestsOpen, _
                   ByVal iSubTotal_TotalDaysClosed, ByVal iSubTotal_TotalRequestsClosed, ByVal iCountDays, _
                   ByRef lcl_subTotal_totalRequests, ByRef lcl_subTotal_totalDaysOpen, ByRef lcl_subTotal_totalRequestsOpen, _
