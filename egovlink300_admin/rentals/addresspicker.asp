	...
	<head>
		...
		<!--
			...
			function doSelect()
			{
				...
				window.opener.document.<%=request("parentform")%>.residentstreetnumber.value = document.frmAddress.stnumber.value;
				window.opener.document.<%=request("parentform")%>.useraddress.value = '';
				<% if request("sCheckType") = "FinalCheck" then %>
					window.opener.checkDuplicateCitizens( 'FinalUserCheckFailed' );
			...
			function doKeep()
			{
				window.opener.document.<%=request("parentform")%>.useraddress.value = document.frmAddress.oldstnumber.value + ' ' + document.frmAddress.stname.value;
				window.opener.document.<%=request("parentform")%>.residentstreetnumber.value = '';
				window.opener.document.<%=request("parentform")%>.address.selectedIndex = 0;
				<% if request("sCheckType") = "FinalCheck" then %>
					window.opener.checkDuplicateCitizens( 'FinalUserCheckFailed' );
				<% elseif request("sCheckType") = "FinalCheckValidate" then %>
					window.opener.finalCheckValidate();
