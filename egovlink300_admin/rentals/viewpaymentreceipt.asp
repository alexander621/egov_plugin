...
Dim dProcessingFee, bHasPaymentFee, lcl_total_display, bShowUnDoBtn
...
Else 	
	...
	If UserHasPermission( Session("UserId"), "undo rental payment" ) Then
		...
		If IsAdminTransaction( iPaymentId ) Then 
			bShowUnDoBtn = true
		End If 
	End If 	
End If 
...
<head>
	...
	<script>
		...
		var unDoTransaction = function( paymentId ) {
			var okToProceed = confirm("This will permanently remove this payment from the system. Do you wish to continue?");
			
			if ( okToProceed ) {
				
				var request = jQuery.ajax({  
					url: "./undo_payment.asp",  
					type: "POST",  
					dataType: "text",
					data: { 
						paymentId : paymentId
				...
				request.done( function( data ) { 
					...
					if (data === 'Success') {
						alert("This payment has successfully been removed. \nPrint this receipt if you need to. Once you leave this page, all related information will be gone.");
						$("#undobutton").hide();
					}
					else {
						alert("Failed: This payment was not successfully removed.");
					}
				});
...
<%
	...
	If GetPaymentDetails(iPaymentId, iUserId, nTotal, dPaymentDate, iAdminLocationId, iJournalEntryTypeId, sNotes, iAdminuserid ) Then 
		...
		response.write "<div><span id=""receiptadmininfo"">"
	...
	'	'response.write "[" & sJournalEntryType & "]"
		...
		ShowReservationDetails iReservationId, iPaymentId, sEntryType, sJournalEntryType, bHasPaymentFee, dProcessingFee
...
Sub ShowReservationDetails( ByVal iReservationId, ByVal iPaymentId, ByVal sEntryType, ByVal sJournalEntryType, ByVal bHasPaymentFee, ByVal dProcessingFee )
	Dim sSql, oRs, dTotalCharge, sChargeLabel, dTotalRefundDue

	dTotalCharge = CDbl(0)
	sChargeLabel = "Charges"
	...
	Do While Not oRs.EOF
		...
		oRs.MoveNext 
	Loop
	'response.write "dTotalCharge1 = " & dTotalCharge & "<br /><br />"
	...
	'response.write "dTotalCharge2 = " & dTotalCharge & "<br /><br />"
	...
	If bHasPaymentFee Then
		...
		response.write vbcrlf & "</table>"
	End If 
	'response.write "dTotalCharge3 = " & dTotalCharge & "<br /><br />"

	' Show the total charges paid
	If sJournalEntryType = "refund" Then
		dTotalCharge = dTotalCharge - ShowRefundFee( iPaymentId )
	End If
	'response.write "dTotalCharge4 = " & dTotalCharge & "<br /><br />"
	...
	If sReservationTypeSelector <> "block" And sReservationTypeSelector <> "class" Then
		...
		response.write "<td class=""totalscell"" align=""right""><strong>Total " & sChargeLabel & "</strong></td>"
		response.write "<td class=""totalscell"" align=""right"">"
		response.write FormatNumber(dTotalCharge,2,,,0) 
		response.write "</td>"
		response.write "</tr>" & vbcrlf


		
		' Do the reservation summary
		' current charges Row
		iRowCount = iRowCount + 1
		If iRowCount Mod 2 = 0 Then
  			sClass = " class=""altrow"" "
		Else
		  	sClass = ""
		End If 
		response.write "<tr" & sClass & " id=""rentalreceiptsummary"">" 
		response.write "<td class=""totalscell"" colspan=""4"">Summary of Payments And Refunds</td>" 
		response.write "</tr>" & vbcrlf

		dCurrentCharges = GetReservationTotalAmount( iReservationId, "totalamount" ) ' In rentalscommonfunctions.asp
		response.write "  <tr" & sClass & ">"
		response.write "      <td id=""receiptRowTotalPaid"" colspan=""3"" align=""right"" class=""totalscell""><strong>Current Charges</strong></td>"
		response.write "      <td id=""receiptRowTotalPaid"" align=""right"" class=""totalscell"">" & FormatNumber(dCurrentCharges,2,,,0) & "</td>"
		response.write "  </tr>" & vbcrlf
		...
		response.write "  <tr" & sClass & ">" 
		response.write "      <td id=""receiptRowPayments"" class=""totalscell"" colspan=""4""><strong>Payments</strong></td>" 
		response.write "  </tr>" & vbcrlf
		...
		response.write "  <tr" & sClass & ">"
		response.write "      <td id=""receiptRowTotalPaid"" colspan=""3"" align=""right"" class=""totalscell""><strong>Total Paid</strong></td>"
		response.write "      <td id=""receiptRowTotalPaid"" align=""right"" class=""totalscell"">" & FormatNumber(dTotalPaid,2,,,0) & "</td>"
		response.write "  </tr>" & vbcrlf
		...
		If sReservationTypeSelector = "public" Then
			...
			End If 
			response.write "<tr" & sClass & ">" 
			response.write "<td class=""totalscell"" colspan=""4""><strong>Refunds & Refund Fees</strong></td>"
			response.write "</tr>" & vbcrlf
		...
		If sReservationTypeSelector = "public" Then
			...
			response.write vbcrlf & "<tr" & sClass & " id=""refunddue"">"
		...
		dBalanceDue = (dCurrentCharges + dTotalRefunded) - (dTotalPaid - dTotalRefundDue)
		'response.write "<!-- Current Charges :" & dCurrentCharges & "-->" & vbcrlf
		'response.write "<!-- Total Refunded:" & dTotalRefunded & "-->" & vbcrlf
		'response.write "<!-- Total Paid:" & dTotalPaid & "-->" & vbcrlf
		'response.write "<!-- Total Refund Due:" & dTotalRefundDue & "-->" & vbcrlf

		response.write "<tr" & sClass & " id=""rentalreceiptbalancedue"">" 
...
Sub ShowReservationReceiptNotes( ByVal iReservationId, ByVal sJournalEntryType )
	...
	If Not oRs.EOF Then
		If oRs("receiptnotes") <> "" Then 
			response.write vbcrlf & "<div class=""receiptsection""><strong>"
			If sJournalEntryType = "rentalpayment" Then 
			...
			response.write oRs("receiptnotes") & "</div>"
		End If 
	End If 
...
'--------------------------------------------------------------------------------------------------
' boolean IsAdminTransaction( iPaymentId )
'--------------------------------------------------------------------------------------------------
Function IsAdminTransaction( ByVal iPaymentId )
	Dim sSql, oRs
	...
	If Not oRs.EOF Then
		' Limit to purchases only (journalentrytypeid = 8) from the admin side (isadminmethod) OR Refunds( =2 )'
		If ( clng(oRs("journalentrytypeid")) = clng(8) And oRs("isadminmethod") ) Or clng(oRs("journalentrytypeid")) = clng(2) Then 
