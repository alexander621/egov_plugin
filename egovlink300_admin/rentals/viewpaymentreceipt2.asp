...
Sub ShowReservationDetails( ByVal iReservationId, ByVal iPaymentId, ByVal sEntryType, ByVal sJournalEntryType, ByVal bHasPaymentFee, ByVal dProcessingFee )
	Dim sSql, oRs, dTotalCharge, dTotalRefundFees

	dTotalCharge = CDbl(0.0000)
	dTotalRefundFees = CDbl( 0.00)
	...
	If sReservationTypeSelector <> "block" And sReservationTypeSelector <> "class" Then
		'lcl_total_charges = GetReservationTotalAmount(iReservationID, "totalamount") ' In rentalscommonfunctions.asp
		lcl_total_charges = getTotalRentalCharges( iReservationID )
  ...
  response.write "  </tr>" & vbcrlf
		...
		If sReservationTypeSelector = "public" Then
			...
			response.write "  </tr>" & vbcrlf
			 ...
			 'Want to show refunds with link to receipt page here
			  dTotalRefunded = displayReservationRefunds( iReservationId, sClass )
  			...
  			response.write "      <td id=""receiptRowTotalRefunds"" class=""totalscell"" align=""right""><strong>Total Refunds</strong></td>" & vbcrlf
		...
		If sReservationTypeSelector = "public" Then
  			response.write "  <tr" & sClass & ">" & vbcrlf
			response.write "      <td class=""totalscell"" colspan=""4"" align=""center""><strong>Refund Fees & Damages</strong></td>" & vbcrlf
			response.write "  </tr>" & vbcrlf

			' Want to show refunds fees with link to receipt page here
			dTotalRefundFees = displayReservationRefundFees( iReservationId, sClass )

  			response.write "  <tr" & sClass & ">" & vbcrlf
		  	response.write "      <td id=""receiptRowTotalRefunds"" class=""totalscell"" colspan=""2"">&nbsp;</td>" & vbcrlf
  			response.write "      <td id=""receiptRowTotalRefunds"" class=""totalscell"" align=""right""><strong>Total Refund Fees</strong></td>" & vbcrlf
		  	response.write "      <td id=""receiptRowTotalRefunds"" class=""totalscell"" align=""right"">" & FormatNumber(dTotalRefundFees,2,,,0) & "</td>" & vbcrlf
			response.write "  </tr>" & vbcrlf
		End If 
	...
	'Balance Due Row
		...
		dBalanceDue = lcl_total_charges - dTotalRefunded - dTotalPaid + dTotalRefundFees
...
'--------------------------------------------------------------------------------------------------
' double dAmount = getTotalRentalCharges( iReservationID )
'--------------------------------------------------------------------------------------------------
Function getTotalRentalCharges( ByVal iReservationID )
	Dim sSql, oRs

	sSql = "SELECT SUM( amount ) AS totalcharges "
	sSql = sSql & "FROM egov_accounts_ledger WHERE reservationid = " & iReservationID 
	sSql = sSql & " AND plusminus = '+' AND entrytype = 'credit' AND ispaymentaccount = 0"
	'response.write sSql & "<br />"
	...
	If Not oRs.EOF Then
		getTotalRentalCharges = CDbl(oRs("totalcharges"))
	Else
		getTotalRentalCharges = CDbl(0.00)
	End If 
...
'--------------------------------------------------------------------------------------------------
' double dAmount = displayReservationRefunds( iReservationID, sClass )
'--------------------------------------------------------------------------------------------------
function displayReservationRefunds( ByVal iReservationId, ByVal sClass )
	Dim sSql, oRs, dTotalRefunds

	dTotalRefunds = CDbl(0.00)
	...
	sSql = sSql & " WHERE A.paymentid = J.paymentid AND A.ispaymentaccount = 1 AND A.entrytype = 'credit' AND A.plusminus = '-' "
	...
	If Not oRs.EOF Then 
		...
		Do While Not oRs.EOF
			...
			dTotalRefunds = dTotalRefunds + CDbl(oRs("refundamount"))
			response.write "</td></tr>"
			oRs.MoveNext
		Loop
	End If 
	...
	displayReservationRefunds = dTotalRefunds

End Function  


'--------------------------------------------------------------------------------------------------
' double dAmount = displayReservationRefundFees( iReservationID, sClass )
'--------------------------------------------------------------------------------------------------
function displayReservationRefundFees( ByVal iReservationId, ByVal sClass )
	Dim sSql, oRs, dTotalRefundFees

	dTotalRefundFees = CDbl(0.00)
	
	sSql = "SELECT A.paymentid, J.paymentdate, SUM(A.amount) as refundfeeamount "
	sSql = sSql & " FROM egov_accounts_ledger A, egov_class_payment J "
	sSql = sSql & " WHERE A.paymentid = J.paymentid AND A.ispaymentaccount = 1 AND A.entrytype = 'credit' AND A.plusminus = '+' "
	sSql = sSql & " AND A.reservationid = " & iReservationId
	sSql = sSql & " GROUP BY A.paymentid, J.paymentdate ORDER BY J.paymentdate"
	...
	If Not oRs.EOF Then 
		...
		Do While Not oRs.EOF
			...
			response.write "<td>&nbsp;</td>"
			response.write "<td align=""right"">"
			response.write FormatNumber(oRs("refundfeeamount"),2,,,0) 
			dTotalRefundFees = dTotalRefundFees + CDbl(oRs("refundfeeamount"))
			response.write "</td></tr>"
			oRs.MoveNext
		Loop
	End If 
	...
	displayReservationRefundFees = dTotalRefundFees
