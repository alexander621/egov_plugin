...
<div id="content">
	<div id="centercontent">
		...
		<%
			...
			subDisplayQuestions(request("irequestid"))
		%>
...
'------------------------------------------------------------------------------------------------------------
' SUB SUBDISPLAYQUESTIONS(IREQUESTID)
'------------------------------------------------------------------------------------------------------------
Sub subDisplayQuestions(irequestid)

	sSQL = "SELECT * FROM egov_submitted_request_fields WHERE submitted_request_id='" & irequestid & "'"
	Set oQuestions = Server.CreateObject("ADODB.Recordset")
	oQuestions.Open sSQL, Application("DSN"), 3, 1
	
	If NOT oQuestions.EOF Then
	
		response.write "<form name=""frmblob"" action=""correction_request_form_cgi.asp"" method=""POST"">"
	...
	Loop
		...
		If UserHasPermission( Session("UserId"), "internalfields" ) Then
			response.write "<tr><th class=correctionsinternal  align=left colspan=2>&nbsp;Internal Use Only - Administrative Fields</th></tr>"
			Call subDisplayAdminQuestions(irequestid)
		End If
	...
	Else
		...
		response.write "<P><div class=correctionsboxnotfound>We're sorry, you cannot edit this form using the original request question\answer formatting.  The form was submitted while the form correction feature was not enabled.</div></P>"
...
Function IsQuestionValueMatch(ifieldid,sValue,sTrueValue)
	...
	sSQL = "SELECT * FROM egov_submitted_request_field_responses WHERE submitted_request_field_id='" & ifieldid & "' AND submitted_request_field_response LIKE '" & sValue & "'"
	Set oQuestions = Server.CreateObject("ADODB.Recordset")
	oQuestions.Open sSQL, Application("DSN"), 3, 1
	...
	If NOT oQuestions.EOF Then
		...
		If Trim(sBlob) <> "" Then
			...
			For iQues = 1 to UBOUND(arrQues)
				...
				'DISPLAY
				response.write "<tr>"
				response.write "<td class=correctionslabel>" & UCASE(sQues) & "</td>"
				response.write "</tr>"
				response.write "<tr>"
				response.write "<td><textarea name=""answer" & iQues & """ class=correctionstextarea>" & sAnswer & "</textarea></td>"
				response.write "</tr>"
