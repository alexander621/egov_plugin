 ...
 Sub CreateDownload()
  	...
  	If NOT oSchema.EOF Then
    		...
    		For Each fldLoop in oSchema.Fields
          		if UCASE(fldLoop.Name) = "FORM VALUES" OR UCASE(fldLoop.Name) = "INTERNAL VALUES" then
          			response.write chr(34) & "" & chr(34) & ","
          		else
          			response.write chr(34) & fldLoop.Name & chr(34) & ","
          		end if
    		Next
		'response.write """Record Count: " & oSchema.RecordCount & ""","
		...
		response.flush
   		...
   		'WRITE DATA
      			...
      			For Each fldLoop in oSchema.Fields
        			...
        			'REMOVE LINE BREAKS
          			If NOT ISNULL(sFieldValue) Then
           				...
           				sFieldValue = replace(sFieldValue,chr(34),chr(34) & chr(34))
			'                sFieldValue = replace(sFieldValue,"</b><br></p>"," [] ")
			'                sFieldValue = replace(sFieldValue,"</b><br>"," [")
			'                sFieldValue = replace(sFieldValue,"</p>","] ")

                			lcl_blank_column = "," & chr(34) & "" & chr(34)
			'                sFieldValue = replace(sFieldValue,"</b><br></p>",chr(34) & "," & chr(34))
                			sFieldValue = replace(sFieldValue,"</b><br></p>",chr(34) & lcl_blank_column & "," & chr(34))
                			sFieldValue = replace(sFieldValue,"</b><br>",chr(34) & "," & chr(34))
                			sFieldValue = replace(sFieldValue,"</p>",chr(34) & "," & chr(34))
			...
			'                if fldLoop.Name = "Internal Values" then
                			if fldLoop.Name = "Form Values" then
                  				'Retrieve all of the internal questions
                      					...
                      					while not rs.eof
                        					'Retrieve all of the answers that have been selected for this question
                         					...
                         					if not rsa.eof then
                            						...
                            						lcl_answer_list = "testing"
                         					end if
	
					'                         lcl_internal_values = lcl_internal_values & rs("submitted_request_field_prompt") & " [" & lcl_answer_list & "] "
	
                          					response.write chr(34) & "[INTERNAL] " & rs("submitted_request_field_prompt") & chr(34) & ","
                          					response.write chr(34) & lcl_answer_list & chr(34) & ","
                         					rs.movenext
                      					wend
                   	  	   	     end if
