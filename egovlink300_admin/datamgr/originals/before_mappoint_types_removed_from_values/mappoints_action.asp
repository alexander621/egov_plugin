...
<%
 ...
 'Execute the user's action
  if lcl_useraction = "DELETE" then
    ...
    'The field is "INactive".  However, it appears as "ACTIVE" on the screen.
    'Therefore, if "checked" it means "ACTIVE" to the user so it is "FALSE" in the database.
    'If "UNchecked" then it means "INactive" to the user and therefore is "TRUE" in the database.
     if request("isInactive") = "Y" then
        lcl_isInactive = 0
     end if
    ...
    'END: Format the columns for the table ------------------------------------
     ...
     if lcl_useraction = "UPDATE" then
        ...
        sSQL = sSQL & "isInactive = "         & lcl_isInactive      & ", "
