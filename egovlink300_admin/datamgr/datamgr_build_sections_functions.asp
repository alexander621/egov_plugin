...
sub buildDMLayout(iLayoutID, iDMTypeID, iDMID, iDisplayFieldsetLegend, iDisplayFieldsetBorder, _
 ...
 'BEGIN: Display all active sections in current MapPoint Type Layout ----------
  ...
  response.write "  <table border=""0"" cellspacing=""0"" cellpadding=""2"" style=""width:" & lcl_totalLayoutWidth & "px;"">" & vbcrlf
 ...
 'BEGIN: Display all "unused" and "active" sections ---------------------------
  if sDisplayAvailableSections then
     ...
     response.write "        <td class=""column"" id=""column" & lcl_totalcolumns + 1 & """ colspan=""" & lcl_totalcolumns & """>" & vbcrlf
                                 ...
                                 buildLayoutColumns lcl_totalcolumns, _
                                                    "L", _
                                                    lcl_wrapWithTDTags, _
                                                    iDMTypeID, _
                                                    iDMID, _
                                                    lcl_columnwidth_left, _
                                                    lcl_displayUnusedSections, _
                                                    lcl_total_draggable_items, _
                                                    sSectionMode, _
                                                    lcl_total_draggable_items
  ...
  if iColumnWidth > 0 then
    ...
    '*** NOTE: in the "unused" and "active" sections, we do NOT want to pull any sections that are "Account Info" sections.
     ...
     if iDisplayUnusedSections then
        lcl_dm_orgid = getOrgIDByDMTypeID(iDMTypeID)
        ...
        sSQL = sSQL & " AND dms.section_orgid in (0, " & lcl_dm_orgid & ") "
  ...
  if iSectionID <> "" then
    ...
    'Determine which section to build
     if lcl_sectionmode = "DRAG" then
        ...
        displaySection_default iDMID, iDMTypeID, iSectionID, lcl_isAccountInfoSection
     end if
  end if
...
'------------------------------------------------------------------------------
sub displaySection_default(iDMID, iDMTypeID, iSectionID, iIsAccountInfoSection)
  ...
  if not oEditDMTSectionFields.eof then
     ...
     do while not oEditDMTSectionFields.eof
        ...
        if oEditDMTSectionFields("fieldname") <> "" then
           lcl_fieldname = "<strong>" & oEditDMTSectionFields("fieldname") & "</strong>"
        end if

        if not oEditDMTSectionFields("displayFieldName") then
...
'------------------------------------------------------------------------------
sub getSectionInfo(ByVal iSectionID, ByRef lcl_sectionname, ByRef lcl_sectiontype)
...
'------------------------------------------------------------------------------
function getOrgIDByDMTypeID(p_dm_typeid)
  dim lcl_return, sDMTypeID
  ...
  if sDMTypeID > 0 then
     ...
     set oGetOrgIDByDMT = Server.CreateObject("ADODB.Recordset")
     oGetOrgIDByDMT.Open sSQL, Application("DSN"), 3, 1

     if not oGetOrgIDByDMT.eof then
        lcl_return = oGetOrgIDByDMT("orgid")
     end if

     oGetOrgIDByDMT.close
     set oGetOrgIDByDMT = nothing
  end if

  getOrgIDByDMTypeID = lcl_return
