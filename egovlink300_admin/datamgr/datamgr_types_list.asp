...
'
'------------------------------------------------------------------------------
' FILENAME: datamgr_types_list.asp
' AUTHOR: ??
' CREATED: ??
' COPYRIGHT: Copyright 2009 eclink, inc.
'			 All Rights Reserved.
'
' Description:  This screen allows an e-gov root admin to maintain all dm types.
'
...
'Check for org features
 lcl_orghasfeature_datamgr             = orghasfeature("datamgr")
 lcl_orghasfeature_datamgr_types_maint = orghasfeature("datamgr_types_maint")

'Check for user permissions
 lcl_userhaspermission_datamgr             = userhaspermission(session("userid"),"datamgr")
 lcl_userhaspermission_datamgr_types_maint = userhaspermission(session("userid"),"datamgr_types_maint")
...
<!--
function confirm_delete(iDMTypeID) {
  lcl_description = document.getElementById("description"+iDMTypeID).innerHTML;
 	if (confirm("Are you sure you want to delete: '" + lcl_description + "' ?")) { 
  				//DELETE HAS BEEN VERIFIED
		  		location.href='datamgr_types_action.asp?user_action=DELETE<%=lcl_isTemplate_url%>&dm_typeid='+ iDMTypeID;
		}
}
...
'     response.write "                  <form name=""searchForm"" id=""searchForm"" action=""datamgr_types_list.asp"" method=""post"">" & vbcrlf
...
'     response.write "            </p>" & vbcrlf
     ...
     if lcl_isTemplate then
        response.write "                 <input type=""button"" name=""addTemplateButton"" id=""addTemplateButton"" value=""Add Template"" class=""button"" onclick=""location.href='datamgr_types_maint.asp?t=Y';"" />" & vbcrlf
     else
        response.write "                 <input type=""button"" name=""addDMTButton"" id=""addDMTButton"" value=""Add DM Type"" class=""button"" onclick=""location.href='datamgr_types_maint.asp';"" />" & vbcrlf
        response.write "                 <input type=""button"" name=""maintainTemplatesButton"" id=""maintainTemplatesButton"" value=""Maintain Templates"" class=""button"" onclick=""location.href='datamgr_types_list.asp?t=Y';"" />" & vbcrlf
     end if
     ...
     response.write "            </p>" & vbcrlf
                              ...
                              displayDMTypes session("orgid"), lcl_isRootAdmin, lcl_isTemplate
  response.write "          </div>" & vbcrlf
  response.write "      </td>" & vbcrlf
  response.write "  </tr>" & vbcrlf
  response.write "</table>" & vbcrlf
...
'------------------------------------------------------------------------------
sub displayDMTypes(p_orgid, p_isRootAdmin, p_isTemplate)
 	Dim iRowCount

 	iRowCount = 0

  sSQL = "SELECT dmt.dm_typeid, "
  sSQL = sSQL & " dmt.isActive, "
  sSQL = sSQL & " dmt.description, "
  'sSQL = sSQL & " dmt.mappointcolor, "
  sSQL = sSQL & " dmt.isTemplate, "
  sSQL = sSQL & " dmt.createdbyid, dmt.createdbydate, "
  sSQL = sSQL & " (select u.firstname + ' ' + u.lastname "
  sSQL = sSQL &   "from users u "
  sSQL = sSQL &   "where u.userid = dmt.createdbyid) as createdbyname, "
  sSQL = sSQL & " dmt.lastmodifiedbyid, "
  sSQL = sSQL & " dmt.lastmodifiedbydate, "
  sSQL = sSQL & " (select u2.firstname + ' ' + u2.lastname "
  sSQL = sSQL &   "from users u2 "
  sSQL = sSQL &   "where u2.userid = dmt.lastmodifiedbyid) as lastmodifiedbyname "
  sSQL = sSQL & " FROM egov_dm_types dmt "

  if p_isTemplate then
     sSQL = sSQL & " WHERE orgid = 0 "
     sSQL = sSQL & " AND dmt.isTemplate = 1 "
  else
 ...
 ' end if
     ...
     do while not oDMTypes.eof
       ...
       'Setup the date info
        ...
        if trim(oDMTypes("createdbydate")) <> "" then
           lcl_display_createdbydate = "[" & oDMTypes("createdbydate") & "]"
        end if

        if trim(oDMTypes("lastmodifiedbydate")) <> "" then
           lcl_display_lastmodifiedbydate = "[" & oDMTypes("lastmodifiedbydate") & "]"
        end if

       'Check to see if this MapPoint Type is active or not
        if oDMTypes("isActive") then
           lcl_isActive = "Y"
        else
           lcl_isActive = ""
        end if
       ...
       'Setup the onclick
        lcl_row_onclick = "location.href='datamgr_types_maint.asp?dm_typeid=" & oDMTypes("dm_typeid") & lcl_isTemplate_url & "';"

       'Check for associated DM Data to determine if this DM Type can/cannot be deleted.
       '*** NOTE: if this IS a template then allow then bypass the check.
        if lcl_isTemplate then
           lcl_canDelete = true
        else
           lcl_canDelete = checkForDMDataByDMTypeID(oDMTypes("dm_typeid"))
        end if

        if lcl_canDelete AND p_isRootAdmin then
           lcl_delete_row = "<input type=""button"" name=""delete" & iRowCount & """ id=""delete" & iRowCount & """ value=""Delete"" class=""button"" onclick=""confirm_delete('" & oDMTypes("dm_typeid") & "');"" />"
        else
           lcl_delete_row = "&nbsp;"
        end if
        ...
        response.write "      <td class=""formlist"" title=""click to edit"" onclick=""" & lcl_row_onclick & """><span id=""description" & oDMTypes("dm_typeid") & """>" & oDMTypes("description") & "</span></td>" & vbcrlf

        'if not p_isTemplate then
        '   response.write "      <td class=""formlist"" title=""click to edit"" onclick=""" & lcl_row_onclick & """>" & oDMTypes("mappointcolor") & "</td>" & vbcrlf
        'end if
        ...
        response.write            trim(oDMTypes("createdbyname")) & "<br />" & vbcrlf
        response.write "          <span style=""color:#800000;"">" & lcl_display_createdbydate & "</span>" & vbcrlf
        response.write "      </td>" & vbcrlf
        response.write "      <td class=""formlist"" title=""click to edit"" onclick=""" & lcl_row_onclick & """ width=""150"" nowrap=""nowrap"" align=""center"">" & vbcrlf
        response.write            trim(oDMTypes("lastmodifiedbyname")) & "<br />" & vbcrlf
     ...
     loop
  	  ...
  	  response.write "</div>" & vbcrlf
   		...
   		response.write "<p style=""padding-top:10px; color:#ff0000; font-weight:bold;"">No DM Types have been created.</p>" & vbcrlf
	 end if
