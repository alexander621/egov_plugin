...
'Set up the return Session variables for the page
 ...
 lcl_session_redirect_url = lcl_session_redirect_url & "../membershipcards/user_search.asp"
...
function goToCard(sUserID,sAction) {
  ...
  if(sAction=="CREATE") {
     lcl_card_url = 'user_takepic.asp';
  }else if (sAction=="REPRINT" || sAction=="PRINT") {
     lcl_card_url = 'user_displaycard.asp';
  }
...
<%
  response.write "<form name=""searchform"" id=""searchform"" action=""user_search.asp"" method=""post"">" & vbcrlf
 ...
 'END: Check for search criteria ----------------------------------------------
  ...
  if not oBuildUserList.eof then
     ...
     do while not oBuildUserList.eof
        ...
        lcl_familyid_url = lcl_familyid_url & "user_search.asp"
