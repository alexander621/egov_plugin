...
'Check to see if the feature is offline
 ...
 lcl_printcard_url = lcl_printcard_url & "user_displaycard_new.asp"
...
function card_print() {
  ...
  lcl_url_cardprint += '&OS=W7';

  lcl_url_carddisplay += 'user_displaycard_new.asp';
...
function retake_picture() {
  location.href="user_takepic_new.asp?userid=<%=lcl_userid%>&reload_pic=Y";
}
...
function remove_image() {
  location.href = "user_displaycard_new.asp?userid=<%=lcl_userid%>&action=CANCEL";
}

function card_save() {
  location.href = "user_displaycard_new.asp?userid=<%=lcl_userid%>&action=SAVE&card_layout=p";
}
</script>
...
<%
 ...
 'BEGIN: Display Instructions -------------------------------------------------
  ...
  response.write "                      <input type=""button"" value=""" & lcl_print_label & """ class=""noprint"" id=""card_print"" name=""card_print"" onclick=""card_print();"" />" & vbcrlf

  if lcl_action <> "CARD_PRINTED" then
     response.write "                   <input type=""button"" value=""Save Card"" class=""noprint"" id=""card_save"" name=""card_save"" onclick=""card_save();"" />" & vbcrlf
  end if
