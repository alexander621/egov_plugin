...
<%
  ...
  response.write "<p><input type=""button"" name=""returnButton"" id=""returnButton"" value=""Back to List"" class=""button"" onclick=""location.href='user_search_new.asp'"" /></p>" & vbcrlf
...
sub OutputForm(iUserID)
  ...
  response.write "<form name=""frmSend"" method=""post"" enctype=""multipart/form-data"" action=""user_takepic_new.asp?userid=" & sUserID & """ onsubmit=""return onSubmitForm();"">" & vbcrlf
...
sub SaveFiles(iUserID, iUploadsDirVar)
  ...
  'response.write lcl_uploadsdirvar
  'response.end
  ...
  Upload.Save(lcl_uploadsdirvar)
	...
	'If something fails inside the script, but the exception is handled
     ...
     if (UBound(ks) <> -1) then
        ...
        response.redirect "user_displaycard_new.asp?userid=" & sUserID
