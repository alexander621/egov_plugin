...
Class ImageUpload
	...
	Public Sub Save(path)
		...
		For Each fileItem In UploadedFiles.Items
			...
			'fileExt = right(fileItem.FileName,InstrRev(fileItem.FileName,".")-1)
			fileExt = mid(fileItem.FileName,InstrRev(fileItem.FileName,"."), len(fileItem.FileName))

			'streamFile.SaveToFile path & fileItem.FileName, 2
			streamFile.SaveToFile path & request.querystring("memberid") & fileExt, 2
			streamFile.close
			Set streamFile = Nothing
			'fileItem.Path = path & fileItem.FileName
			fileItem.Path = path & request.querystring("memberid") & fileExt
		 Next
	End Sub
	...
	Private Sub Upload()
		...
		if Err.Number <> 0 then 
			...
			response.write "The most likely cause for this error is the incorrect setup of AspMaxRequestEntityAllowed in IIS MetaBase. Please see instructions in the <A HREF='http://www.ImageUpload.net/ImageUpload/requirements.asp'>requirements page of ImageUpload.net</A>.<p>"
			Exit Sub
		end if
