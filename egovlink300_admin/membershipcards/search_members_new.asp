...
'
'------------------------------------------------------------------------------
' FILENAME: search_members_new.asp
' AUTHOR: David Boyer
' CREATED: 07/17/07
...
if lcl_demo = "Y" then
   ...
   if not userhaspermission(session("userid"),"create membership cards new") then
     	response.redirect sLevel & "permissiondenied.asp"
   end if
end if
...
'Set up the return Session variables for the page
 ...
 if lcl_family_id = "" then
    session("RedirectPage") = "../MembershipCards/search_members_new.asp?username=" & lcl_search_name_for_url & "&sc_show_expired=" & lcl_show_expired & "&sc_from_date=" & lcl_sc_from_date & "&sc_to_date=" & lcl_sc_to_date & "&demo=" & lcl_demo
 else
    session("RedirectPage") = "../MembershipCards/search_members_new.asp?familyid=" & lcl_family_id   & "&sc_show_expired=" & lcl_show_expired & "&sc_from_date=" & lcl_sc_from_date & "&sc_to_date=" & lcl_sc_to_date & "&demo=" & lcl_demo
 end if
...
function goToCard(sMemberID,sPoolPassID,sAction) {
  if(sAction=="CREATE") {
     location.href='image_takepic_new.asp?memberid=' + sMemberID + '&poolpassid=' + sPoolPassID + '&demo=<%=lcl_demo%>';
  }else if (sAction=="REPRINT") {
     location.href='image_display_new.asp?memberid=' + sMemberID + '&poolpassid=' + sPoolPassID + '&action=REPRINT&demo=<%=lcl_demo%>';
  }
}
...
<%
  ...
  response.write "      <td><font size=""+1""><strong>" & session("sOrgName") & "&nbsp;Search, Create, Reprint Member IDs" & lcl_demo_title & "</strong></font></td>" & vbcrlf
  response.write "  </tr>" & vbcrlf
 ...
 'BEGIN: Search Options -------------------------------------------------------
  ...
  response.write "           <form action=""search_members_new.asp"" method=""post"" name=""searchform"">" & vbcrlf
...
if iMembershipId <> "" then
 ...
 sSQL = sSQL & " AND R.membershipid     = " & oMembership.MembershipId
 end if
   ...
   numstartid	= (currentpage-1) * PageSize
     ...
     'Set up the Previous and Next buttons
      if lcl_totalpages > 1 then
        ...
        'Previous
         if currentpage > 1 then
            response.write "<a href=""search_members_new.asp?" & limit & "pagenum=" & currentpage-1 & "&username=" & p_sc_username & "&sc_show_expired=" & p_sc_show_expired & "&sc_from_date=" & p_sc_from_date & "&sc_to_date=" & p_sc_to_date & "&sMembershipType=" & lcl_membership_type & "&demo=" & p_demo & """>" & vbcrlf
         end if
     ...
     '----------------------------------------------------------------------
      ...
      for intRec = numstartid to numendid
        ...
        'If "no" then do not display anything.
         ...
         response.write "          <a href=""search_members_new.asp?familyid=" & lcl_familyid & "&demo=" & p_demo & "&username=" & p_sc_username & "&sc_show_expired=" & p_sc_show_expired & "&sc_from_date=" & p_sc_from_date & "&sc_to_date=" & p_sc_to_date & """ onMouseOver=""tooltip.show('Click to search on this Family ID');"" onMouseOut=""tooltip.hide();"">" & lcl_familyid & "</a></td>" & vbcrlf
