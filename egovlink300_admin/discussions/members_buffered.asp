...
<%
Dim oRst, i, sSql, sExisting, sAvailable, iID, sTask, sReload, bAdd, bDelete, sMembers, bBack, bForward, SelectID
...
If sTask = "add" Then
  For Each SelectID in Request.Form("RemainingList")
    If Len(sMembers) > 0 Then
      sMembers = sMembers & "," & SelectID
    Else
      sMembers = SelectID
    End If
  Next
  sReload = "onload=""window.opener.document.all.Members.value='" & sMembers & "';window.opener.document.all._task.value='reload';window.opener.document.frmNewDiscGroup.submit();"""
ElseIf sTask = "del" Then
  For Each SelectID in Request.Form("ExistingList")
    sMembers = Replace(sMembers, "," & SelectID, "")
    sMembers = Replace(sMembers, SelectID & ",", "")
    sMembers = Replace(sMembers, SelectID, "")
  Next
  sReload = "onload=""window.opener.document.all.Members.value='" & sMembers & "';window.opener.document.all._task.value='reload';window.opener.document.frmNewDiscGroup.submit();"""
