...
<!--BEGIN: SEARCH OPTIONS-->
	<fieldset id="search">
		...
		<form action="global_report1.asp" method="post" name="searchForm">
			<strong>From:</strong>
			...
			<span class="searchelement"><input type="submit" value="View Report" class="button" /></span>
...
Function DisplayPoolPassTotals( ByVal sFromDate, ByVal sToDate )
	...
	Dim sPassName, curTotal
		...
		sSql = "select  T.description + ' ' + R.description as passname, paymentlocation, count(P.poolpassid) as totalcount, "
		...
		sSql = sSql & " group by T.description, R.description, paymentlocation, T.displayorder, R.displayorder "
		sSql = sSql & " order by T.displayorder, R.displayorder, paymentlocation"
