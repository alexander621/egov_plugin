<%
' SQL Statement changes and removing individual rights lookup improved
' load time from ~.5s (Worst Average) - ~.325s (Best Average) to 
' ~.28s (Worst Average) - ~.141s (Best Average)
%>
<div id="mainmenuitems">
	...
	<script>
		document.addEventListener('DOMContentLoaded', function() {
			...
			{
				if (window.frameElement.hasAttribute("data-close"))
				{
					document.getElementById('pageheader').style.display = 'none';
					document.getElementById('logheader').style.display = 'none';
					document.getElementById('mainmenuitems').style.display = 'none';
					var ifstuff = document.getElementsByClassName('showiniframe');
					for (var i = 0; i < ifstuff.length; i++) {
  						ifstuff[i].style.display = 'inline-block';
					}
				}
			}
		});
...
Sub DisplayFeatureListNav( sLevel )
	...
	sSql = "Select F.featureid, F.featurename, F.haspermissions, F.haspermissionlevels, F.adminurl, "
	sSql = sSql & " Rights.FeatureID AS UserHasNavRights "
	sSql = sSql & " from egov_organization_features F " 
	sSql = sSql & " INNER JOIN egov_organizations_to_features FO  ON FO.featureid = F.featureid and FO.orgid = " & Session("orgid") & " " 
	sSql = sSql & " LEFT JOIN egov_users_to_features Rights ON Rights.FeatureID = F.featureid AND Rights.userid = " & session("UserID") & " "
	sSql = sSql & " where F.parentfeatureid = 0 and F.featuretype = 'N' "
	sSql = sSql & " Order By F.admindisplayorder, F.featurename"
	...
	If Not oFeatures.EOF Then
		...
		Do While NOT oFeatures.EOF
			If oFeatures("haspermissions") Then 
				...
				if not isNull(oFeatures("UserHasNavRights")) then 
					bUserCanNav = True
				else
					bUserCanNav = False
				end if
...
Sub GetChildrenFeatureListNav( iParentID, sLevel )
	...
	sSql = "SELECT F.haspermissions,F.adminurl,F.FeatureName,Rights.FeatureID AS UserHasNavRights FROM egov_organization_features F " 
	sSql = sSql & " INNER JOIN  egov_organizations_to_features FO ON FO.featureid = F.featureid and FO.orgid = " & Session("orgid") & " " 
	sSql = sSql & " LEFT JOIN egov_users_to_features Rights ON Rights.FeatureID = F.featureid AND Rights.userid = " & session("UserID") & " " 
	sSql = sSql & " where F.parentfeatureid = " & iParentID
	sSql = sSql & " and F.featuretype = 'N' Order By F.securitydisplayorder, F.featurename"
	Set oFeatures = Server.CreateObject("ADODB.Recordset")
	oFeatures.Open sSQL, Application("DSN"), 3, 1
